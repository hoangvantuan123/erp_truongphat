...
export const PostAUD = async (dataMaster, gridData, dataSheetAUD ) => {
    try {
        const token = await accessToken()

        const response = await axios.post(
            `${HOST_API_SERVER_1}/mssql/warehouse/slg-stock-real-open-result-aud`, {
                dataMaster, gridData, dataSheetAUD
