...
import { HOST_API_SERVER_17 } from '../../../services'
import {
    accessToken
} from '../../../services/tokenService'

export const auPjtProject = async (masterData, dataItem, dataDelv) => {
    try {
        const token = await accessToken()

        const response = await axios.post(
            `${HOST_API_SERVER_17}/pjt-project/au-pjt-project`, {
                masterData,
                dataItem,
                dataDelv,
        ...
        console.error('Error in AuHrAdmOrd:', error);
        return {
            success: false,
