...
export default function PdmmOutQueryDetailListSeq({
  ...
  const [dataLogsError, setDataLogsError] = useState([])
  ...
  const [openCheckLogsScan, setOpenCheckLogsScan] = useState(false)
  ...
  const addToScanHistory = useCallback((dataResSuccess, callback) => {
    setGridDataItem((prevHistory) => {
      const newItem = {
        ...
        MatOutSeq: dataResSuccess?.MatOutSeq,
        Remark: dataResSuccess?.Remark,
        ItemLotNo: dataResSuccess?.ItemLotNo,
      ...
      callback()
      return updatedHistoryWithIdxNo
    })
  }, [])
  ...
  const appendToLogs = (status, qrcode, logMessage) => {
  ...
  useEffect(() => {
    workerRef.current = new Worker(
      new URL(
        '../../../workers/workerPdmmOutQueryDetailList.js',
        import.meta.url,
      ),
    )
  ...
  const standardizeKeys = (data) => {
    return data.map((item) => {
      return {
        ...item,
        InOutReqItemSerl: item.OutReqItemSerl,
        InOutReqSeq: item.OutReqSeq,
        ReqSeq: item.OutReqSeq,
      }
    })
  }

  const FetchQOutProcItem = async (reqSeq) => {
    if (loadingBarRef.current) {
      loadingBarRef.current.continuousStart()
    }
    try {
      ...
      const etcOutResponse = await PostQOutProcItem(searchData)
      if (etcOutResponse.success) {
        const fetchedData = etcOutResponse.data || []
        const standardizedData = standardizeKeys(fetchedData)
        const updatedData = updateIndexNo(standardizedData)
        setGridDataItem(updatedData)
        setNumRowsItem(updatedData.length + 1)
      } else {
        setGridDataItem([])
        setNumRowsItem(0)
      }
  ...
  useEffect(() => {
    if (seq) {
      ...
      setIdOutReqSeq(data?.OutReqSeq)
      fetchSheetQuery(data?.OutReqSeq)
      if (data?.key === 'MatOutSeq') {
        FetchQOutProcItem(data?.MatOutSeq)
      }
    }
  }, [seq])
  ...
  const handleSaveData = useCallback(async () => {
    ...
    const dataMaster = [
      {
        ...
        EmpSeq: parsedUserInfo?.EmpSeq,
        EmpName: parsedUserInfo?.EmpName,
        Remark: remark,
    ...
    const dataSheetAUD = filterAndSelectColumns(
      gridDataItem,
      [
        'Status',
        'InOutSeq',
    ...
    try {
      ...
      if (response.success) {
        ...
        const newDataMaster = response.data.spdmmOutProcCheck ?? []

        // Gán dữ liệu mới trước khi gọi bất kỳ hành động tiếp theo
        if (newData.length > 0) {
          setInOutSeq(newData[0].InOutSeq || '')
          setInOutNo(newData[0].InOutNo || '')
        }

        // Cập nhật lại grid item
        setGridDataItem((prevGridData) =>
          prevGridData.map((item) => {
            const matchedData = newData.find(
      ...
      const dataMaster = [
        {
          ...
          Remark: remark,
          MatOutDate:
            gridDataItem[0]?.MatOutDate ?? gridData[0]?.InOutDate ?? '',
          MatOutSeq: gridDataItem[0]?.MatOutSeq ?? '',
          MatOutNo: gridDataItem[0]?.MatOutNo ?? '',
        },
      ]
      ...
      const dataMaster = [
        {
          ...
          EmpName: empName,
          Remark: remark,
          MatOutDate: gridDataItem[0]?.MatOutDate ?? '',
          MatOutSeq: gridDataItem[0]?.MatOutSeq ?? '',
          MatOutNo: gridDataItem[0]?.MatOutNo ?? '',
        },
      ]
  ...
  const handleClosCheckLogsScan = () => {
    setOpenCheckLogsScan(false)
  }
  const handleOpenCheckLogsScan = () => {
    setOpenCheckLogsScan(true)
  }
  ...
  return (
    <>
          ...
          <div className="col-start-1 col-end-5 row-start-1 w-full">
            <div className="flex p-2 items-end justify-end">
              <PdmmOutQueryDetailListSeqAction
                ...
                handleOpenCheckLogsScan={handleOpenCheckLogsScan}
              />
            </div>
            <details
          ...
          <div className="col-start-1 col-end-5 row-start-2 w-full flex-1 min-h-0 border-t overflow-hidden relative">
            <Splitter className="w-full h-full">
              ...
              <SplitterPanel size={50} minSize={20}>
                <div className="h-14 w-full border-l bg-white p-0">
      ...
      <CheckLogsDrawer
        ...
        IdOutReqSeq={IdOutReqSeq}
      />
      <CheckScanErrorLogsDrawer
        openLogs={openCheckLogsScan}
        handleCloseLogs={handleClosCheckLogsScan}
        dataLogsError={dataLogsError}
