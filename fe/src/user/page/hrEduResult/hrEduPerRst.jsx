...
export default function HrEduPerRst({
  permissions,
  isMobile,
  canCreate,
  canEdit,
  ...
  const handleSaveBasInfo = useCallback(
    async (dataMainInfo, dataRstCost, dataItem, dataPerObj) => {
      ...
      try {
        const results = await auHrEduPerRst(
          dataMainInfo,
          dataRstCost,
          dataItem,
          dataPerObj,
  ...
  const handleDeleteEduPerRst = useCallback(
    async (dataMainInfo, dataRstCost, dataMgn) => {
      ...
      try {
        const results = await deleteHrEduPerRst(
          dataMainInfo,
          dataRstCost,
          dataMgn,
        )
  ...
  const handleDelete = useCallback(async () => {
    ...
    handleDeleteEduPerRst(mainInfo, dataRstCost, dataMgn)
  ...
  const onClickSearch = useCallback(async () => {
    ...
    try {
      ...
      const response = await getEduRstById(searchParams)
      const { dataCost, dataItem, dataPersRst } = response.data
      ...
      formInfo.setFieldsValue({
        ...
        LecturerName: dataPersRst[0]?.LecturerName,
