...
import IqcReqListActions from '../../components/actions/iqc/iqcReqListActions'
import IqcReqListQuery from '../../components/query/iqc/iqcReqListQuery'
import TableIqcReqList from '../../components/table/iqc/tableIqcReqList'
import { SearchPage } from '../../../features/iqc/searchPage'
...
export default function IqcReqList({
  permissions,
  isMobile,
  canCreate,
  canEdit,
  ...
  const [BLDateFr, setBLDateFr] = useState(dayjs().startOf('month'))
  const [BLDateTo, setBLDateTo] = useState(dayjs())
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const response = await SearchPage(data)
  ...
  const onCellClicked = useCallback(
    (cell, event) => {
      ...
      if (rowIndex >= 0 && rowIndex < gridData.length) {
        ...
        const totalQty = (Number(rowData.OkQty) || 0) + (Number(rowData.BadQty) || 0);
        const filteredData = {
          BLNo: rowData.BLNo,
          ...
          Spec: rowData.Spec,
          Qty: totalQty === 0 ? rowData.Qty : totalQty,
          ReqQty: totalQty === 0 ? rowData.Qty : totalQty,
          ReqInQty: rowData.ReqInQty,
          PassedQty: rowData.OkQty,
          RejectQty: rowData.BadQty,
