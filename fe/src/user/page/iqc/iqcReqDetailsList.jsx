...
import { GetListQcTestReportBatch } from '../../../features/iqc/GetListQcTestReportBatch'
import { CreatedListBy } from '../../../features/iqc/createdListBy'
import TopLoadingBar from 'react-top-loading-bar';
import { GetCodeHelpVer2 } from '../../../features/codeHelp/getCodeHelpVer2'

export default function IqcReqDetailsList({
  permissions,
  isMobile,
  canCreate,
  canEdit,
  ...
  const [cols, setCols] = useState(() =>
    loadFromLocalStorageSheet(
      'S_ERP_COLS_PAGE_IQC_REQ_LIST_DETAILS',
  ...
  const handleSaveData = useCallback(async () => {
    ...
    if (resulA.length > 0 || resulU.length > 0) {
      try {
        ...
        if (resulA.length > 0) {
          promises.push(CreatedListBy(resulA))
        }

        if (resulU.length > 0) {
          promises.push(CreatedListBy(resulU))
        }
  ...
  const handleDeleteDataSheet = useCallback(
    ...
    const fetchGetListQcTestReportBatch = useCallback(async () => {
      ...
      setIsAPISuccess(false)
      try {
        const response = await GetListQcTestReportBatch(dataSelect) 
        if (response.success) {
