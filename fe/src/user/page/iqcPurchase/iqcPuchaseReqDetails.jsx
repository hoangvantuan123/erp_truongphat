...
import IqcPurchaseReqDetailsActions from '../../components/actions/iqc-purchase/iqcPurchaseReqDetailsActions'
import IqcPurchaseReqDetailQuery from '../../components/query/iqcPurchase/iqcPurchaseReqDetailsQuery'
...
import { DeleteBy } from '../../../features/iqc-purchase/deleteBy'
import { PostDFilesItems } from '../../../features/upload/postDFileItems'
import { GetCodeHelpVer2 } from '../../../features/codeHelp/getCodeHelpVer2'

export default function IqcPurchaseReqDetails({
  permissions,
  isMobile,
  canCreate,
  canEdit,
  ...
  const [modalOpenDeleteConfirm, setModalOpenDeleteConfirm] =
    useState(false)
  ...
  const [AcBadRatio, setAcBadRatio] = useState(0)
  ...
  const fetchCodeHelpData = useCallback(async () => {
    ...
    try {
      const [
        ...
        GetCodeHelpVer2(10009, '', '', '', '', '', '1', '', 50, 'TypeSeq = 3031001', 0, 0, 0),
  ...
  const handleDeleteData = useCallback(async () => {
    ...
    setIsAPISuccess(false)
    try {
      const response = await DeleteBy(result)
      setLoading(true)
      if (response.success) {
        const qcTestReportResult = response.data.data || []
  ...
  const handleUploadFiles = async (fileList, formCode, QCSeq) => {
        ...
        formData.append('tableName', 'PDQCTestReportPur')
    ...
    const fetchDataFileSeq = useCallback(async (ItemNoSeq, FormCode) => {
        const controller = new AbortController()
        const signal = controller.signal
        const TableName = 'PDQCTestReportPur'
  ...
  const fetchDetailsData = useCallback(async () => {
    if(!ItemSeq)
      return
    setLoading(true);
    if (controllers.current.fetchDetailsData) {
  ...
  useEffect(() => {
    if (id) {
      ...
      setQCSeq(data?.QCSeq)
      setReqQty(data?.Qty || data?.PassedQty)
  ...
  return (
    <>
        ...
        <div className="flex flex-col gap-4 md:grid md:grid-cols-4 md:grid-rows-[auto_1fr] md:gap-4 h-full">
          <div className="col-start-1 col-end-5 row-start-1 w-full rounded-lg ">
            <div className="flex items-center justify-between">
              ...
              <IqcPurchaseReqDetailsActions
                handleSaveData={handleSaveData}
                modalOpenDeleteConfirm = {modalOpenDeleteConfirm}
                setModalOpenDeleteConfirm = {setModalOpenDeleteConfirm}
      ...
      <ModalConfirm
        modalOpen={modalOpenDeleteConfirm}
        setmodalOpen={setModalOpenDeleteConfirm}
