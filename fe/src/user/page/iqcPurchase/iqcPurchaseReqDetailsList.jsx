...
import TableIqcDetailsListReq from '../../components/table/iqc/tableIqcDetailsListReq'
import IqcReqListDetailsActions from '../../components/actions/iqc/iqcReqListDetailsActions'
import { CreatedPurchaseListBy } from '../../../features/iqc-purchase/createdListBy'
import TopLoadingBar from 'react-top-loading-bar';
import { GetListQcPurchaseTestReport } from '../../../features/iqc-purchase/GetListQcPurchaseTestReport'
import { GetCodeHelpVer2 } from '../../../features/codeHelp/getCodeHelpVer2'

export default function IqcPurchaseReqDetailsList({
  permissions,
  isMobile,
  canCreate,
  canEdit,
  ...
  const [cols, setCols] = useState(() =>
    loadFromLocalStorageSheet(
      'S_ERP_COLS_PAGE_IQC_PURCHASE_LIST_DETAILS',
  ...
  const [QCEmployee, setQCEmployee] = useState(() => loadFromLocalStorageSheet('userInfo'))
  ...
  const fetchCodeHelpData = useCallback(async () => {
    ...
    try {
      const [
        ...
        GetCodeHelp(10009, '', '', '', '', '', '1', 1, 1000, 'TypeSeq = 3031001', 1, 0, 0),
        GetCodeHelpCombo('', 6, 19998, 1, '%', '6035', '1001', '', ''),
      ])
      setDataUser(dataUsers.data?.filter(item => item.TypeSeq === 3031001))
  ...
  const handleSaveData = useCallback(async () => {
    ...
    const resulU = filterAndSelectColumns(gridData, columnsU, 'U').map(item => ({
          ...item,
          QCDate: item.QCDate.trim() ? item.QCDate : formatDate(dayjs()),
        }))
    ...
    if (resulA.length > 0 || resulU.length > 0) {
      ...
      try {
        ...
        if (resulA.length > 0) {
          promises.push(CreatedPurchaseListBy(resulA))
        }

        if (resulU.length > 0) {
          promises.push(CreatedPurchaseListBy(resulU))
        }
  ...
  const handleDeleteDataSheet = useCallback(
    ...
    const fetchGetListQcTestReportBatch = useCallback(async () => {
      ...
      setIsAPISuccess(false)
      try {
        const response = await GetListQcPurchaseTestReport(dataSelect) 
        if (response.success) {
          const qcListQcTestReport = response.data.data || []
          const gridData = qcListQcTestReport.map(item => {
            if (item.QCEmpName === '' || item.QCEmpSeq === 0) {
              item.QCEmpName = QCEmployee.UserName
              item.QCEmpSeq = QCEmployee.UserSeq
            }
            return item
          })
