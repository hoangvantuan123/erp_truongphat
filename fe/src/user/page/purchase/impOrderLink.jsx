...
import ImpOrderActions from '../../components/actions/purchase/impOrderActions'
import ImpOrderQuery from '../../components/query/purchase/impOrderQuery'
import TableImpOrder from '../../components/table/purchase/tableImpOrder'
import TableUploadFileOrdPO from '../../components/table/purchase/tableUploadFileOrdPO'
const menuStyle = {}
import { GetMasterQuery } from '../../../features/purchase/impOrderLink/getMasterQuery'
import { GetSheetQuery } from '../../../features/purchase/impOrderLink/getSheetQuery'
import { getQFileSeq } from './../../../features/basic/daMaterialList/getQFileSeq'

import { PostAUD } from '../../../features/purchase/impOrder/postAUD'
import { PostMasterDelete } from '../../../features/purchase/impOrder/postMasterDelete'
import { PostSheetDelete } from '../../../features/purchase/impOrder/postSheetDelete'
...
export default function ImpOrderLink({
  permissions,
  isMobile,
  canCreate,
  canEdit,
  ...
  const defaultCols = useMemo(
    () => [
      ...
      {
        title: t('ApprovalReqCurAmt'),
        id: 'ApprovalReqCurAmt',
        kind: 'Text',
        readonly: true,
        width: 150,
        hasMenu: true,
  ...
  const [dataUMPriceTermsName, setDataUMPriceTermsName] = useState([])
  const [dataUMPayment1Name, setDataUMPayment1Name] = useState([])
  const [dataUMPayment2Name, setDataUMPayment2Name] = useState([])
  const [dataUMShipViaName, setDataUMShipViaName] = useState([])
  ...
  const calculateTotalQuantityApproval = () => {
    const total = gridData.reduce(
      (sum, item) => sum + (item.ApprovalReqQty || 0),
      0,
    )
  ...
  const [uMPriceTermsName, setUMPriceTermsName] = useState('0')
  const [uMPayment1Name, setUMPayment1Name] = useState('0')
  const [uMPayment2Name, setUMPayment2Name] = useState('0')
  const [uMShipViaName, setUMShipViaName] = useState('0')
  ...
  const [approReqSeq, setApproReqSeq] = useState('0')
  ...
  useEffect(() => {
    if (id) {
      ...
      if (data) {
        fetchDataFileSeq(data.POSeq, 'file')
        fetchMasterQuery(data.POSeq)
        fetchSheetQuery(data.POSeq)
      }
    }
  }, [id])
    ...
    try {
      ...
      if (masterResponse.data.success) {
        ...
        if (data) {
          ...
          setApproReqSeq(data[0].ApproReqSeq)
          setPOSeq(data[0].POSeq)
          setPORefNo(data[0].POMngNo)
          setPONo(data[0].PONo)
          setRemark(data[0].Remark)
          setPayment3(data[0].Payment3)
          setUMPriceTermsName(data[0].UMPriceTermsName)
          setUMPayment1Name(data[0].UMPayment1Name)
          setUMPayment2Name(data[0].UMPayment2Name)
          setUMShipViaName(data[0].UMShipViaName)
  ...
  const fetchSheetQuery = async (pOSeq) => {
    try {
  ...
  const fetchCodeHelpData = useCallback(async () => {
    ...
    try {
      const [
        ...
        codeHelpPurTypeName,
        codeHelpPaymentType,
        codeHelpUMPriceTermsName,
        codeHelpUMPayment1Name,
        codeHelpUMPayment2Name,
        codeHelpUMShipViaName,
        codeHelpWarehouse,
        codeHelpDeptName,
        codeHelpUserName,
        codeHelpCustName,
        ...
        GetCodeHelpCombo('', 6, 19999, 1, '%', '4008', '', '', ''),

        GetCodeHelpCombo('', 6, 19999, 1, '%', '8201', '', '', ''),
        GetCodeHelpCombo('', 6, 19999, 1, '%', '8202', '', '', ''),
        GetCodeHelpCombo('', 6, 19999, 1, '%', '8203', '', '', ''),
        GetCodeHelpCombo('', 6, 19999, 1, '%', '8204', '', '', ''),
      ...
      setDataPaymentType(codeHelpPaymentType?.data || [])
      setDataUMPriceTermsName(codeHelpUMPriceTermsName?.data || [])
      setDataUMPayment1Name(codeHelpUMPayment1Name?.data || [])
      setDataUMPayment2Name(codeHelpUMPayment2Name?.data || [])
      setDataUMShipViaName(codeHelpUMShipViaName?.data || [])
      ...
      setDataPurTypeName([])
      setDataPaymentType([])
      setDataUMPriceTermsName([])
      setDataUMPayment1Name([])
      setDataUMPayment2Name([])
      setDataUMShipViaName([])
      setDataWarehouse([])
      setDataDeptName([])
      setDataUserName([])
      setDataCustName([])
  ...
  const handleUploadFiles = async (fileList, formCode, pOSeq) => {
    ...
    formData.append('itemNoSeq', pOSeq)
    formData.append('tableName', '_TSLImpOrder')
  ...
  const handleSaveData = useCallback(async () => {
    ...
    if (pORefNo === '') {
      message.warning('"Số P/O" không được để trống hoặc null!')
      return
    }
    ...
    if (uMPriceTermsName === '0' || uMPriceTermsName === '') {
      message.warning('"Điều kiện giá" không được để trống hoặc null!')
      return
    }
    ...
    const dataMaster = [
      {
        POSeq: pOSeq,
        PONo: pONo,
        PODate: fromDate ? formatDate(fromDate) : '',
        BizUnit: bizUnit,
        CurrSeq: currSeq,
        CurrName: currName,
        ExRate: exRate,
        ...
        POMngNo: pORefNo,
        PORefNo: pORefNo,
        UMPriceTerms: uMPriceTermsName,
        UMPayment1: uMPayment1Name,
        UMPayment2: uMPayment2Name,
        Payment3: payment3,
        UMShipVia: uMShipViaName,
        SMImpType: purTypeName,
        ApproReqSeq: approReqSeq,
      },
    ]
    ...
    try {
      ...
      if (results[0].data.success) {
        ...
        setIsSent(false)
        setPOSeq(newData[0].POSeq)
        setPONo(newData[0].PONo)
        setEditedRows([])
        updateGridData(newData)
        resetTable()
        return newData[0].POSeq
      } else {
        togglePageInteraction(false)
        setIsAPISuccess(true)
        setIsAPISuccess(true)
  ...
  const handleSaveAll = async () => {
    try {
      const newPOSeq = await handleSaveData()
      if (newPOSeq && newPOSeq !== '0') {
        const saveB = await handleUploadFiles(fileList, 'file', newPOSeq)
        if (!saveB) {
  ...
  const handleDeleteDataSheet = useCallback(
    (e) => {
      ...
      console.log('rowsWithStatusA', rowsWithStatusA)
  ...
  const handleDeleteDataMaster = useCallback(async () => {
    ...
    const idsWithStatusD = selectedRows
      .filter(
        (row) =>
          row.POSeq !== undefined && row.POSeq !== '0' && row.POSeq !== '',
      )
      .map((row) => {
        row.Status = 'D'
        return row
    ...
    if (pOSeq === '0' || pOSeq === '' || idsWithStatusD.length === 0) {
      resetTable()
  ...
  const items = [
    {
      ...
      children: (
        <TableImpOrder
          ...
          dataPaymentType={dataPaymentType}
  ...
  return (
    <>
      <Helmet>
        <title>ITM - {t('Nhập đặt hàng nhập khẩu')}</title>
      </Helmet>
            ...
            >
              ...
              <div className="flex p-2 gap-4">
                <ImpOrderQuery
                  ...
                  uMPriceTermsName={uMPriceTermsName}
                  dataUMPriceTermsName={dataUMPriceTermsName}
                  setUMPriceTermsName={setUMPriceTermsName}
                  uMPayment1Name={uMPayment1Name}
                  dataUMPayment1Name={dataUMPayment1Name}
                  setUMPayment1Name={setUMPayment1Name}
                  uMPayment2Name={uMPayment2Name}
                  dataUMPayment2Name={dataUMPayment2Name}
                  setUMPayment2Name={setUMPayment2Name}
                  uMShipViaName={uMShipViaName}
                  dataUMShipViaName={dataUMShipViaName}
                  setUMShipViaName={setUMShipViaName}
                  whName={whName}
                  setWhName={setWhName}
                  setWhSeq={setWhSeq}
                  ...
                  payment3={payment3}
                  setPayment3={setPayment3}
                  pONo={pONo}
                  setPONo={setPONo}
                  pORefNo={pORefNo}
                  setPORefNo={setPORefNo}
                  remark={remark}
                  setRemark={setRemark}
                  isReset={isReset}
