...
import PurDelvItemListQuery from '../../components/query/purchase/purDelvItemListQuery'
import PurDelvItemListActions from '../../components/actions/purchase/purDelvItemListActions'
import TablePurDelvItemList from '../../components/table/purchase/tablePurDelvItemList'
import { GetSheetQuery } from '../../../features/purchase/purDelvItemList/getSheetQuery'
...
export default function PurDelvItemList({
  permissions,
  isMobile,
  canEdit,
  setCollapsed,
  ...
  const defaultCols = useMemo(
    () => [
      ...
      {
        title: t('Số lượng chưa nhập kho'),
        id: 'RemainQty',
        kind: 'Text',
        readonly: true,
        width: 120,
  ...
  const [cols, setCols] = useState(() =>
    loadFromLocalStorageSheet(
      'S_ERP_COLS_PAGE_DELV_ITEM_LIST',
  ...
  const nextPage = useCallback(() => {
    if (keyPath && sMImpType === 8008001) {
      window.open(`/pur/u/purchase/pu-delv-in/${keyPath}`, '_blank')
    }
  }, [keyPath])
  ...
  useEffect(() => {
    ...
    if (data && data.length > 0) {
      if (data[0].DelvSeq !== '') {
        if (data[0].RemainQty === 0) {
          setKeyPath(null)
          message.warning('Đã hoàn thành nhập kho!')
        } else {
          const filteredData = {
  ...
  return (
    <>
      <Helmet>
        <title>HPM - {t('Truy vấn chi tiết giao hàng trong nước')}</title>
      </Helmet>
          ...
          <div className="col-start-1 col-end-5 row-start-1 w-full rounded-lg ">
            <div className="flex items-center justify-between mb-2">
              <Title level={4} className="mt-2 uppercase opacity-85 ">
                {t('Truy vấn chi tiết giao hàng trong nước')}
              </Title>
              <PurDelvItemListActions
                nextPage={nextPage}
                debouncedFetchSPUDelvItemListQueryWEB={fetchSheetQuery}
              />
            </div>
            <details
