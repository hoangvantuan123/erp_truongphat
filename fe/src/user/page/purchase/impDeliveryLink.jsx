...
import { GetMasterQuery } from '../../../features/purchase/impDelivery/getMasterQuery'
import { GetSheetQuery } from '../../../features/purchase/impDelivery/getSheetQuery'
...
export default function ImpDeliveryLink({
  permissions,
  isMobile,
  canCreate,
  canEdit,
  ...
  useEffect(() => {
    if (id) {
      ...
      if (data) {
        ...
        fetchSheetLinkQuery(data.DelvSeq)
    ...
    try {
      ...
      if (masterResponse.data.success) {
        ...
        if (fetchedData.length > 0) {
          ...
          setDelvSeq(fetchedData[0].DelvSeq)
          setDelvNo(fetchedData[0].DelvNo)
  ...
  const fetchSheetLinkQuery = async (delvSeq) => {
    try {
      if (loadingBarRef.current) {
        loadingBarRef.current.continuousStart()
      }
      const sheetResponse = await GetSheetQuery(delvSeq)
