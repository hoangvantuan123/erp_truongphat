...
import PurOrderPOItemListQuery from '../../components/query/purchase/purOrderPOItemListQuery'
import PurOrderPOItemListActions from '../../components/actions/purchase/purOrderPOItemListActions'
import TablePurOrdPOItemList from '../../components/table/purchase/tablePurOrdPOItemList'
import { GetSheetQuery } from '../../../features/purchase/purOrdPOItemList/getSheetQuery'
...
export default function PurOrdPOItemList({
  permissions,
  isMobile,
  canEdit,
  setCollapsed,
  ...
  const [totalDelvQty, setTotalDelvQty] = useState(0)
  const [totalRemainQty, setTotalRemainQty] = useState(0)
  const [totalDelvCurAmt, setTotalDelvCurAmt] = useState(0)
  const [totalDelvDomAmt, setTotalDelvDomAmt] = useState(0)
  const [totalDelvInQty, setTotalDelvInQty] = useState(0)
  const [totalDelvInCurAmt, setTotalDelvInCurAmt] = useState(0)
  const [totalDelvInDomAmt, setTotalDelvInDomAmt] = useState(0)
  const [totalStopQty, setTotalStopQty] = useState(0)
  ...
  const defaultCols = useMemo(
    () => [
...

      {
        ...
        visible: true,
        trailingRowOptions: {
          hint: '' + formatNumberCellSum(totalDelvQty),
          addIcon: GridColumnIcon.HeaderMath,
          disabled: false,
          themeOverride: {
            textDark: '#009CA6',
...

      {
        ...
        visible: true,
        trailingRowOptions: {
          hint: '' + formatNumberCellSum(totalDelvCurAmt),
          addIcon: GridColumnIcon.HeaderMath,
          disabled: false,
          themeOverride: {
            textDark: '#009CA6',
...

      {
        ...
        visible: true,
        trailingRowOptions: {
          hint: '' + formatNumberCellSum(totalDelvDomAmt),
          addIcon: GridColumnIcon.HeaderMath,
          disabled: false,
          themeOverride: {
            textDark: '#009CA6',
...

      {
        ...
        visible: true,
        trailingRowOptions: {
          hint: '' + formatNumberCellSum(totalDelvInQty),
          addIcon: GridColumnIcon.HeaderMath,
          disabled: false,
          themeOverride: {
            textDark: '#009CA6',
      ...
      },
      {
        ...
        visible: true,
        trailingRowOptions: {
          hint: '' + formatNumberCellSum(totalDelvInCurAmt),
          addIcon: GridColumnIcon.HeaderMath,
          disabled: false,
          themeOverride: {
            textDark: '#009CA6',
      ...
      },
      {
        ...
        visible: true,
        trailingRowOptions: {
          hint: '' + formatNumberCellSum(totalDelvInDomAmt),
          addIcon: GridColumnIcon.HeaderMath,
          disabled: false,
          themeOverride: {
            textDark: '#009CA6',
      ...
      },
      {
        ...
        visible: true,
        trailingRowOptions: {
          hint: '' + formatNumberCellSum(totalStopQty),
          addIcon: GridColumnIcon.HeaderMath,
          disabled: false,
          themeOverride: {
            textDark: '#009CA6',
  ...
  const [sMImpType, setSMImpType] = useState(null)
  ...
  const calculateTotal = () => {
    const totalDelvQty = gridData.reduce(
      (sum, item) => sum + (item.DelvQty || 0),
      0,
    )
    setTotalDelvQty(totalDelvQty)
    const totalRemainQty = gridData.reduce(
      (sum, item) => sum + (item.RemainQty || 0),
      0,
    )
    setTotalRemainQty(totalRemainQty)
    const totalDelvCurAmt = gridData.reduce(
      (sum, item) => sum + (item.DelvCurAmt || 0),
      0,
    )
    setTotalDelvCurAmt(totalDelvCurAmt)
    const totalDelvDomAmt = gridData.reduce(
      (sum, item) => sum + (item.DelvDomAmt || 0),
      0,
    )
    setTotalDelvDomAmt(totalDelvDomAmt)
    const totalDelvInQty = gridData.reduce(
      (sum, item) => sum + (item.DelvInQty || 0),
      0,
    )
    setTotalDelvInQty(totalDelvInQty)
    const totalDelvInCurAmt = gridData.reduce(
      (sum, item) => sum + (item.DelvInCurAmt || 0),
      0,
    )
    setTotalDelvInCurAmt(totalDelvInCurAmt)
    const totalDelvInDomAmt = gridData.reduce(
      (sum, item) => sum + (item.DelvInDomAmt || 0),
      0,
    )
    setTotalDelvInDomAmt(totalDelvInDomAmt)
    const totalStopQty = gridData.reduce(
      (sum, item) => sum + (item.StopQty || 0),
      0,
    )
    setTotalStopQty(totalStopQty)
  }
  ...
  const [cols, setCols] = useState(() =>
    loadFromLocalStorageSheet(
      'S_ERP_COLS_PAGE_ORD_PO_ITEM_LIST',
  ...
  const nextPage = useCallback(() => {
    if (keyPath && sMImpType === 8008001) {
      window.open(`/pur/u/purchase/pu-delv/${keyPath}`, '_blank')
    }
  }, [keyPath])
  ...
  useEffect(() => {
    ...
    if (data && data.length > 0) {
      if (data[0].POSeq !== '') {
        ...
        } else if (data[0].CompleteCHECK === 40) {
          setKeyPath(null)
          message.warning('Đã tồn tại dữ liệu đặt mua hàng!')
        } else if (data[0].IsStop === '1') {
          setKeyPath(null)
          message.warning('Đơn yêu cầu đã bị dừng lại!')
        } else if (data[0].RemainQty === 0) {
          setKeyPath(null)
          message.warning('Đã hết số lượng giao hàng!')
        } else {
          const filteredData = {
            POSeq: data[0].POSeq,
          }
          ...
          <div className="col-start-1 col-end-5 row-start-1 w-full rounded-lg ">
            <div className="flex items-center justify-between mb-2">
              <Title level={4} className="mt-2 uppercase opacity-85 ">
                {t('Truy vấn chi tiết nhập đặt hàng trong nước')}
              </Title>
              <PurOrderPOItemListActions
                nextPage={nextPage}
                debouncedFetchSPUORDPOItemListQueryWEB={fetchSheetQuery}
              />
            </div>
            <details
