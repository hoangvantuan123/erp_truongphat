...
import PurOrderApprovalReqActions from '../../components/actions/purchase/purOrderApprovalReqActions'
import PurOrderApprovalReqQuery from '../../components/query/purchase/purOrderApprovalReqQuery'

import TablePurOrdApprovalReq from '../../components/table/purchase/tablePurOrdApprovalReq'
import TableUploadFileOrd from '../../components/table/purchase/tableUploadFileOrd'
const menuStyle = {}
import { GetMasterQuery } from '../../../features/purchase/purOrdApprovalReq/getMasterQuery'
import { GetSheetQuery } from '../../../features/purchase/purOrdApprovalReq/getSheetQuery'
import { getQFileSeq } from './../../../features/basic/daMaterialList/getQFileSeq'

import { PostAUD } from '../../../features/purchase/purOrdApprovalReq/postAUD'
import { PostMasterDelete } from '../../../features/purchase/purOrdApprovalReq/postMasterDelete'
import { PostSheetDelete } from '../../../features/purchase/purOrdApprovalReq/postSheetDelete'
...
export default function PurOrdApprovalReqLink({
  permissions,
  isMobile,
  canCreate,
  canEdit,
  ...
  const [dataExRateCurr, setDataExRateCurr] = useState([])
  ...
  const [cols, setCols] = useState(() =>
    loadFromLocalStorageSheet(
      'S_ERP_COLS_PAGE_PUR_ORD_APPROVAL_REQ',
      defaultCols.filter((col) => col.visible),
    ),
  )
  const [colsC, setColsC] = useState(() =>
    loadFromLocalStorageSheet(
      'ORD_APPROVAL_REQ_UPLOAD',
  ...
  const fetchDataFileSeq = useCallback(async (ItemNoSeq, FormCode) => {
    ...
    const TableName = '_TPUORDApprovalReq'
    try {
  ...
  const fetchMasterQuery = async (approReqSeq) => {
    try {
      ...
      if (masterResponse.data.success) {
        ...
        if (data) {
          setFromDate(dayjs(data[0].ApproReqDate))
          ...
          setApprovalNo(data[0].ApproReqNo)
          ...
          setDocumentNo(data[0].DocumentNo)
  ...
  const fetchCodeHelpData = useCallback(async () => {
    ...
    try {
      ...
      setDataExRateCurr(codeHelpExrateCurr?.data || [])
    } catch (error) {
      setDataBizUnit([])
      setDataPurTypeName([])
      setDataItemName([])
  ...
  const handleUploadFiles = async (fileList, formCode, approReqSeq) => {
    ...
    formData.append('itemNoSeq', approReqSeq)
    formData.append('tableName', '_TPUORDApprovalReq')
    ...
    if (result.data.success) {
      ...
      fetchDataFileSeq(approReqSeq, 'file')
      setFileList([])
      return true
    } else {
  ...
  const handleSaveData = useCallback(async () => {
    ...
    const dataMaster = [
      {
        ...
        ApproReqNo: approvalNo,
        ApproReqDate: fromDate ? formatDate(fromDate) : '',
        BizUnit: bizUnit,
        DeptSeq: deptSeq,
        DeptName: deptName,
        WHSeq: whSeq,
    ...
    if (!validateColumns(dataSheetAUD, ['CurrName'])) {
      message.warning('Cột "Tiền tệ" không được để trống hoặc null!')
      return
    }
    if (!validateColumns(dataSheetAUD, ['ExRate'])) {
      message.warning('Cột "Tỷ giá" không được để trống hoặc null!')
      return
    }
    ...
    try {
      ...
      if (results[0].data.success) {
        ...
        setIsSent(false)
        setApproReqSeq(newData[0].ApproReqSeq)
        setApprovalNo(newData[0].ApproReqNo)
        setEditedRows([])
        updateGridData(newData)
        resetTable()
        return newData[0].ApproReqSeq
      } else {
        togglePageInteraction(false)
        setIsAPISuccess(true)
        setIsAPISuccess(true)
  ...
  const handleSaveAll = async () => {
    try {
      const newApproReqSeq = await handleSaveData()
      if (newApproReqSeq && newApproReqSeq !== '0') {
        const saveB = await handleUploadFiles(fileList, 'file', newApproReqSeq)
        if (!saveB) {
  ...
  const items = [
    {
      key: '0',
      label: 'Thông tin xin phép mua hàng',
      children: (
        <TablePurOrdApprovalReq
          ...
          <div className="col-start-1 col-end-5 row-start-1 w-full rounded-lg ">
            <div className="flex items-center justify-between mb-2">
              <Title level={4} className="mt-2 uppercase opacity-85 ">
                {t('Nhập xin phép mua hàng')}
              </Title>
              <PurOrderApprovalReqActions
