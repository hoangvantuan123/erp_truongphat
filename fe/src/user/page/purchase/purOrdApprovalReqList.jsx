...
import PurOrderApprovalReqListQuery from '../../components/query/purchase/purOrderApprovalReqListQuery'
import PurOrderApprovalReqListActions from '../../components/actions/purchase/purOrderApprovalReqListActions'
import TablePurOrdApprovalReqList from '../../components/table/purchase/tablePurOrdApprovalReqList'
import { GetSheetQuery } from '../../../features/purchase/purOrdApprovalReqList/getSheetQuery'
import { PostApprovalReqListConfirm } from '../../../features/purchase/purOrdApprovalReqList/postApprovalReqListConfirm'
import { PostApprovalReqListStop } from '../../../features/purchase/purOrdApprovalReqList/postApprovalReqListStop'
...
export default function PurOrdApprovalReqList({
  permissions,
  isMobile,
  canEdit,
  controllers,
  ...
  const [cols, setCols] = useState(() =>
    loadFromLocalStorageSheet(
      'S_ERP_COLS_PAGE_ORD_APPROVAL_REQ_LIST',
  ...
  const [approReqSeq, setApproReqSeq] = useState('')
  ...
  const [approReqNo, setApproReqNo] = useState('')
  ...
  useEffect(() => {
    const savedState = localStorage.getItem('detailsStateOrdApprovalReqList')
  ...
  const handleToggle = (event) => {
    ...
    localStorage.setItem(
      'detailsStateOrdApprovalReqList',
      isOpen ? 'open' : 'closed',
    )
  }

  const fetchSPUORDApprovalReqListQueryWEB = async () => {
      ...
      const data = [
        {
          ...
          BizUnit: bizUnit,
          BizUnitName: '',
          ApproReqDateFr: fromDate ? formatDate(fromDate) : '',
          ApproReqDateTo: toDate ? formatDate(toDate) : '',
          WHSeq: whSeq,
  ...
  const nextPage = useCallback(() => {
    if (keyPath) {
      window.open(`/pur/u/pur-order/ord-approval-req/${keyPath}`, '_blank')
    }
  }, [keyPath])
  //Sheet
  ...
  useEffect(() => {
    ...
    if (data && data.length > 0) {
      if (data[0].ApproReqSeq !== '') {
        const filteredData = {
          ApproReqSeq: data[0].ApproReqSeq,
        }
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      ...
      if ((e.ctrlKey || e.metaKey) && e.key === 'q') {
        e.preventDefault()
        fetchSPUORDApprovalReqListQueryWEB()
      }
    }
    ...
    if (resulAUD.length > 0 || resulAUD.length > 0) {
      ...
      try {
        ...
        if (resulAUD.length > 0) {
          promises.push(PostApprovalReqListConfirm(resulAUD))
        }
  ...
  }, [rowIndex, reason, approReqSeq, isConfirm, isAPISuccess])
  ...
  const handleStopDataSheet = useCallback(async () => {
    ...
    if (resulAUD.length > 0 || resulAUD.length > 0) {
      ...
      try {
        ...
        if (resulAUD.length > 0) {
          promises.push(PostApprovalReqListStop(resulAUD))
        }
  ...
  }, [rowIndex, reason, approReqSeq, isStop, isAPISuccess])
  return (
    <>
      <Helmet>
        <title>HPM - {t('Truy vấn xin phép mua hàng')}</title>
      </Helmet>
          ...
          <div className="col-start-1 col-end-5 row-start-1 w-full rounded-lg ">
            <div className="flex items-center justify-between mb-2">
              <Title level={4} className="mt-2 uppercase opacity-85 ">
                {t('Truy vấn xin phép mua hàng')}
              </Title>
              <PurOrderApprovalReqListActions
                nextPage={nextPage}
                debouncedFetchSPUORDApprovalReqListQueryWEB={
                  fetchSPUORDApprovalReqListQueryWEB
                }
              />
            </div>
          ...
          <div className="col-start-1 col-end-5 row-start-2 w-full h-full rounded-lg">
            {
              <TablePurOrdApprovalReqList
                ...
                setApproReqSeq={setApproReqSeq}
                setApproReqNo={setApproReqNo}
