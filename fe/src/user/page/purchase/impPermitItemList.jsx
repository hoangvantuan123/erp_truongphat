...
import ImpPermitItemListQuery from '../../components/query/purchase/impPermitItemListQuery'
import ImpPermitItemListActions from '../../components/actions/purchase/impPermitItemListActions'
import TableImpPermitItemList from '../../components/table/purchase/tableImpPermitItemList'
import { GetSheetQuery } from '../../../features/purchase/impPermitItemList/getSheetQuery'
...
export default function ImpPermitItemList({
  permissions,
  isMobile,
  canEdit,
  setCollapsed,
  ...
  const [totalTaxDomAmount, setTotalTaxDomAmount] = useState(0)
  ...
  const defaultCols = useMemo(
    () => [
      ...
      },
      {
        ...
        visible: true,
        trailingRowOptions: {
          hint: '' + formatNumberCellSum(totalTaxAmount),
          addIcon: GridColumnIcon.HeaderMath,
          disabled: false,
          themeOverride: {
            textDark: '#009CA6',
      ...
      },
      {
        ...
        visible: true,
        trailingRowOptions: {
          hint: '' + formatNumberCellSum(totalTaxDomAmount),
          addIcon: GridColumnIcon.HeaderMath,
          disabled: false,
          themeOverride: {
            textDark: '#009CA6',
  ...
  const [cols, setCols] = useState(() =>
    loadFromLocalStorageSheet(
      'S_ERP_COLS_PAGE_IMP_PERMIT_ITEM_LIST',
  ...
  const calculateTotalTaxAmount = () => {
    const total = gridData.reduce((sum, item) => sum + (item.TaxAmt || 0), 0)
    setTotalTaxAmount(total)
  }
  const calculateTotalTaxDomAmount = () => {
    const total = gridData.reduce((sum, item) => sum + (item.TaxDomAmt || 0), 0)
    setTotalTaxDomAmount(total)
  }
  ...
  //   if (keyPath) {
  //     navigate(`/pur/u/import/imp-delivery/${keyPath}`)
  //     handClose()
  //   }
  // }, [keyPath, navigate])
  ...
  const nextPage = useCallback(() => {
    if (keyPath) {
      window.open(`/pur/u/import/imp-delivery/${keyPath}`, '_blank')
    }
  }, [keyPath])
  //Sheet
  ...
  useEffect(() => {
    ...
    if (data && data.length > 0) {
      if (data[0].PermitSeq !== '') {
        const filteredData = {
          PermitSeq: data[0].PermitSeq,
          SMProgressType: data[0].SMProgressType,
        ...
        if (isAPISuccess === true) {
          if (
            filteredData.IsStop === 1 ||
            filteredData.SMProgressType === 1037008
          ) {
            if (filteredData.IsStop === 1) {
              setCheckIsStop(true)
            }
            if (filteredData.SMProgressType === 1037008) {
          ...
          <div className="col-start-1 col-end-5 row-start-1 w-full rounded-lg ">
            <div className="flex items-center justify-between mb-2">
              <Title level={4} className="mt-2 uppercase opacity-85 ">
                {t('Truy vấn chi tiết tờ khai nhập khẩu')}
              </Title>
              <ImpPermitItemListActions
                nextPage={nextPage}
                debouncedFetchSSLImpPermitItemListQueryWEB={fetchSheetQuery}
              />
            </div>
            <details
