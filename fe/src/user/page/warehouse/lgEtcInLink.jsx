...
import LGEtcInActions from '../../components/actions/warehouse/lgEtcInActions'
import LGEtcInQuery from '../../components/query/warehouse/lgEtcInQuery'
import TableLGEtcIn from '../../components/table/warehouse/tableLGEtcIn'
import TableLGEtcInItem from '../../components/table/warehouse/tableLGEtcInItem'
import { Splitter, SplitterPanel } from 'primereact/splitter'

import { GetReqSheetQuery } from '../../../features/warehouse/lgEtcIn/getReqSheetQuery'
import { GetSheetQuery } from '../../../features/warehouse/lgEtcIn/getSheetQuery'
import { PostQRCheck } from '../../../features/warehouse/lgEtcIn/postQRCheck'

import { PostAUD } from '../../../features/warehouse/lgEtcIn/postAUD'
import { PostMasterDelete } from '../../../features/warehouse/lgEtcIn/postMasterDelete'
import { PostSheetDelete } from '../../../features/warehouse/lgEtcIn/postSheetDelete'
...
export default function LGEtcInLink({
  permissions,
  isMobile,
  canCreate,
  canEdit,
  ...
  const nameFrom = ' NHẬP KHO SẢN PHẨM KHÁC '
  ...
  useEffect(() => {
    workerRef.current = new Worker(
      new URL('../../../workers/workerEtcIn.js', import.meta.url),
    )
  ...
  const fetchSheetQuery = async (reqSeq, inOutSeq) => {
    try {
      ...
      const etcInResponse = await GetReqSheetQuery(reqSeq)
      const inOutResponse = await GetSheetQuery(inOutSeq)

      if (etcInResponse.data.success && inOutResponse?.success) {
        const fetchedData = etcInResponse.data.data || []
        ...
        if (fetchedDataItem.length > 0) {
          ...
          setWhSeq(fetchedDataItem[0].InWHSeq)
          setWhName(fetchedDataItem[0].InWHName)
  ...
  const handleResetData = useCallback(async () => {
    ...
    resetTableItem()
    fetchSheetQuery(reqSeq, inOutSeq)
