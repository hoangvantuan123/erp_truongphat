...
import { GetMasterQuery } from '../../../features/warehouse/lgEtcInReq/getMasterQuery'
...
export default function LGEtcInReqLink({
  permissions,
  isMobile,
  canCreate,
  canEdit,
  ...
  const fetchMasterQuery = async (reqSeq) => {
    ...
    try {
      const etcInReqResponse = await GetMasterQuery(reqSeq)
  ...
  const fetchSheetQuery = async (reqSeq) => {
    try {
      ...
      const etcInReqResponse = await GetSheetQuery(reqSeq)
          ...
          <div className="col-start-1 col-end-5 row-start-1 w-full rounded-lg ">
            <div className="flex items-center justify-between mb-2">
              <Title level={4} className="mt-2 uppercase opacity-85 ">
                {t('Đăng ký yêu cầu nhập kho khác')}
              </Title>
              <LGEtcInReqActions
