...
import LGStockClosingMonthQuery from '../../components/query/warehouse/lgStockClosingMonthQuery'
import LGStockClosingMonthActions from '../../components/actions/warehouse/lgStockClosingMonthActions'
import TableLGStockMonthClosing from '../../components/table/warehouse/tableLGStockMonthClosing'

import { GetSCOMClosingMonthDynamicQuery } from '../../../features/warehouse/lgStockClosing/postSCOMClosingMonthDynamicQuery'
import { PostAUDCloseMonth } from '../../../features/warehouse/lgStockClosing/postAUDCloseMonth'
...
export default function LGStockClosingMonth({
  permissions,
  isMobile,
  canEdit,
  canCreate,
  ...
  const [inOutYYMM, setInOutYYMM] = useState(dayjs().startOf('year'))
  ...
  const [cols, setCols] = useState(() =>
    loadFromLocalStorageSheet(
      'S_ERP_COLS_PAGE_STOCK_CLOSING_MONTH_LIST',
  ...
  const fetchDataQuery = async () => {
    ...
    try {
      const data = [
        {
          ClosingYear: inOutYYMM ? formatDate(inOutYYMM) : '',
          BizUnit: bizUnit,
        },
      ]

      const response = await GetSCOMClosingMonthDynamicQuery(data)
  ...
  const handleSaveData = useCallback(async () => {
    ...
    if (canCreate === false) {
      message.warning('Bạn không có quyền lưu dữ liệu')
      return
    }
    const columnsA = [
      'Status',
    ...
    try {
      const promises = []
      promises.push(PostAUDCloseMonth(dataSheetAUD))
  ...
  return (
    <>
      <Helmet>
        <title>ITM - {t('Xử lý khóa sổ kho theo tháng')}</title>
      </Helmet>
          ...
          <div className="col-start-1 col-end-5 row-start-1 w-full rounded-lg ">
            <div className="flex items-center justify-between mb-2">
              <Title level={4} className="mt-2 uppercase opacity-85 ">
                {t('Xử lý khóa sổ kho theo tháng')}
              </Title>
              <LGStockClosingMonthActions
