...
import { PostInventoryCheck } from '../../../features/warehouse/lgEtcTransTP/postInventoryCheck'
...
export default function LGEtcTransTPLink({
    ...
    try {
      ...
      if (etcInReqResponse.data.success) {
        ...
        if (data) {
          ...
          setBizUnitTrans(data[0].ReqBizUnit)
          ...
          setWHSeqIn(data[0].InWHSeq)
          setWHNameIn(data[0].InWHName)
          setWHSeqOut(data[0].OutWHSeq)
          setWHNameOut(data[0].OutWHName)
    ...
    const dataSheetAUD = gridData
      .filter(
        (row) =>
          ...
          (row.InOutDetailKind !== '' && row.InOutDetailKind !== null),
      )
      .map((row) => ({
        ...row,
        InWHSeq: whSeqIn,
  ...
  return (
    <>
      <Helmet>
        <title>ITM - {t('Đăng ký di chuyển kho sản phẩm')}</title>
      </Helmet>
