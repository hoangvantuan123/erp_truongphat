...
import LGEtcOutTPActions from '../../components/actions/warehouse/lgEtcOutTPActions'
import LGEtcOutTPQuery from '../../components/query/warehouse/lgEtcOutTPQuery'
import TableLGEtcOutTP from '../../components/table/warehouse/tableLGEtcOutTP'

import { GetMasterQuery } from '../../../features/warehouse/lgEtcOutTP/getMasterQuery'
import { GetSheetQuery } from '../../../features/warehouse/lgEtcOutTP/getSheetQuery'
import { PostInventoryCheck } from '../../../features/warehouse/lgEtcOutTP/postInventoryCheck'
import { PostAUD } from '../../../features/warehouse/lgEtcOutTP/postAUD'
import { PostMasterDelete } from '../../../features/warehouse/lgEtcOutTP/postMasterDelete'
import { PostSheetDelete } from '../../../features/warehouse/lgEtcOutTP/postSheetDelete'
...
export default function LGEtcOutTPLink({
  permissions,
  isMobile,
  canCreate,
  canEdit,
  ...
  useEffect(() => {
    if (id) {
      ...
      if (data) {
        fetchMasterQuery(data.InOutSeq)
        fetchSheetQuery(data.InOutSeq)
      }
    }
  }, [id])

  const fetchMasterQuery = async (inOutSeq) => {
    if (loadingBarRef.current) {
      loadingBarRef.current.continuousStart()
    }
    try {
      const etcInReqResponse = await GetMasterQuery(inOutSeq)

      if (etcInReqResponse.data.success) {
        const data = etcInReqResponse.data.data || []
        if (data) {
          setFromDate(dayjs(data[0].InOutDate))
          setBizUnit(data[0].BizUnit)
          setTypeName(data[0].InOutType)
          setEmpName(data[0].EmpName)
          setEmpSeq(data[0].EmpSeq)
          setDeptName(data[0].DeptName)
          setDeptSeq(data[0].DeptSeq)
          setInOutNo(data[0].InOutNo)
          setInOutSeq(data[0].InOutSeq)
  ...
  const fetchSheetQuery = async (inOutSeq) => {
    try {
      if (loadingBarRef.current) {
        loadingBarRef.current.continuousStart()
      }
      const etcInReqResponse = await GetSheetQuery(inOutSeq)

      if (etcInReqResponse.data.success) {
        const fetchedData = etcInReqResponse.data.data || []
        const emptyData = generateEmptyData(50, defaultCols)
  ...
  const handleResetData = useCallback(async () => {
    ...
    setInOutNo(initialValues.EtcInOutNo)
    ...
    setInOutSeq(initialValues.InOutSeq)
