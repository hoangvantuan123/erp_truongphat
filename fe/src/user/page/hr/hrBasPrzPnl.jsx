...
import HrBasPrzPnlAction from '../../components/actions/hr/hrBasPrzPnlAction'
import HrBasPrzPnlQuery from '../../components/query/hr/hrBasPrzPnlQuery'
import HrBasPrzPnlTable from '../../components/table/hr/hrBasPrzPnlTable'
import ErrorListModal from '../default/errorListModal'
import { HandleError } from '../default/handleError'
export default function HrBasPrzPnl({ permissions, isMobile, canCreate, canEdit, canDelete, controllers,
    cancelAllRequests }) {
    ...
    const [UMPrzPnlSeq, setUMPrzPnlSeq] = useState(null)
    ...
    }, [dataSearch, dataSearch1, UMPrzPnlSeq])
    ...
    const handleDeleteDataSheet = useCallback(
        (e) => {
            ...
            if (rowsWithStatusD.length > 0) {
                ...
                PostDHrBasPrzPnl(rowsWithStatusD)
                    .then((response) => {
                        if (response.success) {
    ...
    const handleSaveData = useCallback(async () => {
        ...
        try {
            const apiCalls = [];
            if (resulA.length > 0) apiCalls.push(PostAHrBasPrzPnl(resulA));
            if (resulU.length > 0) apiCalls.push(PostUHrBasPrzPnl(resulU));
            ...
            setGridData(prev => {
                const updated = prev.map(item => {
                    ...
                    return found ? { ...item, Status: '', PrzPnlSeq: found.PrzPnlSeq } : item;
                });
    ...
    return (
        <>
            <Helmet>
                <title>{t('Đăng ký hạng mục thưởng phạt')}</title>
            </Helmet>
