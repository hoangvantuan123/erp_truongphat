...
import HrBasTravelAction from '../../components/actions/hr/hrBasTravelAction'
import HrBasTravelTable from '../../components/table/hr/hrBasTravelTable'
import HrBasTravelQuery from '../../components/query/hr/hrBasTravelQuery'
import dayjs from 'dayjs'
import ErrorListModal from '../default/errorListModal'
import { HandleError } from '../default/handleError'
export default function HrBasTravelRec({ permissions, isMobile, canCreate, canEdit, canDelete, controllers,
    cancelAllRequests }) {
    ...
    const defaultCols = useMemo(() => [
        ...
        {
            title: 'Phân loại làm việc/nghỉ việc',
            id: 'EntRetTypeName',
            kind: 'Text',
            readonly: false,
            width: 200,
    ...
    }, [dataSearch, dataSearch1, formData, toDate])
    ...
    const handleDeleteDataSheet = useCallback(
        (e) => {
            ...
            if (rowsWithStatusD.length > 0) {
                ...
                PostDHrBaseTravel(rowsWithStatusD)
                    .then((response) => {
                        if (response.success) {
    ...
    const handleSaveData = useCallback(async () => {
        ...
        try {
            const apiCalls = [];
            if (resulA.length > 0) apiCalls.push(PostAHrBaseTravel(resulA));
            if (resulU.length > 0) apiCalls.push(PostUHrBaseTravel(resulU));
            ...
            setGridData(prev => {
                const updated = prev.map(item => {
                    ...
                    return found ? { ...item, Status: '', TravelRecSeq: found.TravelRecSeq } : item;
                });
    ...
    return (
        <>
            <Helmet>
                <title>{t('Đăng ký thông tin đi công tác')}</title>
            </Helmet>
