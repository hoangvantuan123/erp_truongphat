...
import HrBasAcademyTable from '../../components/table/hr/hrBasAcademyTable'
import HrBasAcademyQuery from '../../components/query/hr/HrBasAcademyQuery'
...
export default function HrBasAcademy({ permissions, isMobile, canCreate, canEdit, canDelete, controllers,
    cancelAllRequests }) {
    ...
    const handleDeleteDataSheet = useCallback(
        (e) => {
            ...
            if (rowsWithStatusD.length > 0) {
                ...
                PostDHrBasAcademi(rowsWithStatusD)
                    .then((response) => {
                        if (response.success) {
                            const deletedIds = rowsWithStatusD.map((item) => item.AcademicSeq);
                            const updatedData = gridData.filter((row) => !deletedIds.includes(row.AcademicSeq));
    ...
    const handleSaveData = useCallback(async () => {
        ...
        try {
            const apiCalls = [];
            if (resulA.length > 0) apiCalls.push(PostAHrBasAcademi(resulA));
            if (resulU.length > 0) apiCalls.push(PostUHrBasAcademi(resulU));
            ...
            setGridData(prev => {
                const updated = prev.map(item => {
                    ...
                    return found ? { ...item, Status: '', AcademicSeq: found.AcademicSeq } : item;
                });
