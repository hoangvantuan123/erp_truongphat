...
export default function IqcOutsourceReqDetails({
  permissions,
  isMobile,
  canCreate,
  canEdit,
  ...
  const [dataTestReportResult, setDataTestReportResutl] = useState([])
  ...
  const handleDeleteData = useCallback(async () => {
    ...
    try {
      const response = await DeleteIqcOutsourceBy(result)
      setLoading(true)
      if (response.success) {
        const qcTestReportResult = response.data.data || []
        // handleSaveListItemData(qcTestReportResult[0].QCSeq)
  ...
  const resetAll = () => {
    ...
    const handleDeleteFiles = async (fileList, formCode, QCSeq) => {
      ...
      formData.append('tableName', 'PDQCTestReportOSP')
  ...
  const handleUploadFiles = async (fileList, formCode, QCSeq) => {
      ...
      const fetchDataFileSeq = useCallback(async (ItemNoSeq, FormCode) => {
          const controller = new AbortController()
          const signal = controller.signal
          const TableName = 'PDQCTestReportOSP'
