...
import TableIqcOutsourceDetailsListReq from '../../components/table/iqcOutsource/tableIqcOutsourceDetailsListReq'
import { CreatedOutsourceListBy } from '../../../features/iqc-outsource/createdQcOutsourceListBy'
import TopLoadingBar from 'react-top-loading-bar';
import { GetListQcOutsourceTestReportBatch } from '../../../features/iqc-outsource/GetListQcOutsourceTestReportBatch'
...
export default function IqcOutsourceReqDetailsList({
  permissions,
  isMobile,
  canCreate,
  canEdit,
  ...
  const [cols, setCols] = useState(() =>
    loadFromLocalStorageSheet(
      'S_ERP_COLS_PAGE_IQC_OUTSOURCE_LIST_DETAILS',
  ...
  const handleSaveData = useCallback(async () => {
    ...
    if (resulA.length > 0 || resulU.length > 0) {
      ...
      try {
        ...
        if (resulA.length > 0) {
          promises.push(CreatedOutsourceListBy(resulA))
        }

        if (resulU.length > 0) {
          promises.push(CreatedOutsourceListBy(resulU))
        }
    ...
    const fetchGetListQcTestReportBatch = useCallback(async () => {
      ...
      try {
        const selectData = dataSelect.map(item => {
          const cleanedQCDate = item.QCDate.trim() ? item.QCDate : formatDate(dayjs());
          return {
            ...item,
            QCDate: cleanedQCDate
          }
        })
        const response = await GetListQcOutsourceTestReportBatch(selectData) 
        if (response.success) {
      ...
      <div className="bg-slate-50 p-3 h-screen overflow-hidden">
        <div className="flex flex-col gap-4 md:grid md:grid-cols-4 md:grid-rows-[auto_1fr] md:gap-4 h-full">
          ...
          <div className="col-start-1 col-end-5 row-start-2 w-full h-full rounded-lg  overflow-auto">
            <TableIqcOutsourceDetailsListReq
