...
import IqcOutsourceReqListActions from '../../components/actions/iqc-outsource/iqcOutsourceReqListActions'
import IqcOutsourceReqListQuery from '../../components/query/iqcOutsource/iqcOutsourceReqListQuery'
import TableIqcOutsourceReqList from '../../components/table/iqcOutsource/tableIqcOutsourceReqList'
import { SearchIqcOutsourcePage } from '../../../features/iqc-outsource/searchIqcOutsourcePage'
import TopLoadingBar from 'react-top-loading-bar';
import { GetCodeHelpVer2 } from '../../../features/codeHelp/getCodeHelpVer2'

export default function IqcOutsourceReqList({
  permissions,
  isMobile,
  canCreate,
  canEdit,
  ...
  const [cols, setCols] = useState(() =>
    loadFromLocalStorageSheet(
      'S_ERP_COLS_PAGE_IQC_OUTSOURCE_PAGE',
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const response = await SearchIqcOutsourcePage(data)
  ...
  const fetchCodeHelpDataByFactUnit = useCallback(async () => {
    try {
      const [
        dataWH,
      ] = await Promise.all([
        GetCodeHelpVer2(10006, '', FactUnit, '', '', '', '1', '', 1, '', 0, 0, 0),
      ])
      setDataWHName(dataWH.data)
    } catch (error) {
    } finally {
  ...
  const nextPageDeatails = useCallback(() => {
    if (keyPath) {
      ...
      navigate(`/qc/u/iqc-outsource/${keyPath}`)
    } else {
  ...
  const nextPageDeatailsList = useCallback(() => {
    ...
    if (dataSelect.length > 0) {
      navigate(`/qc/u/iqc-outsource-list`, { state: { dataSelect } })
    } else {
      if (!keyPath) {
        message.warning('Lựa chọn đề nghị xử lý kiểm tra')
      } else {
  ...
  const onCellClicked = useCallback((cell, event) => {
    ...
    if (rowIndex >= 0 && rowIndex < gridData.length) {
      ...
      const filteredData = {
        ...
        SourceTypeName: rowData.SourceTypeName,
