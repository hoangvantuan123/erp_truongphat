...
export default function StockOutRequestFiFo({ permissions, isMobile, controllers,
  cancelAllRequests }) {
  ...
  const fetchDataA = async (outReqSeq) => {
    if (controllers.current.fetchDataA) {
      controllers.current.fetchDataA.abort();
      controllers.current.fetchDataA = null;
    ...
    controllers.current.fetchDataA = controller;
    const searchData = {
      OutReqSeq: outReqSeq,
    }
    try {
      setLoading(true)
      const response = await PostQScanPdmmOutProc(searchData)
