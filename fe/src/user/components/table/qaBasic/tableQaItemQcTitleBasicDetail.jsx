...
function TableQaItemQcTitleBasicDetail({
  dataQcUmTitleName,
  setSelection,
  selection,
  setShowSearch,
  ...
  const [hiddenColumns, setHiddenColumns] = useState(() => {
    return loadFromLocalStorageSheet('H_ERP_COLS_PAGE_QA_ITEM_QC_TYPE', [])
  })
  ...
  const columnNames = ['UMQcTitleName',]
  ...
  const [modalVisibleItemName, setModalVisibleItemName] =
  useState(false)
  ...
  const getData = useCallback(
    ([col, row]) => {
      ...
      if (
        columnKey === 'IsStrictLevel' ||
        columnKey === 'IsMidLevel' ||
        columnKey === 'IsSimpleLevel'
      ) {
        const booleanValue =
          value === 1 || value === '1'
            ? true
        ...
        if (newValue.kind === GridCellKind.Boolean) {
          setGridData((prev) => {
            ...
            if (selectedName) {
              ...
              product['IsStrictLevel'] = selectedName
            } else {
              product[cols[col].id] = false
              product['IsStrictLevel'] = false
            }
            product.isEdited = true
            product['IdxNo'] = row + 1
            const currentStatus = product['Status'] || 'U'
      ...
      if (key === 'IsMidLevel') {
        if (newValue.kind === GridCellKind.Boolean) {
          setGridData((prev) => {
            ...
            if (selectedName) {
              product[cols[col].id] = selectedName
              product['IsMidLevel'] = selectedName
            } else {
              product[cols[col].id] = false
              product['IsMidLevel'] = false
            }
            product.isEdited = true
            product['IdxNo'] = row + 1
            const currentStatus = product['Status'] || 'U'
      ...
      if (key === 'IsSimpleLevel') {
        if (newValue.kind === GridCellKind.Boolean) {
          setGridData((prev) => {
            ...
            if (selectedName) {
              product[cols[col].id] = selectedName
              product['IsSimpleLevel'] = selectedName
            } else {
              product[cols[col].id] = false
              product['IsSimpleLevel'] = false
            }
            product.isEdited = true
            product['IdxNo'] = row + 1
            const currentStatus = product['Status'] || 'U'
  ...
  const updateHiddenColumns = (newHiddenColumns) => {
    setHiddenColumns((prevHidden) => {
      ...
      saveToLocalStorageSheet('H_ERP_COLS_PAGE_QA_ITEM_QC_TYPE', newHidden)
      return newHidden
    })
  }
  ...
  const updateVisibleColumns = (newVisibleColumns) => {
    setCols((prevCols) => {
      ...
      saveToLocalStorageSheet('S_ERP_COLS_PAGE_QA_ITEM_QC_TYPE', uniqueCols)
      return uniqueCols
    })
  }
  // Hàm ẩn cột
  ...
  const handleReset = () => {
    ...
    setHiddenColumns([])
    localStorage.removeItem('S_ERP_COLS_PAGE_QA_ITEM_QC_TYPE')
    localStorage.removeItem('H_ERP_COLS_PAGE_QA_ITEM_QC_TYPE')
    setShowMenu(null)
  }
  ...
  const onColumnMoved = useCallback((startIndex, endIndex) => {
    setCols((prevCols) => {
      ...
      saveToLocalStorageSheet('S_ERP_COLS_PAGE_QA_ITEM_QC_TYPE', updatedCols)
      return updatedCols
    })
  }, [])
  ...
  const handleCheckboxChange = (columnId, isChecked) => {
    if (isChecked) {
      ...
      setCols((prevCols) => {
        ...
        saveToLocalStorageSheet('S_ERP_COLS_PAGE_QA_ITEM_QC_TYPE', newCols)
        return newCols
      })
      setHiddenColumns((prevHidden) => {
...
export default  TableQaItemQcTitleBasicDetail
