...
function TableQaQcTitle({
  ...
  const [hiddenColumns, setHiddenColumns] = useState(() => {
    return loadFromLocalStorageSheet('H_ERP_COLS_PAGE_QA_QC_TITLE', [])
  })
  ...
  const getData = useCallback(
    ([col, row]) => {
      ...
      const cellConfig = {
        ...
        InPutTypeName: {
          kind: 'input-type-name-cell',
          allowedValues: dataInputTypeName,
        },
        SMAQLLevelName: {
          kind: 'sm-aql-level-name-cell',
          allowedValues: dataSMAQLLevelName,
        },
      }
      ...
      if (columnKey === 'InPutTypeName') {
        const value = person['InPutType'] || ''
        const InPutType = dataInputTypeName.find(
        ...
        return {
          ...
          allowOverlay: true,
          copyData: String(InPutType.MinorName),
          data: {
            kind: cellConfig[columnKey].kind,
            allowedValues: cellConfig[columnKey].allowedValues,
            value: InPutType.MinorName,
            boundingBox: boundingBox,
          },
          displayData: String(InPutType.MinorName),
          readonly: column?.readonly || false,
          hasMenu: column?.hasMenu || false,
        }
      }

      if (columnKey === 'SMAQLLevelName') {
        const value = person['SMAQLLevel'] || ''
        const InPutType = dataSMAQLLevelName.find(
          (item) => item.Value === value,
        ) || { MinorName: '', Value: 0 }
      ...
      if (key === 'InPutTypeName') {
        if (newValue.kind === GridCellKind.Custom) {
          setGridData((prev) => {
            ...
            if (selectedName) {
              product[cols[col].id] = selectedName.MinorName
              product['InPutTypeName'] = selectedName.MinorName
              product['InPutType'] = selectedName.Value
            } else {
              product[cols[col].id] = ''
              product['InPutTypeName'] = ''
              product['InPutType'] = ''
            }
  ...
  const updateHiddenColumns = (newHiddenColumns) => {
    setHiddenColumns((prevHidden) => {
      ...
      saveToLocalStorageSheet('H_ERP_COLS_PAGE_QA_QC_TITLE', newHidden)
      return newHidden
    })
  }
  ...
  const updateVisibleColumns = (newVisibleColumns) => {
    setCols((prevCols) => {
      ...
      saveToLocalStorageSheet('S_ERP_COLS_PAGE_QA_QC_TITLE', uniqueCols)
      return uniqueCols
    })
  }
  // Hàm ẩn cột
  ...
  const handleReset = () => {
    ...
    setHiddenColumns([])
    localStorage.removeItem('S_ERP_COLS_PAGE_QA_QC_TITLE')
    localStorage.removeItem('H_ERP_COLS_PAGE_QA_QC_TITLE')
    setShowMenu(null)
  }
  ...
  const onColumnMoved = useCallback((startIndex, endIndex) => {
    setCols((prevCols) => {
      ...
      saveToLocalStorageSheet('S_ERP_COLS_PAGE_QA_QC_TITLE', updatedCols)
      return updatedCols
    })
  }, [])
  ...
  const handleCheckboxChange = (columnId, isChecked) => {
    if (isChecked) {
      ...
      setCols((prevCols) => {
        ...
        saveToLocalStorageSheet('S_ERP_COLS_PAGE_QA_QC_TITLE', newCols)
        return newCols
      })
      setHiddenColumns((prevHidden) => {
