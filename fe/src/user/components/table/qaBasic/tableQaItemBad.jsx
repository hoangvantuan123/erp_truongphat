...
import { CellsBadKindName } from '../../sheet/cells/cellsBadKindName'

function TableQaItemBad({
  dataBadKindName,
  dataBadReasonName,
  setSelection,
  selection,
  ...
  const [hiddenColumns, setHiddenColumns] = useState(() => {
    return loadFromLocalStorageSheet('H_ERP_COLS_PAGE_QA_ITEM_BAD', [])
  })
  ...
  const columnNames = ['BadKind', 'BadKindName', 'BadReasonName']
  ...
  const getData = useCallback(
    ([col, row]) => {
      ...
      const cellConfig = {
        BadKindName: {
          kind: 'bad-kind-name-cell',
          allowedValues: dataBadKindName,
        },
        BadReasonName: {
          kind: 'bad-reason-cell',
          allowedValues: dataBadReasonName,
        },
      }

      if (columnKey === 'BadKindName') {
        const value = person['BadKind'] || ''
        const BadKindName = dataBadKindName.find((item) => item.Value === value) || { MinorName: '', Value: 0 };
        
        return {
          kind: GridCellKind.Custom,
          allowOverlay: true,
          copyData: String(BadKindName.MinorName),
          data: {
            kind: cellConfig[columnKey].kind,
            allowedValues: cellConfig[columnKey].allowedValues,
            value: BadKindName.MinorName,
            boundingBox: boundingBox,
          },
          displayData: String(BadKindName.MinorName),
          readonly: column?.readonly || false,
          hasMenu: column?.hasMenu || false,
        }
      }

      if (columnKey === 'BadReasonName') {
        const value = person['BadReason'] || ''
        const BadKindName = dataBadReasonName.find((item) => item.Value === value) || { MinorName: '', Value: 0 };
      ...
      if (key === 'BadKindName') {
        if (newValue.kind === GridCellKind.Custom) {
          setGridData((prev) => {
            ...
            if (!selectedName) {
              selectedName = dataBadKindName.find(
                (item) => item.MinorName === checkCopyData,
              )
            }
            if (selectedName) {
              product[cols[col].id] = selectedName.MinorName
              product['BadKindName'] = selectedName.MinorName
              product['BadKind'] = selectedName.Value
            } else {
              product[cols[col].id] = ''
              product['BadKindName'] = ''
              product['BadKind'] = ''
      ...
      if (key === 'BadReasonName') {
        if (newValue.kind === GridCellKind.Custom) {
          setGridData((prev) => {
            ...
            if (selectedName) {
              product[cols[col].id] = selectedName.MinorName
              product['BadReasonName'] = selectedName.MinorName
              product['BadReason'] = selectedName.Value
            } else {
              product[cols[col].id] = ''
  ...
  const updateHiddenColumns = (newHiddenColumns) => {
    setHiddenColumns((prevHidden) => {
      ...
      saveToLocalStorageSheet('H_ERP_COLS_PAGE_QA_ITEM_BAD', newHidden)
      return newHidden
    })
  }
  ...
  const updateVisibleColumns = (newVisibleColumns) => {
    setCols((prevCols) => {
      ...
      saveToLocalStorageSheet('S_ERP_COLS_PAGE_QA_ITEM_BAD', uniqueCols)
      return uniqueCols
    })
  }
  // Hàm ẩn cột
  ...
  const handleReset = () => {
    ...
    setHiddenColumns([])
    localStorage.removeItem('S_ERP_COLS_PAGE_QA_ITEM_BAD')
    localStorage.removeItem('H_ERP_COLS_PAGE_QA_ITEM_BAD')
    setShowMenu(null)
  }
  ...
  const onColumnMoved = useCallback((startIndex, endIndex) => {
    setCols((prevCols) => {
      ...
      saveToLocalStorageSheet('S_ERP_COLS_PAGE_QA_ITEM_BAD', updatedCols)
      return updatedCols
    })
  }, [])
  ...
  const handleCheckboxChange = (columnId, isChecked) => {
    if (isChecked) {
      ...
      setCols((prevCols) => {
        ...
        saveToLocalStorageSheet('S_ERP_COLS_PAGE_QA_ITEM_BAD', newCols)
        return newCols
      })
      setHiddenColumns((prevHidden) => {
    ...
    <div className="w-full gap-1 h-full flex items-center justify-center pb-8">
      <div className="w-full h-full flex flex-col border bg-white rounded-lg overflow-hidden ">
        ...
        <DataEditor
          ...
          customRenderers={[CellsBadReason, CellsBadKindName]}
