...
import { CellsTestMethodName } from '../../sheet/cells/cellsTestMethodName'
import { CellsAQLStrictName } from '../../sheet/cells/cellsAQLStrictName'
import { CellsSamplingStdName } from '../../sheet/cells/cellsSamplingStdName'
import { CellsItemName } from '../../sheet/cells/cellsItemName'
import { CellsSpec } from '../../sheet/cells/cellsSpec'
import { CellsAQLPointName } from '../../sheet/cells/cellsAqlPointName'

function TableQaCustQcTitleRight({
  dataItemName,
  dataSpec,
  dataTestMethod,
  dataSamplingStd,
  ...
  const [hiddenColumns, setHiddenColumns] = useState(() => {
    return loadFromLocalStorageSheet('H_ERP_COLS_PAGE_QA_CUST_QC_TITLE_RIGHT', [])
  })
  ...
  const getData = useCallback(
    ([col, row]) => {
      ...
      const cellConfig = {
        ...
        Spec: {
          kind: 'specs-cell',
          allowedValues: dataSpec,
        },

        SMTestMethodName: {
          kind: 'test-method-name-cell',
          allowedValues: dataTestMethod,
        },

        SMSamplingStdName: {
          kind: 'sampling-std-cell',
          allowedValues: dataSamplingStd,
        },
        
        SMAQLLevelName: {
          kind: 'sm-aql-level-name-cell',
          allowedValues: dataAQLLevelName,
        },

        SMAQLStrictName: {
          kind: 'aql-strict-cell',
          allowedValues: dataAQLStrictName,
        },

        SMAQLPointName: {
          kind: 'aql-point-name-cell',
          allowedValues: dataAQLPointName,
        },
      ...
      if (
        ...
        return {
          ...
          data: value,
          displayData: formatDate(value.trim()) || '',
          readonly: column?.readonly || false,
          allowOverlay: true,
          hasMenu: false,
        }
        ...
        if (newValue.kind === GridCellKind.Custom) {
          setGridData((prev) => {
            ...
            if (!selectedName && selectedName !== undefined) {
              selectedName = dataItemName.find(
      ...
      if (key === 'Spec') {
        if (newValue.kind === GridCellKind.Custom) {
          setGridData((prev) => {
            ...
            const checkCopyData = newValue.copyData
            if (!selectedName) {
              selectedName = dataSpec.find(
                (item) => item.Spec === checkCopyData,
              )
            }

            if (selectedName || selectedName !== undefined) {
              product[cols[col].id] = selectedName.Spec
              product['Spec'] = selectedName.Spec
            } else {
              product[cols[col].id] = ''
              product['Spec'] = ''
      ...
      if (key === 'SMTestMethodName') {
        if (newValue.kind === GridCellKind.Custom) {
          setGridData((prev) => {
            ...
            const checkCopyData = newValue.copyData
            if (!selectedName) {
              selectedName = dataTestMethod.find(
            ...
            if (selectedName || selectedName !== undefined) {
              ...
              product['SMTestMethodName'] = selectedName.MinorName
              product['SMTestMethod'] = selectedName.Value
            } else {
              product[cols[col].id] = ''
              product['SMTestMethodName'] = ''
              product['SMTestMethod'] = ''
            }
      ...
      if (key === 'SMSamplingStdName') {
        if (newValue.kind === GridCellKind.Custom) {
          setGridData((prev) => {
            ...
            const checkCopyData = newValue.copyData
            if (!selectedName) {
              selectedName = dataAQLLevelName.find(
                (item) => item.MinorName === checkCopyData,
              )
            }
            ...
            if (selectedName || selectedName !== undefined) {
              product[cols[col].id] = selectedName.MinorName
              product['SMSamplingStdName'] = selectedName.MinorName
              product['SMSamplingStd'] = selectedName.Value
            } else {
              product[cols[col].id] = ''
              product['SMSamplingStdName'] = ''
              product['SMSamplingStd'] = ''
            }
      ...
      if (key === 'SMAQLLevelName') {
        if (newValue.kind === GridCellKind.Custom) {
          setGridData((prev) => {
            ...
            const checkCopyData = newValue.copyData
            if (!selectedName) {
              selectedName = dataAQLStrictName.find(
                (item) => item.MinorName === checkCopyData,
              )
            }
            ...
            if (selectedName || selectedName !== undefined) {
              product[cols[col].id] = selectedName.MinorName
              product['SMAQLLevelName'] = selectedName.MinorName
              product['SMAQLLevel'] = selectedName.Value
            } else {
              product[cols[col].id] = ''
              product['SMAQLLevelName'] = ''
              product['SMAQLLevel'] = ''
            }
      ...
      if (key === 'SMAQLStrictName') {
        if (newValue.kind === GridCellKind.Custom) {
          setGridData((prev) => {
            ...
            if (selectedName || selectedName !== undefined) {
              product[cols[col].id] = selectedName.MinorName
              product['SMAQLStrictName'] = selectedName.MinorName
              product['SMAQLStrict'] = selectedName.Value
            } else {
              product[cols[col].id] = ''
              product['SMAQLStrictName'] = ''
              product['SMAQLStrict'] = ''
            }
      ...
      if (key === 'SMAQLPointName') {
        if (newValue.kind === GridCellKind.Custom) {
          setGridData((prev) => {
            ...
            const checkCopyData = newValue.copyData
            if (!selectedName) {
              selectedName = dataAQLPointName.find(
                (item) => item.MinorName === checkCopyData,
              )
            }
            ...
            if (selectedName || selectedName !== undefined) {
              product[cols[col].id] = selectedName.MinorName
              product['SMAQLPointName'] = selectedName.MinorName
              product['SMAQLPoint'] = selectedName.Value
            } else {
              product[cols[col].id] = ''
              product['SMAQLPointName'] = ''
              product['SMAQLPoint'] = ''
            }
  ...
  const updateHiddenColumns = (newHiddenColumns) => {
    setHiddenColumns((prevHidden) => {
      ...
      saveToLocalStorageSheet('H_ERP_COLS_PAGE_QA_CUST_QC_TITLE_RIGHT', newHidden)
      return newHidden
    })
  }
  ...
  const updateVisibleColumns = (newVisibleColumns) => {
    setCols((prevCols) => {
      ...
      saveToLocalStorageSheet('S_ERP_COLS_PAGE_QA_CUST_QC_TITLE_RIGHT', uniqueCols)
      return uniqueCols
    })
  }
  // Hàm ẩn cột
  ...
  const handleReset = () => {
    ...
    setHiddenColumns([])
    localStorage.removeItem('S_ERP_COLS_PAGE_QA_CUST_QC_TITLE_RIGHT')
    localStorage.removeItem('H_ERP_COLS_PAGE_QA_CUST_QC_TITLE_RIGHT')
    setShowMenu(null)
  }
  ...
  const onColumnMoved = useCallback((startIndex, endIndex) => {
    setCols((prevCols) => {
      ...
      saveToLocalStorageSheet('S_ERP_COLS_PAGE_QA_CUST_QC_TITLE_RIGHT', updatedCols)
      return updatedCols
    })
  }, [])
  ...
  const handleCheckboxChange = (columnId, isChecked) => {
    if (isChecked) {
      ...
      setCols((prevCols) => {
        ...
        saveToLocalStorageSheet('S_ERP_COLS_PAGE_QA_CUST_QC_TITLE_RIGHT', newCols)
        return newCols
      })
      setHiddenColumns((prevHidden) => {
...
export default TableQaCustQcTitleRight
