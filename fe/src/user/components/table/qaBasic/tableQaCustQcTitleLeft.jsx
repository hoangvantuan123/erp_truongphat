...
import { CellsSMQcTypeName } from '../../sheet/cells/cellsSMQcTypeName'

function TableQaCustQcTitleLeft({
  dataCustomer,
  dataSmQcTypeName,
  ...
  const [hiddenColumns, setHiddenColumns] = useState(() => {
    return loadFromLocalStorageSheet('H_ERP_COLS_PAGE_QA_CUST_QC_LEFT', [])
  })
  ...
  const columnNames = ['CustName', 'SMQCTypeName']
  ...
  const getData = useCallback(
    ([col, row]) => {
      ...
      const cellConfig = {
        ...
        SMQCTypeName: {
          kind: 'qc-type-name-cell',
          allowedValues: dataSmQcTypeName,
        },
      }
  ...
  const onCellEdited = useCallback(
      ...
      if (key === 'SMQCTypeName') {
        if (newValue.kind === GridCellKind.Custom) {
          setGridData((prev) => {
            ...
            const checkCopyData = newValue.copyData
            if (!selectedName) {
              selectedName = dataSmQcTypeName.find(
            ...
            if (selectedName || selectedName !== undefined) {
              ...
              product['SMQCTypeName'] = selectedName.MinorName
              product['SMQCType'] = selectedName.Value
              setSMQCTypeName(selectedName.MinorName)
            } else {
              product[cols[col].id] = ''
              product['SMQCTypeName'] = ''
              product['SMQCType'] = ''
            }
    ...
    [canEdit, cols, gridData, CustName, SMQCTypeName],
  )
  ...
  const updateHiddenColumns = (newHiddenColumns) => {
    setHiddenColumns((prevHidden) => {
      ...
      saveToLocalStorageSheet('H_ERP_COLS_PAGE_QA_CUST_QC_LEFT', newHidden)
      return newHidden
    })
  }
  ...
  const updateVisibleColumns = (newVisibleColumns) => {
    setCols((prevCols) => {
      ...
      saveToLocalStorageSheet('S_ERP_COLS_PAGE_QA_CUST_QC_LEFT', uniqueCols)
      return uniqueCols
    })
  }
  // Hàm ẩn cột
  ...
  const handleReset = () => {
    ...
    setHiddenColumns([])
    localStorage.removeItem('S_ERP_COLS_PAGE_QA_CUST_QC_LEFT')
    localStorage.removeItem('H_ERP_COLS_PAGE_QA_CUST_QC_LEFT')
    setShowMenu(null)
  }
  ...
  const onColumnMoved = useCallback((startIndex, endIndex) => {
    setCols((prevCols) => {
      ...
      saveToLocalStorageSheet('S_ERP_COLS_PAGE_QA_CUST_QC_LEFT', updatedCols)
      return updatedCols
    })
  }, [])
  ...
  const handleCheckboxChange = (columnId, isChecked) => {
    if (isChecked) {
      ...
      setCols((prevCols) => {
        ...
        saveToLocalStorageSheet('S_ERP_COLS_PAGE_QA_CUST_QC_LEFT', newCols)
        return newCols
      })
      setHiddenColumns((prevHidden) => {
...
export default TableQaCustQcTitleLeft
