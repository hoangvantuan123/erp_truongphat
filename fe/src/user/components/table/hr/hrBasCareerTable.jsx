...
import { CellsUMRetReasonName } from '../../sheet/cells/cellUMRetReasonName'

const parseBoolean = (value) => {
    const normalized = String(value).trim().toLowerCase();
    return normalized === '1' || normalized === 'true';
};
function HrBasCareerTable({
    setSelection,
    selection,
    setShowSearch,
    showSearch,
    ...
    const [hiddenColumns, setHiddenColumns] = useState(() => {
        return loadFromLocalStorageSheet('hr_bas_career_h', [])
    })
    ...
    const getData = useCallback(
        ([col, row]) => {
            ...
            /* 
                        if (columnKey === 'EntDate' || columnKey === "RetDate" || columnKey === "HandiAppdate") {
                            let displayValue = ''
                            let dataValue = value
        ...
        [gridData, cols, helpData02, helpData03, helpData04]
    )
    ...
    const onCellEdited = useCallback(
            ...
            if (key === 'UMRetReasonName') {
                if (newValue.kind === GridCellKind.Custom) {
                    setGridData((prev) => {
                        ...
                        if (selectedName) {
                            product[cols[col].id] = selectedName.MinorName
                            product['UMRetReason'] = selectedName.Value
        ...
        [cols, gridData, helpData03, helpData04]
    )
    ...
    const updateHiddenColumns = (newHiddenColumns) => {
        setHiddenColumns((prevHidden) => {
            ...
            saveToLocalStorageSheet('hr_bas_career_h', newHidden)
            return newHidden
        })
    }
    ...
    const updateVisibleColumns = (newVisibleColumns) => {
        setCols((prevCols) => {
            ...
            saveToLocalStorageSheet('hr_bas_career_a', uniqueCols)
            return uniqueCols
        })
    }
    ...
    const handleReset = () => {
        ...
        setHiddenColumns([])
        localStorage.removeItem('hr_bas_career_a')
        localStorage.removeItem('hr_bas_career_h')
    }
    ...
    const onColumnMoved = useCallback((startIndex, endIndex) => {
        setCols((prevCols) => {
            ...
            saveToLocalStorageSheet('hr_bas_career_a', updatedCols)
            return updatedCols
        })
    }, [])
    ...
    const showDrawer = () => {
        ...
        const currentVisibleCols = loadFromLocalStorageSheet('hr_bas_career_a', []).map(
            (col) => col.id
        )
    ...
    const handleCheckboxChange = (columnId, isChecked) => {
        if (isChecked) {
            ...
            setCols((prevCols) => {
                ...
                saveToLocalStorageSheet('hr_bas_career_a', newCols)
                return newCols
            })
            setHiddenColumns((prevHidden) => {
...
export default HrBasCareerTable
