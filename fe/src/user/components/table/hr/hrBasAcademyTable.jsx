...
function HrBasAcademyTable({
    ...
    const [hiddenColumns, setHiddenColumns] = useState(() => {
        return loadFromLocalStorageSheet('hr_emp_h', [])
    })
    ...
    const onCellEdited = useCallback(
        async (cell, newValue) => {
            ...
            if (key === 'FamilySeq' || key === 'EmpID' || key === "EntRetTypeName"
                ...
                || key === "UMHandiGrd"
                || key === "SMDayNightType"
                || key === 'DegreeNo' || key === "EntRetTypeName"
            ) {
                return
            }
    ...
    const updateHiddenColumns = (newHiddenColumns) => {
        setHiddenColumns((prevHidden) => {
            ...
            saveToLocalStorageSheet('hr_emp_h', newHidden)
            return newHidden
        })
    }
    ...
    const updateVisibleColumns = (newVisibleColumns) => {
        setCols((prevCols) => {
            ...
            saveToLocalStorageSheet('hr_emp_a', uniqueCols)
            return uniqueCols
        })
    }
    ...
    const handleReset = () => {
        ...
        setHiddenColumns([])
        localStorage.removeItem('hr_emp_a')
        localStorage.removeItem('hr_emp_h')
    }
    ...
    const onColumnMoved = useCallback((startIndex, endIndex) => {
        setCols((prevCols) => {
            ...
            saveToLocalStorageSheet('hr_emp_a', updatedCols)
            return updatedCols
        })
    }, [])
    ...
    const showDrawer = () => {
        ...
        const currentVisibleCols = loadFromLocalStorageSheet('hr_emp_a', []).map(
            (col) => col.id
        )
    ...
    const handleCheckboxChange = (columnId, isChecked) => {
        if (isChecked) {
            ...
            setCols((prevCols) => {
                ...
                saveToLocalStorageSheet('hr_emp_a', newCols)
                return newCols
            })
            setHiddenColumns((prevHidden) => {
...
export default HrBasAcademyTable
