...
function HrBasFamilyTable({
    ...
    const [hiddenColumns, setHiddenColumns] = useState(() => {
        return loadFromLocalStorageSheet('hr_bas_family_h', [])
    })
    ...
    const getData = useCallback(
        ([col, row]) => {
            ...
            if (columnKey === "FamilyResidID" || columnKey === "DispSeq") {
        ...
        [gridData, cols, helpData02, helpData03, helpData04, helpData05, helpData06, helpData07, helpData08]
    )
    ...
    const onCellEdited = useCallback(
        async (cell, newValue) => {
            ...
            if (key === "FamilyResidID" || key === "DispSeq") {
                const input = (newValue.data ?? "").toString().trim();
                ...
                const isOnlyDigits = /^[0-9]+$/.test(input);
        ...
        [cols, gridData, helpData02, helpData03, helpData04, helpData05, helpData06, helpData07, helpData08]
    )
    ...
    const updateHiddenColumns = (newHiddenColumns) => {
        setHiddenColumns((prevHidden) => {
            ...
            saveToLocalStorageSheet('hr_bas_family_h', newHidden)
            return newHidden
        })
    }
    ...
    const updateVisibleColumns = (newVisibleColumns) => {
        setCols((prevCols) => {
            ...
            saveToLocalStorageSheet('hr_bas_family_a', uniqueCols)
            return uniqueCols
        })
    }
    ...
    const handleReset = () => {
        ...
        setHiddenColumns([])
        localStorage.removeItem('hr_bas_family_a')
        localStorage.removeItem('hr_bas_family_h')
    }
    ...
    const onColumnMoved = useCallback((startIndex, endIndex) => {
        setCols((prevCols) => {
            ...
            saveToLocalStorageSheet('hr_bas_family_a', updatedCols)
            return updatedCols
        })
    }, [])
    ...
    const showDrawer = () => {
        ...
        const currentVisibleCols = loadFromLocalStorageSheet('hr_bas_family_a', []).map(
            (col) => col.id
        )
    ...
    const handleCheckboxChange = (columnId, isChecked) => {
        if (isChecked) {
            ...
            setCols((prevCols) => {
                ...
                saveToLocalStorageSheet('hr_bas_family_a', newCols)
                return newCols
            })
            setHiddenColumns((prevHidden) => {
...
export default HrBasFamilyTable
