...
import { CellsUMChrgWkName } from '../../sheet/cells/cellUMChrgWkName'
import { CellsUMPjtTypeName } from '../../sheet/cells/cellUMPjtTypeName'
import { CellsJobName } from '../../sheet/cells/cellJobName'
import { Save, Scissors , Search} from 'lucide-react';
import moment from 'moment'
import { useTranslation } from 'react-i18next'
function HrBasCareerProjectTable({
    setSelection,
    selection,
    setShowSearch,
    showSearch,
    ...
    const [hiddenColumns, setHiddenColumns] = useState(() => {
        return loadFromLocalStorageSheet('hr_bas_pjt_career_h', [])
    })
    ...
    const getData = useCallback(
        ...
        [gridData, cols, helpData03, helpData07, helpData09]
    )
    ...
    const onCellEdited = useCallback(
                ...
                if (newValue.kind === GridCellKind.Custom) {
                    setGridData((prev) => {
                        ...
                        if (selectedName) {
                            ...
                            product['UMChrgWkSeq'] = selectedName.Value
            ...
            if (key === 'UMPjtTypeName') {
                if (newValue.kind === GridCellKind.Custom) {
                    setGridData((prev) => {
                        ...
                        if (selectedName) {
                            product[cols[col].id] = selectedName.MinorName
                            product['UMPjtType'] = selectedName.Value
            ...
            if (key === 'JobName') {
                if (newValue.kind === GridCellKind.Custom) {
                    setGridData((prev) => {
                        ...
                        if (selectedName) {
                            product[cols[col].id] = selectedName.JobName
        ...
        [cols, gridData, helpData03, helpData07, helpData09]
    )
    ...
    const updateHiddenColumns = (newHiddenColumns) => {
        setHiddenColumns((prevHidden) => {
            ...
            saveToLocalStorageSheet('hr_bas_pjt_career_h', newHidden)
            return newHidden
        })
    }
    ...
    const updateVisibleColumns = (newVisibleColumns) => {
        setCols((prevCols) => {
            ...
            saveToLocalStorageSheet('hr_bas_pjt_career_a', uniqueCols)
            return uniqueCols
        })
    }
    ...
    const handleReset = () => {
        ...
        setHiddenColumns([])
        localStorage.removeItem('hr_bas_pjt_career_a')
        localStorage.removeItem('hr_bas_pjt_career_h')
    }
    ...
    const onColumnMoved = useCallback((startIndex, endIndex) => {
        setCols((prevCols) => {
            ...
            saveToLocalStorageSheet('hr_bas_pjt_career_a', updatedCols)
            return updatedCols
        })
    }, [])
    ...
    const showDrawer = () => {
        ...
        const currentVisibleCols = loadFromLocalStorageSheet('hr_bas_pjt_career_a', []).map(
            (col) => col.id
        )
    ...
    const handleCheckboxChange = (columnId, isChecked) => {
        if (isChecked) {
            ...
            setCols((prevCols) => {
                ...
                saveToLocalStorageSheet('hr_bas_pjt_career_a', newCols)
                return newCols
            })
            setHiddenColumns((prevHidden) => {
...
export default HrBasCareerProjectTable
