...
import { CellsUMJobHighClassName } from '../../sheet/cells/cellsUMJobHighClassName'
import { PostQHrBasOrgJobClasSeq } from '../../../../features/hr/hrBasOrgJob/postQHrBasOrgJobClassSeq'
import moment from 'moment'
function HrBasOrgJobTable({
    ...
    const [hiddenColumns, setHiddenColumns] = useState(() => {
        return loadFromLocalStorageSheet('hr_bas_org_job_h', [])
    })
                ...
                if (newValue.kind === GridCellKind.Custom) {
                    ...
                    setGridData((prev) => {
                        ...
                        if (selectedName) {
                            ...
                            product['UMJobClass'] = selectedName.Value;
                        } else {
                            product[cols[col].id] = '';
                            product['UMJobClass'] = '';
                            product['UMJobHighClassName'] = '';
                            product['UMJobHighClass'] = '';
                        }
                    ...
                    if (selectedName) {
                        PostQHrBasOrgJobClasSeq({
                            IdxNo: row + 1,
                            UMJobClass: Number(selectedName.Value),
                        }).then((response) => {
                            if (response?.success) {
                                ...
                                setGridData((prev) => {
                                    const newData = [...prev];
                                    const product = newData[row];
                                    product['UMJobHighClassName'] = logs?.UMJobHighClassName || '';
                                    product['UMJobHighClass'] = logs?.UMJobHighClass || '';
                                    return newData;
                                });
                            }
                        });
    ...
    const updateHiddenColumns = (newHiddenColumns) => {
        setHiddenColumns((prevHidden) => {
            ...
            saveToLocalStorageSheet('hr_bas_org_job_h', newHidden)
            return newHidden
        })
    }
    ...
    const updateVisibleColumns = (newVisibleColumns) => {
        setCols((prevCols) => {
            ...
            saveToLocalStorageSheet('hr_bas_org_job_a', uniqueCols)
            return uniqueCols
        })
    }
    ...
    const handleReset = () => {
        ...
        setHiddenColumns([])
        localStorage.removeItem('hr_bas_org_job_a')
        localStorage.removeItem('hr_bas_org_job_h')
    }
    ...
    const onColumnMoved = useCallback((startIndex, endIndex) => {
        setCols((prevCols) => {
            ...
            saveToLocalStorageSheet('hr_bas_org_job_a', updatedCols)
            return updatedCols
        })
    }, [])
    ...
    const showDrawer = () => {
        ...
        const currentVisibleCols = loadFromLocalStorageSheet('hr_bas_org_job_a', []).map(
            (col) => col.id
        )
    ...
    const handleCheckboxChange = (columnId, isChecked) => {
        if (isChecked) {
            ...
            setCols((prevCols) => {
                ...
                saveToLocalStorageSheet('hr_bas_org_job_a', newCols)
                return newCols
            })
            setHiddenColumns((prevHidden) => {
...
export default HrBasOrgJobTable
