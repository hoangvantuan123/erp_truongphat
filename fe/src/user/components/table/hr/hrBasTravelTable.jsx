...
function HrBasTravelTable({
    ...
    const [hiddenColumns, setHiddenColumns] = useState(() => {
        return loadFromLocalStorageSheet('hr_bas_travel_rec_h', [])
    })
    ...
    const onCellEdited = useCallback(
        async (cell, newValue) => {
            ...
            if (key === 'FamilySeq' || key === 'EmpID' || key === "EntRetTypeName"
                || key === "DeptName" || key === "PosName" || key === "UMNationSeq"
                || key === 'SMAbrd'
            ) {
                return
            }
    ...
    const updateHiddenColumns = (newHiddenColumns) => {
        setHiddenColumns((prevHidden) => {
            ...
            saveToLocalStorageSheet('hr_bas_travel_rec_h', newHidden)
            return newHidden
        })
    }
    ...
    const updateVisibleColumns = (newVisibleColumns) => {
        setCols((prevCols) => {
            ...
            saveToLocalStorageSheet('hr_bas_travel_rec_a', uniqueCols)
            return uniqueCols
        })
    }
    ...
    const handleReset = () => {
        ...
        setHiddenColumns([])
        localStorage.removeItem('hr_bas_travel_rec_a')
        localStorage.removeItem('hr_bas_travel_rec_h')
    }
    ...
    const onColumnMoved = useCallback((startIndex, endIndex) => {
        setCols((prevCols) => {
            ...
            saveToLocalStorageSheet('hr_bas_travel_rec_a', updatedCols)
            return updatedCols
        })
    }, [])
    ...
    const showDrawer = () => {
        ...
        const currentVisibleCols = loadFromLocalStorageSheet('hr_bas_travel_rec_a', []).map(
            (col) => col.id
        )
    ...
    const handleCheckboxChange = (columnId, isChecked) => {
        if (isChecked) {
            ...
            setCols((prevCols) => {
                ...
                saveToLocalStorageSheet('hr_bas_travel_rec_a', newCols)
                return newCols
            })
            setHiddenColumns((prevHidden) => {
...
export default HrBasTravelTable
