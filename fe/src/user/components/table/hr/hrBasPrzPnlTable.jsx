...
function HrBasPrzPnlTable({
    ...
    const [hiddenColumns, setHiddenColumns] = useState(() => {
        return loadFromLocalStorageSheet('hr_prz_pnl_h', [])
    })
    ...
    const onCellEdited = useCallback(
        async (cell, newValue) => {
            ...
            if (key === 'FamilySeq' || key === 'EmpID' || key === "EntRetTypeName"
                || key === "DeptName" || key === "PosName" || key === 'PrzPnlSeq'
                || key === "SMPrzPnlType" || key === "SMInOutType" || key === "UMPrzPnlSeq"
        ...
        [cols, gridData, helpData04, helpData05, helpData06]
    )
    ...
    const updateHiddenColumns = (newHiddenColumns) => {
        setHiddenColumns((prevHidden) => {
            ...
            saveToLocalStorageSheet('hr_prz_pnl_h', newHidden)
            return newHidden
        })
    }
    ...
    const updateVisibleColumns = (newVisibleColumns) => {
        setCols((prevCols) => {
            ...
            saveToLocalStorageSheet('hr_prz_pnl_a', uniqueCols)
            return uniqueCols
        })
    }
    ...
    const handleReset = () => {
        ...
        setHiddenColumns([])
        localStorage.removeItem('hr_prz_pnl_a')
        localStorage.removeItem('hr_prz_pnl_h')
    }
    ...
    const onColumnMoved = useCallback((startIndex, endIndex) => {
        setCols((prevCols) => {
            ...
            saveToLocalStorageSheet('hr_prz_pnl_a', updatedCols)
            return updatedCols
        })
    }, [])
    ...
    const showDrawer = () => {
        ...
        const currentVisibleCols = loadFromLocalStorageSheet('hr_prz_pnl_a', []).map(
            (col) => col.id
        )
    ...
    const handleCheckboxChange = (columnId, isChecked) => {
        if (isChecked) {
            ...
            setCols((prevCols) => {
                ...
                saveToLocalStorageSheet('hr_prz_pnl_a', newCols)
                return newCols
            })
            setHiddenColumns((prevHidden) => {
...
export default HrBasPrzPnlTable
