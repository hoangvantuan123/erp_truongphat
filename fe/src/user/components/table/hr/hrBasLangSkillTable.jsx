...
import { CellsUMLanguageTypeName } from '../../sheet/cells/cellUMLanguageTypeName'
import { GetCodeHelp } from '../../../../features/codeHelp/getCodeHelp'
import { togglePageInteraction } from '../../../../utils/togglePageInteraction'
import { CellsUMAuthTypeName } from '../../sheet/cells/cellUMAuthTypeName'
import { CellsUMGradeName } from '../../sheet/cells/cellUMGradeName'

const parseBoolean = (value) => {
    const normalized = String(value).trim().toLowerCase();
    return normalized === '1' || normalized === 'true';
};
function HrBasLangSkillTable({
    data,
    setSelection,
    selection,
    setShowSearch,
    ...
    const [hiddenColumns, setHiddenColumns] = useState(() => {
        return loadFromLocalStorageSheet('hr_bas_lang_skill_h', [])
    })
    ...
    const onCellEdited = useCallback(
        async (cell, newValue) => {
            ...
            if (key === 'EmpName' && newValue.kind === GridCellKind.Custom) {
                ...
                setGridData((prev) => {
                    ...
                    if (selectedName) {
                        ...
                        product['EntRetTypeName'] = selectedName.TypeName
                ...
                if (newValue.kind === GridCellKind.Custom) {
                    setGridData((prev) => {
                        ...
                        if (selectedName) {
                            ...
                            product['UMLanguageType'] = selectedName.Value

                        } else {
                            product[cols[col].id] = ''
                            product['UMLanguageType'] = ''
            ...
            if (key === 'UMAuthTypeName') {
                if (newValue.kind === GridCellKind.Custom) {
                    setGridData((prev) => {
                        ...
                        if (selectedName) {
                            product[cols[col].id] = selectedName.MinorName
                            product['UMAuthType'] = selectedName.Value
            ...
            if (key === 'UMGradeName') {
                if (newValue.kind === GridCellKind.Custom) {
                    setGridData((prev) => {
                        ...
                        if (selectedName) {
                            product[cols[col].id] = selectedName.MinorName
                            product['UMGrade'] = selectedName.Value
            ...
            if (key === 'linguisticSeq' || key === 'EmpID' || key === "EntRetTypeName"
                || key === "DeptName" || key === "PosName"
                || key === "UMLanguageType" || key === "UMAuthType"
                || key === "UMGrade"
            ) {
                return
            }
    ...
    const updateHiddenColumns = (newHiddenColumns) => {
        setHiddenColumns((prevHidden) => {
            ...
            saveToLocalStorageSheet('hr_bas_lang_skill_h', newHidden)
            return newHidden
        })
    }
    ...
    const updateVisibleColumns = (newVisibleColumns) => {
        setCols((prevCols) => {
            ...
            saveToLocalStorageSheet('hr_bas_lang_skill_a', uniqueCols)
            return uniqueCols
        })
    }
    ...
    const handleReset = () => {
        ...
        setHiddenColumns([])
        localStorage.removeItem('hr_bas_lang_skill_a')
        localStorage.removeItem('hr_bas_lang_skill_h')
    }
    ...
    const onColumnMoved = useCallback((startIndex, endIndex) => {
        setCols((prevCols) => {
            ...
            saveToLocalStorageSheet('hr_bas_lang_skill_a', updatedCols)
            return updatedCols
        })
    }, [])
    ...
    const showDrawer = () => {
        ...
        const currentVisibleCols = loadFromLocalStorageSheet('hr_bas_lang_skill_a', []).map(
            (col) => col.id
        )
    ...
    const handleCheckboxChange = (columnId, isChecked) => {
        if (isChecked) {
            ...
            setCols((prevCols) => {
                ...
                saveToLocalStorageSheet('hr_bas_lang_skill_a', newCols)
                return newCols
            })
            setHiddenColumns((prevHidden) => {
...
export default HrBasLangSkillTable
