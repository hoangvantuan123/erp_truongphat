...
import { CellsCCtrName } from '../../sheet/cells/cellCCtrName'
import { CellsUMJoName } from '../../sheet/cells/cellUMJoName'
import { CellsUMPosLvlName } from '../../sheet/cells/cellUMPosLvlName'
import moment from 'moment'
function HrBasOrgPosTable({
    data,
    setSelection,
    selection,
    setShowSearch,
    ...
    const [hiddenColumns, setHiddenColumns] = useState(() => {
        return loadFromLocalStorageSheet('hr_bas_org_pos_h', [])
    })
                    ...
                    setGridData((prev) => {
                        ...
                        if (!selectedName) {
                            selectedName = helpData02.find(
                                (item) => item.CCtrName === checkCopyData,
                            )
                        }
                        if (selectedName) {
                            product[cols[col].id] = selectedName.CCtrName
                            product['CCtrSeq'] = selectedName.CCtrSeq
            ...
            if (key === 'UMPosLvlName') {
                if (newValue.kind === GridCellKind.Custom) {
                    setGridData((prev) => {
                        ...
                        if (selectedName) {
                            product[cols[col].id] = selectedName.MinorName
                            product['UMPosLvlSeq'] = selectedName.Value
    ...
    const updateHiddenColumns = (newHiddenColumns) => {
        setHiddenColumns((prevHidden) => {
            ...
            saveToLocalStorageSheet('hr_bas_org_pos_h', newHidden)
            return newHidden
        })
    }
    ...
    const updateVisibleColumns = (newVisibleColumns) => {
        setCols((prevCols) => {
            ...
            saveToLocalStorageSheet('hr_bas_org_pos_a', uniqueCols)
            return uniqueCols
        })
    }
    ...
    const handleReset = () => {
        ...
        setHiddenColumns([])
        localStorage.removeItem('hr_bas_org_pos_a')
        localStorage.removeItem('hr_bas_org_pos_h')
    }
    ...
    const onColumnMoved = useCallback((startIndex, endIndex) => {
        setCols((prevCols) => {
            ...
            saveToLocalStorageSheet('hr_bas_org_pos_a', updatedCols)
            return updatedCols
        })
    }, [])
    ...
    const showDrawer = () => {
        ...
        const currentVisibleCols = loadFromLocalStorageSheet('hr_bas_org_pos_a', []).map(
            (col) => col.id
        )
    ...
    const handleCheckboxChange = (columnId, isChecked) => {
        if (isChecked) {
            ...
            setCols((prevCols) => {
                ...
                saveToLocalStorageSheet('hr_bas_org_pos_a', newCols)
                return newCols
            })
            setHiddenColumns((prevHidden) => {
...
export default HrBasOrgPosTable
