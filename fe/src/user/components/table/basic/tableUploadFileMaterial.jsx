...
function TableUploadFileMaterial({
    setSelection,
    selection,
    setShowSearch,
    showSearch,
    ...
    const [hiddenColumns, setHiddenColumns] = useState(() => {
        return loadFromLocalStorageSheet('da_material_list_more_ch', [])
    })
    ...
    const getData = useCallback(
        ([col, row]) => {
            ...
            if (columnKey === 'OriginalName') {
                const fileUrl = `${HOST_API_SERVER_5}/${person['Filename']}`;
    ...
    const updateHiddenColumns = (newHiddenColumns) => {
        setHiddenColumns((prevHidden) => {
            ...
            saveToLocalStorageSheet('da_material_list_more_ch', newHidden)
            return newHidden
        })
    }
    ...
    const updateVisibleColumns = (newVisibleColumns) => {
        setCols((prevCols) => {
            ...
            saveToLocalStorageSheet('da_material_list_more_c', uniqueCols)
            return uniqueCols
        })
    }
    ...
    const handleReset = () => {
        ...
        setHiddenColumns([])
        localStorage.removeItem('da_material_list_more_c')
        localStorage.removeItem('da_material_list_more_ch')
        setShowMenu(null)
    }
    ...
    const onColumnMoved = useCallback((startIndex, endIndex) => {
        setCols((prevCols) => {
            ...
            saveToLocalStorageSheet('da_material_list_more_c', updatedCols)
            return updatedCols
        })
    }, [])
    ...
    const handleCheckboxChange = (columnId, isChecked) => {
        if (isChecked) {
            ...
            setCols((prevCols) => {
                ...
                saveToLocalStorageSheet('da_material_list_more_c', newCols)
                return newCols
            })
            setHiddenColumns((prevHidden) => {
    ...
    return (
        <div className="w-full h-screen flex flex-col overflow-hidden">
            <div className="flex-1 overflow-auto flex flex-col items-center justify-center">
                <Dragger {...uploadProps} style={{ borderRadius: '0', border: 'none' }} className={`w-full cursor-${dataSub.length > 0 ? 'pointer' : 'not-allowed'}`}
...
export default TableUploadFileMaterial