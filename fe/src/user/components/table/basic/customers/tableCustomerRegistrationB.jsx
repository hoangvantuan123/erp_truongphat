...
import { CellsUMJobRollKind } from '../../../sheet/cells/cellsUMJobRollKind'


function TableCustomerRegistrationB({
  setSelection,
  selection,
  setShowSearch,
  showSearch,
  ...
  const [hiddenColumns, setHiddenColumns] = useState(() => {
    return loadFromLocalStorageSheet('page_customrer_reg_b_h', [])
  })
        ...
        if (newValue.kind === GridCellKind.Custom) {
          setGridData(prev => {
            ...
            if (selectedName) {
              ...
              product["UMJobRollKind"] = selectedName.Value;
            } else {
              product[key] = "";
              product["UMJobRollKind"] = "";
            }
  ...
  const updateHiddenColumns = (newHiddenColumns) => {
    setHiddenColumns((prevHidden) => {
      ...
      saveToLocalStorageSheet('page_customrer_reg_b_h', newHidden)
      return newHidden
    })
  }
  ...
  const updateVisibleColumns = (newVisibleColumns) => {
    setCols((prevCols) => {
      ...
      saveToLocalStorageSheet('page_customrer_reg_a', uniqueCols)
      return uniqueCols
    })
  }
  ...
  const handleReset = () => {
    ...
    setHiddenColumns([])
    localStorage.removeItem('page_customrer_reg_a')
    localStorage.removeItem('page_customrer_reg_b_h')
  }
  ...
  const onColumnMoved = useCallback((startIndex, endIndex) => {
    setCols((prevCols) => {
      ...
      saveToLocalStorageSheet('page_customrer_reg_a', updatedCols)
      return updatedCols
    })
  }, [])
  ...
  const showDrawer = () => {
    ...
    const currentVisibleCols = loadFromLocalStorageSheet('page_customrer_reg_a', []).map(
      (col) => col.id
    )
  ...
  const handleCheckboxChange = (columnId, isChecked) => {
    if (isChecked) {
      ...
      setCols((prevCols) => {
        ...
        saveToLocalStorageSheet('page_customrer_reg_a', newCols)
        return newCols
      })
      setHiddenColumns((prevHidden) => {
    ...
    <div className="w-full gap-1 h-full flex items-center justify-center">
      <div className="w-full h-full flex flex-col  bg-white  overflow-hidden ">
        <div className='flex items-center px-2 p-1 justify-between border-b text-[10px] text-black  font-medium uppercase'>
          <span>DANH SÁCH THÔNG TIN ĐỊA CHỈ NGƯỜI PHỤ TRÁCH</span>
        </div>
        <DataEditor
          {...cellProps}
          ref={gridRef}
          ...
          onKeyUp={onKeyUp}
          customRenderers={[CellsUMJobRollKind]}
...
export default TableCustomerRegistrationB
