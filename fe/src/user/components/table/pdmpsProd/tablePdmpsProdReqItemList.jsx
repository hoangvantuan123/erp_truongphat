...
function TablePdmpsProdReqItemList({
  setSelection,
  selection,
  setShowSearch,
  showSearch,
  ...
  const [hiddenColumns, setHiddenColumns] = useState(() => {
    return loadFromLocalStorageSheet('pdmps_prod_req_item_list_ah', [])
  })
      ...
      if (row === lastRowIndex) {
        if (['Qty'].includes(columnKey)) {
          const total = gridData.reduce(
            (sum, item) => sum + (Number(item[columnKey]) || 0),
            0,
          )
  ...
  const updateHiddenColumns = (newHiddenColumns) => {
    setHiddenColumns((prevHidden) => {
      ...
      saveToLocalStorageSheet('pdmps_prod_req_item_list_ah', newHidden)
      return newHidden
    })
  }
  ...
  const updateVisibleColumns = (newVisibleColumns) => {
    setCols((prevCols) => {
      ...
      saveToLocalStorageSheet('pdmps_prod_req_item_list_a', uniqueCols)
      return uniqueCols
    })
  }
  ...
  const handleReset = () => {
    ...
    setHiddenColumns([])
    localStorage.removeItem('pdmps_prod_req_item_list_a')
    localStorage.removeItem('pdmps_prod_req_item_list_ah')
    setShowMenu(null)
  }
  ...
  const onColumnMoved = useCallback((startIndex, endIndex) => {
    setCols((prevCols) => {
      ...
      saveToLocalStorageSheet('pdmps_prod_req_item_list_a', updatedCols)
      return updatedCols
    })
  }, [])
  ...
  const handleCheckboxChange = (columnId, isChecked) => {
    if (isChecked) {
      ...
      setCols((prevCols) => {
        ...
        saveToLocalStorageSheet('pdmps_prod_req_item_list_a', newCols)
        return newCols
      })
      setHiddenColumns((prevHidden) => {
...
export default TablePdmpsProdReqItemList
