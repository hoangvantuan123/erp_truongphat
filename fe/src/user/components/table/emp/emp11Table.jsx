...
import { CellsSMAbrdName } from '../../sheet/cells/cellSMAbrdName'
...
function Emp11Table({
    ...
    const [hiddenColumns, setHiddenColumns] = useState(() => {
        return loadFromLocalStorageSheet('emp_11_h', [])
    })
    ...
    const getData = useCallback(
        ([col, row]) => {
            ...
            const cellConfig = {
                UMNationName: {
                    ...
                    allowedValues: helpData19,
                    setCacheData: setHelpData20,
                },
                SMAbrdName: {
                    kind: 'cell-SMAbrdName',
                    allowedValues: helpData20,
                    setCacheData: setHelpData20,
                },

            }

            if (columnKey === 'DptDate' || columnKey === "ArvDate") {
                let displayValue = ''
                let dataValue = value
        ...
        [gridData, cols, helpData19, helpData20]
    )
    ...
    const updateHiddenColumns = (newHiddenColumns) => {
        setHiddenColumns((prevHidden) => {
            ...
            saveToLocalStorageSheet('emp_11_h', newHidden)
            return newHidden
        })
    }
    ...
    const updateVisibleColumns = (newVisibleColumns) => {
        setCols((prevCols) => {
            ...
            saveToLocalStorageSheet('emp_11_a', uniqueCols)
            return uniqueCols
        })
    }
    ...
    const handleReset = () => {
        ...
        setHiddenColumns([])
        localStorage.removeItem('emp_11_a')
        localStorage.removeItem('emp_11_h')
    }
    ...
    const onColumnMoved = useCallback((startIndex, endIndex) => {
        setCols((prevCols) => {
            ...
            saveToLocalStorageSheet('emp_11_a', updatedCols)
            return updatedCols
        })
    }, [])
    ...
    const showDrawer = () => {
        ...
        const currentVisibleCols = loadFromLocalStorageSheet('emp_11_a', []).map(
            (col) => col.id
        )
    ...
    const handleCheckboxChange = (columnId, isChecked) => {
        if (isChecked) {
            ...
            setCols((prevCols) => {
                ...
                saveToLocalStorageSheet('emp_11_a', newCols)
                return newCols
            })
            setHiddenColumns((prevHidden) => {
    ...
    const onCellEdited = useCallback(
            ...
            if (key === 'SMAbrdName') {
                if (newValue.kind === GridCellKind.Custom) {
                    setGridData((prev) => {
                        ...
                        const checkCopyData = newValue.copyData
                        if (!selectedName) {
                            selectedName = helpData20.find(
                                (item) => item.MinorName === checkCopyData,
                            )
                        }
                        if (selectedName) {
                            product[cols[col].id] = selectedName.MinorName
                            product['SMAbrd'] = selectedName.Value
        ...
        [cols, gridData, helpData19, helpData20]
    )
    return (
        <div className="w-full gap-1 h-full flex items-center justify-center">
            <div className="w-full h-full flex flex-col  bg-white  overflow-hidden ">
                <h2 className="text-[10px]  text-blue-600 border-b font-medium flex items-center gap-2 p-2  uppercase">
                    Đăng ký hạng mục thông tin công tác
                </h2>
                <DataEditor
                    {...cellProps}
                    ref={gridRef}
