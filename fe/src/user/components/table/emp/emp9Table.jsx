...
import { CellsUMLicName } from '../../sheet/cells/cellUMLicName'
...
function Emp9Table({
    ...
    const [hiddenColumns, setHiddenColumns] = useState(() => {
        return loadFromLocalStorageSheet('emp_09_h', [])
    })
    ...
    const getData = useCallback(
        ([col, row]) => {
            ...
            const cellConfig = {
                UMLicName: {
                    kind: 'cell-UMLicName',
                    allowedValues: helpData46,
                    setCacheData: setHelpData46,
                },
            ...
            if (columnKey === 'ValDate' || columnKey === "AcqDate") {
                let displayValue = ''
                let dataValue = value
    ...
    const updateHiddenColumns = (newHiddenColumns) => {
        setHiddenColumns((prevHidden) => {
            ...
            saveToLocalStorageSheet('emp_09_h', newHidden)
            return newHidden
        })
    }
    ...
    const updateVisibleColumns = (newVisibleColumns) => {
        setCols((prevCols) => {
            ...
            saveToLocalStorageSheet('emp_09_a', uniqueCols)
            return uniqueCols
        })
    }
    ...
    const handleReset = () => {
        ...
        setHiddenColumns([])
        localStorage.removeItem('emp_09_a')
        localStorage.removeItem('emp_09_h')
    }
    ...
    const onColumnMoved = useCallback((startIndex, endIndex) => {
        setCols((prevCols) => {
            ...
            saveToLocalStorageSheet('emp_09_a', updatedCols)
            return updatedCols
        })
    }, [])
    ...
    const showDrawer = () => {
        ...
        const currentVisibleCols = loadFromLocalStorageSheet('emp_09_a', []).map(
            (col) => col.id
        )
    ...
    const handleCheckboxChange = (columnId, isChecked) => {
        if (isChecked) {
            ...
            setCols((prevCols) => {
                ...
                saveToLocalStorageSheet('emp_09_a', newCols)
                return newCols
            })
            setHiddenColumns((prevHidden) => {
