...
import { CellsSMDayNightTypeName } from '../../sheet/cells/cellSMDayNightTypeName'
import { CellsUMDegreeType } from '../../sheet/cells/cellUMDegreeType'
...
function Emp6Table({
    ...
    const [hiddenColumns, setHiddenColumns] = useState(() => {
        return loadFromLocalStorageSheet('emp_06_h', [])
    })
    ...
    const getData = useCallback(
        ([col, row]) => {
            ...
            const cellConfig = {
                ...
                SMDayNightTypeName: {
                    kind: 'cell-SMDayNightTypeName',
                    allowedValues: helpData11,
                    setCacheData: setHelpData11,
                },
                UMDegreeTypeName: {
                    kind: 'cell-UMDegreeType',
                    allowedValues: helpData12,
                    setCacheData: setHelpData12,
                },
            ...
            if (columnKey === 'GrdYm' || columnKey === 'EntYm') {
                let displayValue = '';
                let dataValue = value;
                ...
                if (value && typeof value === 'string' && value.trim() !== '') {
                    ...
                    if (/^\d{6}$/.test(value)) {
                        momentObj = moment(value, "YYYYMM", true);
                    } else if (/^\d{4}[-/]\d{2}$/.test(value)) {
                        momentObj = moment(value, "YYYY-MM", true) || moment(value, "YYYY/MM", true);
                    }

                    if (momentObj && momentObj.isValid()) {
                        displayValue = momentObj.format("YYYY-MM");
                        dataValue = momentObj.format("YYYYMM");
                    } else {
                        displayValue = '';
                        dataValue = '';
                    }
        ...
        [gridData, cols, helpData10, helpData11, helpData12,]
    )
    ...
    const updateHiddenColumns = (newHiddenColumns) => {
        setHiddenColumns((prevHidden) => {
            ...
            saveToLocalStorageSheet('emp_06_h', newHidden)
            return newHidden
        })
    }
    ...
    const updateVisibleColumns = (newVisibleColumns) => {
        setCols((prevCols) => {
            ...
            saveToLocalStorageSheet('emp_06_a', uniqueCols)
            return uniqueCols
        })
    }
    ...
    const handleReset = () => {
        ...
        setHiddenColumns([])
        localStorage.removeItem('emp_06_a')
        localStorage.removeItem('emp_06_h')
    }
    ...
    const onColumnMoved = useCallback((startIndex, endIndex) => {
        setCols((prevCols) => {
            ...
            saveToLocalStorageSheet('emp_06_a', updatedCols)
            return updatedCols
        })
    }, [])
    ...
    const showDrawer = () => {
        ...
        const currentVisibleCols = loadFromLocalStorageSheet('emp_06_a', []).map(
            (col) => col.id
        )
    ...
    const handleCheckboxChange = (columnId, isChecked) => {
        if (isChecked) {
            ...
            setCols((prevCols) => {
                ...
                saveToLocalStorageSheet('emp_06_a', newCols)
                return newCols
            })
            setHiddenColumns((prevHidden) => {
    ...
    const onCellEdited = useCallback(
            ...
            if (key === 'SMDayNightTypeName') {
                if (newValue.kind === GridCellKind.Custom) {
                    setGridData((prev) => {
                        ...
                        if (selectedName) {
                            product[cols[col].id] = selectedName.MinorName
                            product['SMDayNightType'] = selectedName.Value

                        } else {
                            product[cols[col].id] = ''
                            product['SMDayNightType'] = ''
            ...
            if (key === 'UMDegreeTypeName') {
                if (newValue.kind === GridCellKind.Custom) {
                    setGridData((prev) => {
                        ...
                        const checkCopyData = newValue.copyData
                        if (!selectedName) {
                            selectedName = helpData12.find(
                                (item) => item.MinorName === checkCopyData,
                            )
                        }
                        if (selectedName) {
                            product[cols[col].id] = selectedName.MinorName
                            product['UMDegreeType'] = selectedName.Value

                        } else {
                            product[cols[col].id] = ''
                            product['UMDegreeType'] = ''
        ...
        [cols, gridData.helpData10, helpData11, helpData12,]
    )
    return (
        <div className="w-full gap-1 h-full flex items-center justify-center">
            <div className="w-full h-full flex flex-col  bg-white  overflow-hidden ">
                ...
                <DataEditor
                    ...
                    customRenderers={[
                        CellsUMSchCareerName,
                        CellsSMDayNightTypeName,
                        CellsUMDegreeType
