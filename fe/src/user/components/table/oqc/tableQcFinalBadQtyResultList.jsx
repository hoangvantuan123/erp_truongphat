...
function TableQcFinalBadQtyResultList({
  dataProgStatusName,
  dataUnit,
  setSelection,
  selection,
  ...
  const formattedData = gridData.map((item) => {
    ...
    cols.forEach((header) => {
      if (item[header.id] !== undefined) {
        if (header.id === 'WorkDate' || header.id === 'TestEndDate') {
    ...
    const [hiddenColumns, setHiddenColumns] = useState(() => {
        return loadFromLocalStorageSheet('H_ERP_COLS_PAGE_FINAL_BAD_QTY_RESULT_LIST', [])
    })
  ...
  const [keybindings, setKeybindings] = useState({
    ...
    const getData = useCallback(
      ...
      if (columnKey === 'ProgStatusName') {
        const value = person['ProgStatus'] || ''
        const ProgStatus = dataProgStatusName.find(
          (item) => item.Value === value,
        ) || { MinorName: '', Value: 0 }

        return {
          kind: GridCellKind.Text,
          data: ProgStatus.MinorName,
          copyData: String(ProgStatus.MinorName),
          displayData: String(ProgStatus.MinorName),
          readonly: column?.readonly || false,
          allowOverlay: true,
          hasMenu: column?.hasMenu || false,
        }
      ...
      if (
        ...
        columnKey === 'RemainQty' ||
        columnKey === 'ReWorkQty' ||
        columnKey === 'BreakOrderQty' ||
        columnKey === 'ReOrderQty'
      ) {
        return {
  ...
  const handleSort = (columnId, direction) => {
    ...
    const updateHiddenColumns = (newHiddenColumns) => {
        setHiddenColumns((prevHidden) => {
            ...
            saveToLocalStorageSheet('H_ERP_COLS_PAGE_FINAL_BAD_QTY_RESULT_LIST', newHidden)
            return newHidden
        })
    }
    ...
    const updateVisibleColumns = (newVisibleColumns) => {
        setCols((prevCols) => {
            ...
            saveToLocalStorageSheet('S_ERP_COLS_PAGE_FINAL_BAD_QTY_RESULT_LIST', uniqueCols)
            return uniqueCols
        })
    }
    // Hàm ẩn cột
    ...
    const handleReset = () => {
        ...
        setHiddenColumns([])
        localStorage.removeItem('S_ERP_COLS_PAGE_FINAL_BAD_QTY_RESULT_LIST')
        localStorage.removeItem('H_ERP_COLS_PAGE_FINAL_BAD_QTY_RESULT_LIST')
        setShowMenu(null);
    }
    ...
    const onColumnMoved = useCallback((startIndex, endIndex) => {
        setCols((prevCols) => {
            ...
            saveToLocalStorageSheet('S_ERP_COLS_PAGE_FINAL_BAD_QTY_RESULT_LIST', updatedCols)
            return updatedCols
        })
    }, [])
    ...
    const handleCheckboxChange = (columnId, isChecked) => {
        if (isChecked) {
            ...
            setCols((prevCols) => {
                ...
                saveToLocalStorageSheet('S_ERP_COLS_PAGE_FINAL_BAD_QTY_RESULT_LIST', newCols)
                return newCols
            })
            setHiddenColumns((prevHidden) => {
...
export default TableQcFinalBadQtyResultList
