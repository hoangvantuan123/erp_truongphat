...
function TableOqcReqList({
  ...
  const [hiddenColumns, setHiddenColumns] = useState(() => {
    return loadFromLocalStorageSheet('H_ERP_COLS_PAGE_OQC_PAGE', [])
  })
  ...
  const getData = useCallback(
    ([col, row]) => {
      ...
      const cellConfig = {
        IsGoodInNm: {
          kind: 'is-good-in-cell',
          allowedValues: dataIsGoodInNm,
        },
      }

      if (columnKey === 'IsGoodInNm') {
        const value = person['IsGoodIn'] || ''
        const IsGoodInNm = dataIsGoodInNm.find(
          (item) => item.Value === value,
        ) || { MinorName: '', Value: 0 }

        return {
          kind: GridCellKind.Text,
          data: IsGoodInNm.MinorName,
          copyData: String(IsGoodInNm.MinorName),
          displayData: String(IsGoodInNm.MinorName),
      ...
      if (
        ...
        columnKey === 'Qty' ||
        columnKey === 'OrderQty' ||
        columnKey === 'StdOrderQty' ||
        columnKey === 'StdQty' ||
        columnKey === 'StdOKQty' ||
        columnKey === 'RemainQty' ||
        columnKey === 'StdRemainQty' ||
        columnKey === 'StdBadQty' ||
        columnKey === 'GoodInQty' ||
        columnKey === 'StdGoodInQty'
      ) {
        return {
  ...
  const updateHiddenColumns = (newHiddenColumns) => {
    setHiddenColumns((prevHidden) => {
      ...
      saveToLocalStorageSheet('H_ERP_COLS_PAGE_OQC_PAGE', newHidden)
      return newHidden
    })
  }
  ...
  const updateVisibleColumns = (newVisibleColumns) => {
    setCols((prevCols) => {
      ...
      saveToLocalStorageSheet('S_ERP_COLS_PAGE_OQC_PAGE', uniqueCols)
      return uniqueCols
    })
  }
  // Hàm ẩn cột
  ...
  const handleReset = () => {
    ...
    setHiddenColumns([])
    localStorage.removeItem('S_ERP_COLS_PAGE_OQC_PAGE')
    localStorage.removeItem('H_ERP_COLS_PAGE_OQC_PAGE')
    setShowMenu(null)
  }
  ...
  const onColumnMoved = useCallback((startIndex, endIndex) => {
    setCols((prevCols) => {
      ...
      saveToLocalStorageSheet('S_ERP_COLS_PAGE_OQC_PAGE', updatedCols)
      return updatedCols
    })
  }, [])
  ...
  const handleCheckboxChange = (columnId, isChecked) => {
    if (isChecked) {
      ...
      setCols((prevCols) => {
        ...
        saveToLocalStorageSheet('S_ERP_COLS_PAGE_OQC_PAGE', newCols)
        return newCols
      })
      setHiddenColumns((prevHidden) => {
  ...
  const formattedData = gridData.map((item) => {
    ...
    cols.forEach((header) => {
      if (item[header.id] !== undefined) {
        if (header.id === 'QCDate' || header.id === ' DevlDate') {
...
export default TableOqcReqList
