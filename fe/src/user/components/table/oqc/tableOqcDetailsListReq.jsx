...
function TableOqcDetailsListReq({
  ...
  const columnNames = ['SMTestResultName', 'EmpName']
  ...
  const getData = useCallback(
    ([col, row]) => {
      ...
      if (columnKey === 'SMTestResultName') {
        const value = person['SMTestResult'] || ''
        const SMTestResult = dataSMTestResultName.find(
        ...
        return {
          kind: GridCellKind.Custom,
          allowOverlay: true,
          copyData: String(SMTestResult.MinorName),
          data: {
            kind: cellConfig[columnKey].kind,
            allowedValues: cellConfig[columnKey].allowedValues,
            value: SMTestResult.MinorName,
            boundingBox: boundingBox,
          },
          displayData: String(SMTestResult.MinorName),
          readonly: column?.readonly || false,
          hasMenu: column?.hasMenu || false,
        }
      }
      ...
      if (columnKey === 'WorkDate') {
        let WorkDate = value.trim() === '' ? formatDate(dayjs()) : value
        return {
          kind: GridCellKind.Text,
          data: WorkDate,
          copyData: String(WorkDate),
          displayData: dateFormat(WorkDate),
          readonly: column?.readonly || false,
          allowOverlay: true,
          hasMenu: column?.hasMenu || false,
        }
      ...
      if (columnKey === 'TestEndDate') {
        let TestEndDate = value.trim() === '' ? formatDate(dayjs()) : value
        return {
          kind: GridCellKind.Text,
          data: TestEndDate,
          copyData: String(TestEndDate),
          displayData: dateFormat(TestEndDate),
          readonly: column?.readonly || false,
          allowOverlay: true,
          hasMenu: column?.hasMenu || false,
        }
            ...
            if (selectedName) {
              if (product.SMTestResult === 6035006 ) {
                if (product.ReqQty < selectedName  || selectedName < 0){
                  ...
                  product['ReqInQty'] = product.ReqQty
                }else{
                  product['OKQty'] = selectedName
                  product['Qty'] = selectedName
                  product['ReqInQty'] = selectedName
          ...
          setGridData((prev) => {
            ...
            if (selectedName) {
              ...
              if (selectedName.Value === 6035003) {
                ...
                product['BadQty'] = product.ReqQty
                product['OKQty'] = 0
                product['ReqInQty'] = 0
              } else if (selectedName.Value === 6035006) {
                const qty = product.ReqQty - product.BadQty
                product['OKQty'] = qty
                product['ReqInQty'] = qty
              }
            } else {
...
export default TableOqcDetailsListReq
