...
import { PostDocs } from '../../../../features/upload/postDocs'
function Equip4Table({
    setSelection,
    selection,
    setShowSearch,
    showSearch,
    ...
    const [hiddenColumns, setHiddenColumns] = useState(() => {
        return loadFromLocalStorageSheet('asset_file_h', [])
    })
    ...
    const updateHiddenColumns = (newHiddenColumns) => {
        setHiddenColumns((prevHidden) => {
            ...
            saveToLocalStorageSheet('asset_file_h', newHidden)
            return newHidden
        })
    }
    ...
    const updateVisibleColumns = (newVisibleColumns) => {
        setCols((prevCols) => {
            ...
            saveToLocalStorageSheet('asset_file_a', uniqueCols)
            return uniqueCols
        })
    }
    ...
    const handleReset = () => {
        ...
        setHiddenColumns([])
        localStorage.removeItem('asset_file_a')
        localStorage.removeItem('asset_file_h')
    }
    ...
    const onColumnMoved = useCallback((startIndex, endIndex) => {
        setCols((prevCols) => {
            ...
            saveToLocalStorageSheet('asset_file_a', updatedCols)
            return updatedCols
        })
    }, [])
    ...
    const showDrawer = () => {
        ...
        const currentVisibleCols = loadFromLocalStorageSheet('asset_file_a', []).map(
            (col) => col.id
        )
    ...
    const handleCheckboxChange = (columnId, isChecked) => {
        if (isChecked) {
            ...
            setCols((prevCols) => {
                ...
                saveToLocalStorageSheet('asset_file_a', newCols)
                return newCols
            })
            setHiddenColumns((prevHidden) => {
    ...
    const handleFileChange = async (file) => {
        ...
        formData.append('Type', 'FILE_ASSET');

        setLoading(true);
        const result = await PostDocs(formData, ToolSeq);
        setLoading(false);
    ...
    const handelOpenDownload = () => {
        ...
        if (!ToolSeq) {
            return message.error('Vui lòng chọn tài sản!');
        }
        ...
        selectedFiles.forEach(file => {
            ...
            const relativePath = file.Path.replace('/ERP_CLOUD/asset_files/', '');

            const fileUrl = `${HOST_API_SERVER_PUBIC}/viewer/${relativePath}`;
