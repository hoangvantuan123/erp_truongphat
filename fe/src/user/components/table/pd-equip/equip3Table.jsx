...
import { getPdEquipRepairById } from '../../../../features/assetMng/getPdEquipRepairById'
const parseBoolean = (value) => {
  const normalized = String(value).trim().toLowerCase()
  return normalized === '1' || normalized === 'true'
}
function Equip3Table({
  setSelection,
  selection,
  setShowSearch,
  showSearch,
  ...
  const getData = useCallback(
    ([col, row]) => {
      ...
      if (
        columnKey === 'RepairDate' ||
        columnKey === 'NextQCDate' ||
        columnKey === 'HandiAppdate'
      ) {
        let displayValue = ''
        let dataValue = value
  ...
  useEffect(() => {
    ...
    fetchGenericData({
      controllerKey: 'getPdEquipRepairById',
      postFunction: getPdEquipRepairById,
