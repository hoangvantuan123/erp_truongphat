...
function Equip2Table({
  ...
  const [hiddenColumns, setHiddenColumns] = useState(() => {
    return loadFromLocalStorageSheet('eqp_mold_h', [])
  })
  ...
  const getData = useCallback(
    ([col, row]) => {
      ...
      if (columnKey === 'EntDate' || columnKey === 'EmpChnName') {
        let displayValue = ''
        let dataValue = value
  ...
  const updateHiddenColumns = (newHiddenColumns) => {
    setHiddenColumns((prevHidden) => {
      ...
      saveToLocalStorageSheet('eqp_mold_h', newHidden)
      return newHidden
    })
  }
  ...
  const updateVisibleColumns = (newVisibleColumns) => {
    setCols((prevCols) => {
      ...
      saveToLocalStorageSheet('eqp_mold_a', uniqueCols)
      return uniqueCols
    })
  }
  ...
  const handleReset = () => {
    ...
    setHiddenColumns([])
    localStorage.removeItem('eqp_mold_a')
    localStorage.removeItem('eqp_mold_h')
  }
  ...
  const onColumnMoved = useCallback((startIndex, endIndex) => {
    setCols((prevCols) => {
      ...
      saveToLocalStorageSheet('eqp_mold_a', updatedCols)
      return updatedCols
    })
  }, [])
  ...
  const showDrawer = () => {
    const invisibleCols = defaultCols
      ...
      .map((col) => col.id)
    const currentVisibleCols = loadFromLocalStorageSheet('eqp_mold_a', []).map(
      (col) => col.id,
    )
  ...
  const handleCheckboxChange = (columnId, isChecked) => {
    if (isChecked) {
      ...
      setCols((prevCols) => {
        ...
        saveToLocalStorageSheet('eqp_mold_a', newCols)
        return newCols
      })
      setHiddenColumns((prevHidden) => {
