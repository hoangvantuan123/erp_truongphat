...
import { CellsUMPayStatus } from '../../../sheet/cells/cellsUMPayStatus'
import { CellsUMPayCondName } from '../../../sheet/cells/cellsUMPayCond'
...
function PM01BTable({
    ...
    const [hiddenColumns, setHiddenColumns] = useState(() => {
        return loadFromLocalStorageSheet('pm01_b_h', [])
    })
    ...
    const totalColumns = ['PayAmt', 'DomPayAmt', 'PayVATAmt', 'DomPayVATAmt', 'DomSumPayAmt', 'SumPayAmt', 'SumAmt'];
    ...
    const getData = useCallback(
        ([col, row]) => {
            ...
            if (columnKey === "PayRate" ||
                columnKey === "PayAmt" ||
                columnKey === "DomPayAmt" ||
                columnKey === "PayVATAmt" ||
                columnKey === "Amt" ||
                columnKey === "SumPayAmt" ||
                columnKey === "DomSumPayAmt" ||
                columnKey === "DomPayVATAmt"
            ...
            if (
                columnKey === "PayPlanDate" ||
                columnKey === "PayRepeatDate"
            ) {
                const rawText = value?.toString?.() || "";
                const cleaned = rawText.replace(/[^\d]/g, "");
        ...
        [gridData, cols, numRows, helpData08, helpData09]
    );
    ...
    const onCellEdited = useCallback(
        async (cell, newValue) => {
            ...
            if (key === 'UMPayCondName') {
                if (newValue.kind === GridCellKind.Custom) {
                    setGridData(prev => {
                        ...
                        if (selectedName) {
                            product[key] = selectedName.MinorName;
                            product["UMPayCondSeq"] = selectedName.Value;
                        } else {
                            product[key] = "";
                            product["UMPayCondSeq"] = "";
                        }
            ...
            if (key === 'PayCondStatus') {
                if (newValue.kind === GridCellKind.Custom) {
                    setGridData(prev => {
                        ...
                        if (!selectedName && Array.isArray(helpData09)) {
                        ...
                        if (selectedName) {
                            product[key] = selectedName.MinorName;
                            product["PayCondStatusSeq"] = selectedName.Value;
                        } else {
                            product[key] = "";
                            product["PayCondStatusSeq"] = "";
                        }
            ...
            if (key === "PayAmt") {
                setGridData((prev) => {
                    ...
                    const payAmt = Number(newData[row]["PayAmt"]) || 0;
                    const payVATAmt = Number(newData[row]["PayVATAmt"]) || 0;

                    const sumPayAmt = parseFloat((payAmt + payVATAmt).toFixed(2));
                    newData[row]["DomPayAmt"] = payAmt;
                    newData[row]["SumPayAmt"] = sumPayAmt;
                    newData[row]["DomSumPayAmt"] = sumPayAmt;
            ...
            if (key === "PayVATAmt") {
                setGridData((prev) => {
                    ...
                    const sumPayAmt = parseFloat((payVATAmt + payAmt).toFixed(2));
                    newData[row]["DomPayVATAmt"] = payVATAmt;
                    newData[row]["SumPayAmt"] = sumPayAmt;
                    newData[row]["DomSumPayAmt"] = sumPayAmt;
        ...
        [cols, gridData, helpData08, helpData09]
    );
    ...
    const updateHiddenColumns = (newHiddenColumns) => {
        setHiddenColumns((prevHidden) => {
            ...
            saveToLocalStorageSheet('pm01_b_h', newHidden)
            return newHidden
        })
    }
    ...
    const updateVisibleColumns = (newVisibleColumns) => {
        setCols((prevCols) => {
            ...
            saveToLocalStorageSheet('pm01_b_a', uniqueCols)
            return uniqueCols
        })
    }
    ...
    const handleReset = () => {
        ...
        setHiddenColumns([])
        localStorage.removeItem('pm01_b_a')
        localStorage.removeItem('pm01_b_h')
    }
    ...
    const onColumnMoved = useCallback((startIndex, endIndex) => {
        setCols((prevCols) => {
            ...
            saveToLocalStorageSheet('pm01_b_a', updatedCols)
            return updatedCols
        })
    }, [])
    ...
    const showDrawer = () => {
        ...
        const currentVisibleCols = loadFromLocalStorageSheet('pm01_b_a', []).map(
            (col) => col.id
        )
    ...
    const handleCheckboxChange = (columnId, isChecked) => {
        if (isChecked) {
            ...
            setCols((prevCols) => {
                ...
                saveToLocalStorageSheet('pm01_b_a', newCols)
                return newCols
            })
            setHiddenColumns((prevHidden) => {
        ...
        <div className="w-full gap-1 h-full flex items-center justify-center">
            <div className="w-full h-full flex flex-col  bg-white  overflow-hidden ">
                <h2 className="text-[10px]  text-blue-600 border-b font-medium flex items-center gap-2 p-1  uppercase">
                    {t('DANH SÁCH ĐIỀU KIỆN THANH TOÁN')}
                </h2>
                <DataEditor
                    {...cellProps}
                    ref={gridRef}
                    ...
                    customRenderers={[CellsUMPayCondName, CellsUMPayStatus]}
