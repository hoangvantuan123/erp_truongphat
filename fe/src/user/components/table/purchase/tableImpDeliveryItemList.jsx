...
function TableImpDeliveryItemList({
  data,
  setSelection,
  selection,
  setShowSearch,
  ...
  const [hiddenColumns, setHiddenColumns] = useState(
    loadFromLocalStorageSheet('H_ERP_COLS_PAGE_IMP_DELIVERY_ITEM_LIST', []),
  )
  ...
  const getData = useCallback(
    ([col, row]) => {
      ...
      if (columnKey === 'IsCostCalc' || columnKey === 'IsCostSlip') {
        const booleanValue =
          value === 1 || value === '1'
            ? true
            : value === 0 || value === '0'
  ...
  const updateHiddenColumns = (newHiddenColumns) => {
    setHiddenColumns((prevHidden) => {
      ...
      saveToLocalStorageSheet(
        'H_ERP_COLS_PAGE_IMP_DELIVERY_ITEM_LIST',
        newHidden,
      )
      return newHidden
    })
  ...
  const handleReset = () => {
    ...
    setHiddenColumns([])
    localStorage.removeItem('S_ERP_COLS_PAGE_IMP_DELIVERY_ITEM_LIST')
    localStorage.removeItem('H_ERP_COLS_PAGE_IMP_DELIVERY_ITEM_LIST')
  }
  ...
  const formattedData = gridData.map((item) => {
    ...
    visibleHeaders.forEach((header) => {
      ...
      if (item[header.id] !== undefined) {
        ...
        if (header.id === 'DelvDate' || header.id === 'CfmDate') {
...
export default TableImpDeliveryItemList
