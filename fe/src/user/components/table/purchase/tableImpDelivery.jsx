...
function TableImpDelivery({
  ...
  const [hiddenColumns, setHiddenColumns] = useState(
    loadFromLocalStorageSheet('H_ERP_COLS_PAGE_IMP_DELIVERY', []),
  )
  ...
  const updateHiddenColumns = (newHiddenColumns) => {
    setHiddenColumns((prevHidden) => {
      ...
      saveToLocalStorageSheet('H_ERP_COLS_PAGE_IMP_DELIVERY', newHidden)
      return newHidden
    })
  }
  ...
  const updateVisibleColumns = (newVisibleColumns) => {
    setCols((prevCols) => {
      ...
      saveToLocalStorageSheet('S_ERP_COLS_PAGE_IMP_DELIVERY', uniqueCols)
      return uniqueCols
    })
  }
  ...
  const handleReset = () => {
    ...
    setHiddenColumns([])
    localStorage.removeItem('S_ERP_COLS_PAGE_IMP_DELIVERY')
    localStorage.removeItem('H_ERP_COLS_PAGE_IMP_DELIVERY')
  }
  ...
  const onColumnMoved = useCallback((startIndex, endIndex) => {
    setCols((prevCols) => {
      ...
      saveToLocalStorageSheet('S_ERP_COLS_PAGE_IMP_DELIVERY', updatedCols)
      return updatedCols
    })
  }, [])
  ...
  const handleCheckboxChange = (columnId, isChecked) => {
    if (isChecked) {
      ...
      setCols((prevCols) => {
        ...
        saveToLocalStorageSheet('S_ERP_COLS_PAGE_IMP_DELIVERY', newCols)
        return newCols
      })
      setHiddenColumns((prevHidden) => {
  ...
  const findRowByItemNo = useCallback(
    (itemNo, permitSerl) =>
      gridData.findIndex(
        (row) => row.ItemNo === itemNo && row.PermitSerl === permitSerl,
      ),
    [gridData],
  )
  const highlightRowByItemNo = useCallback(
    (itemNo) => {
      const rowIndex = findRowByItemNo(itemNo, permitSerl)
      if (rowIndex !== -1) {
        // Use -1 for not found
        setHighlightRegions([
          {
  ...
  useEffect(() => {
    highlightRowByItemNo(inputItemNo, permitSerl)
  }, [inputItemNo, permitSerl, highlightRowByItemNo])
  ...
  return (
    <div className="w-full gap-1 h-full flex items-center justify-center pb-[23px]  overflow-hidden">
...
export default TableImpDelivery
