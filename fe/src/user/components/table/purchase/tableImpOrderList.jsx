...
function TableImpOrderList({
  ...
  const [hiddenColumns, setHiddenColumns] = useState(
    loadFromLocalStorageSheet('H_ERP_COLS_PAGE_IMP_ORDER_LIST', []),
  )
  ...
  const getData = useCallback(
    ([col, row]) => {
      ...
      if (columnKey === 'Confirm' || columnKey === 'IsStop') {
        const booleanValue =
          value === 1 || value === '1'
            ? true
            : value === 0 || value === '0'
  ...
  const onCellEdited = useCallback(
    async (cell, newValue) => {
      ...
      if (key === 'Confirm') {
        console.log('gridData[row]', gridData[row])
        if (gridData[row].Confirm === 0) {
          setReqNoTitle(`Xác nhận đơn yêu cầu số ${gridData[row].PONo}`)
          setPOSeq(gridData[row].POSeq)
          setPONo(gridData[row].PONo)
          setRowIndex(row)
          setIsConfirm('1')
        } else if (gridData[row].Confirm === 1) {
          setReqNoTitle(`Hủy xác nhận đơn yêu cầu số ${gridData[row].PONo}`)
          setPOSeq(gridData[row].POSeq)
          setPONo(gridData[row].PONo)
          setRowIndex(row)
          setIsConfirm('0')
        }
        // console.log('key', gridData[row].Confirm)
        // setPOSeq(gridData[row].POSeq)
        // setPONo(gridData[row].PONo)
        // setRowIndex(row)
        // setIsConfirm('1')
      ...
      if (key === 'IsStop') {
        if (gridData[row].IsStop === '0') {
          setReqNoTitle(`Xác nhận đơn dừng yêu cầu số ${gridData[row].PONo}`)
          setPOSeq(gridData[row].POSeq)
          setPONo(gridData[row].PONo)
          setRowIndex(row)
          setIsStop('1')
        } else if (gridData[row].IsStop === '1') {
          setReqNoTitle(
            `Hủy xác nhận dừng đơn yêu cầu số ${gridData[row].PONo}`,
          )
          setPOSeq(gridData[row].POSeq)
          setPONo(gridData[row].PONo)
          setRowIndex(row)
  ...
  const updateHiddenColumns = (newHiddenColumns) => {
    setHiddenColumns((prevHidden) => {
      ...
      saveToLocalStorageSheet('H_ERP_COLS_PAGE_IMP_ORDER_LIST', newHidden)
      return newHidden
    })
  }
  ...
  const updateVisibleColumns = (newVisibleColumns) => {
    setCols((prevCols) => {
      ...
      saveToLocalStorageSheet('S_ERP_COLS_PAGE_IMP_ORDER_LIST', uniqueCols)
      return uniqueCols
    })
  }
  ...
  const handleReset = () => {
    ...
    setHiddenColumns([])
    localStorage.removeItem('S_ERP_COLS_PAGE_IMP_ORDER_LIST')
    localStorage.removeItem('H_ERP_COLS_PAGE_IMP_ORDER_LIST')
  }
  ...
  const onColumnMoved = useCallback((startIndex, endIndex) => {
    setCols((prevCols) => {
      ...
      saveToLocalStorageSheet('S_ERP_COLS_PAGE_IMP_ORDER_LIST', updatedCols)
      return updatedCols
    })
  }, [])
  ...
  const showDrawer = () => {
    ...
    const currentVisibleCols = loadFromLocalStorageSheet(
      'S_ERP_COLS_PAGE_IMP_ORDER_LIST',
      [],
    ).map((col) => col.id)
  ...
  const handleCheckboxChange = (columnId, isChecked) => {
    if (isChecked) {
      ...
      setCols((prevCols) => {
        ...
        saveToLocalStorageSheet('S_ERP_COLS_PAGE_IMP_ORDER_LIST', newCols)
        return newCols
      })
      setHiddenColumns((prevHidden) => {
  ...
  return (
    <div className="w-full gap-1 h-full flex items-center justify-center pb-[28px] overflow-hidden">
...
export default TableImpOrderList
