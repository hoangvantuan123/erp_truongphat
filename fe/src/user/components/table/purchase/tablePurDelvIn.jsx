...
function TablePurDelvIn({
  ...
  const [hiddenColumns, setHiddenColumns] = useState(
    loadFromLocalStorageSheet('H_ERP_COLS_PAGE_PUR_DELVIN', []),
  )
  ...
  const updateHiddenColumns = (newHiddenColumns) => {
    setHiddenColumns((prevHidden) => {
      ...
      saveToLocalStorageSheet('H_ERP_COLS_PAGE_PUR_DELVIN', newHidden)
      return newHidden
    })
  }
  ...
  const updateVisibleColumns = (newVisibleColumns) => {
    setCols((prevCols) => {
      ...
      saveToLocalStorageSheet('S_ERP_COLS_PAGE_PUR_DELVIN', uniqueCols)
      return uniqueCols
    })
  }
  ...
  const handleReset = () => {
    ...
    setHiddenColumns([])
    localStorage.removeItem('S_ERP_COLS_PAGE_PUR_DELVIN')
    localStorage.removeItem('H_ERP_COLS_PAGE_PUR_DELVIN')
  }
  ...
  const onColumnMoved = useCallback((startIndex, endIndex) => {
    setCols((prevCols) => {
      ...
      saveToLocalStorageSheet('S_ERP_COLS_PAGE_PUR_DELVIN', updatedCols)
      return updatedCols
    })
  }, [])
  ...
  const handleCheckboxChange = (columnId, isChecked) => {
    if (isChecked) {
      ...
      setCols((prevCols) => {
        ...
        saveToLocalStorageSheet('S_ERP_COLS_PAGE_PUR_DELVIN', newCols)
        return newCols
      })
      setHiddenColumns((prevHidden) => {
  ...
  const findRowByItemNo = useCallback(
    (itemNo, delvSerl) =>
      gridData.findIndex(
        (row) => row.ItemNo === itemNo && row.DelvSerl === delvSerl,
      ),
    [gridData],
  )
  const highlightRowByItemNo = useCallback(
    (itemNo) => {
      const rowIndex = findRowByItemNo(itemNo, delvSerl)
      if (rowIndex !== -1) {
        // Use -1 for not found
        setHighlightRegions([
          {
  ...
  useEffect(() => {
    highlightRowByItemNo(inputItemNo, delvSerl)
  }, [inputItemNo, delvSerl, highlightRowByItemNo])
