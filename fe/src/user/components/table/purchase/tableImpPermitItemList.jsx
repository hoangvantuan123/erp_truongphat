...
function TableImpPermitItemList({
  setSelection,
  selection,
  setShowSearch,
  showSearch,
  ...
  const [hiddenColumns, setHiddenColumns] = useState(
    loadFromLocalStorageSheet('H_ERP_COLS_PAGE_IMP_PERMIT_ITEM_LIST', []),
  )
  ...
  const updateHiddenColumns = (newHiddenColumns) => {
    setHiddenColumns((prevHidden) => {
      ...
      saveToLocalStorageSheet('H_ERP_COLS_PAGE_IMP_PERMIT_ITEM_LIST', newHidden)
      return newHidden
    })
  }
  ...
  const handleReset = () => {
    ...
    setHiddenColumns([])
    localStorage.removeItem('S_ERP_COLS_PAGE_IMP_PERMIT_ITEM_LIST')
    localStorage.removeItem('H_ERP_COLS_PAGE_IMP_PERMIT_ITEM_LIST')
  }
  ...
  const handleCheckboxChange = (columnId, isChecked) => {
    if (isChecked) {
      ...
      setCols((prevCols) => {
        ...
        saveToLocalStorageSheet('S_ERP_COLS_PAGE_IMP_PERMIT_ITEM_LIST', newCols)
        return newCols
      })
      setHiddenColumns((prevHidden) => {
        const newHidden = prevHidden.filter((id) => id !== columnId)
        saveToLocalStorageSheet(
          'H_ERP_COLS_PAGE_IMP_PERMIT_ITEM_LIST',
          newHidden,
        )
        return newHidden
      })
  ...
  const formattedData = gridData.map((item) => {
    ...
    visibleHeaders.forEach((header) => {
      ...
      if (item[header.id] !== undefined) {
        ...
        if (header.id === 'PermitDate' || header.id === 'CfmDate') {
...
export default TableImpPermitItemList
