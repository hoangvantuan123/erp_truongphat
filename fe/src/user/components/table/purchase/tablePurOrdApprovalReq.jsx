...
function TablePurOrdApprovalReq({
  ...
  const [hiddenColumns, setHiddenColumns] = useState(() => {
    return loadFromLocalStorageSheet('H_ERP_COLS_PAGE_PUR_ORD_APPROVAL_REQ', [])
  })
  ...
  const highlightRegions = [
    ...
    },
    {
      color: '#e8f0ff',
      range: {
        x: reorderColumns(cols).indexOf('CurrName'),
        y: 0,
        width: 1,
        height: numRows,
      },
  ...
  const getData = useCallback(
    ...
    [gridData, cols, dataItemName, dataPaymentType, dataExRateCurr],
  )
    ...
    async (cell, newValue) => {
      ...
      if (key === 'CurrName') {
        if (newValue.kind === GridCellKind.Custom) {
          setGridData((prev) => {
            ...
            if (selectedName) {
              product[cols[col].id] = selectedName.CurrName
              product['CurrSeq'] = selectedName.CurrSeq
              product['ExRate'] = selectedName.RateBuy
              product['BasicAmt'] = selectedName.BasicAmt
            } else {
              product[cols[col].id] = ''
              product['CurrSeq'] = ''
              product['ExRate'] = ''
      ...
      setGridData((prevData) => {
        ...
        updatedData[row]['DomPrice'] =
          (updatedData[row]['Price'] * updatedData[row]['ExRate']) /
          updatedData[row]['BasicAmt']
  ...
  const updateHiddenColumns = (newHiddenColumns) => {
    setHiddenColumns((prevHidden) => {
      ...
      saveToLocalStorageSheet('H_ERP_COLS_PAGE_PUR_ORD_APPROVAL_REQ', newHidden)
      return newHidden
    })
  }
  ...
  const handleReset = () => {
    ...
    setHiddenColumns([])
    localStorage.removeItem('S_ERP_COLS_PAGE_PUR_ORD_APPROVAL_REQ')
    localStorage.removeItem('H_ERP_COLS_PAGE_PUR_ORD_APPROVAL_REQ')
  }
  ...
  const handleCheckboxChange = (columnId, isChecked) => {
    if (isChecked) {
      ...
      setCols((prevCols) => {
        ...
        saveToLocalStorageSheet('S_ERP_COLS_PAGE_PUR_ORD_APPROVAL_REQ', newCols)
        return newCols
      })
      setHiddenColumns((prevHidden) => {
        const newHidden = prevHidden.filter((id) => id !== columnId)
        saveToLocalStorageSheet(
          'H_ERP_COLS_PAGE_PUR_ORD_APPROVAL_REQ',
          newHidden,
        )
        return newHidden
      })
      ...
      <div className="flex-1  flex items-center justify-center">
        <div className="w-full h-full flex flex-col">
          ...
          <DataEditor
            ...
            onKeyUp={onKeyUp}
            customRenderers={[CellsItemName, CellsSMPayType, CellsCurrName]}
          />{' '}
          {/* * */}
          {showMenu !== null &&
            renderLayer(
...
export default TablePurOrdApprovalReq
