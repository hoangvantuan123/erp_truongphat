...
function TablePurDelv({
  ...
  const [hiddenColumns, setHiddenColumns] = useState(() => {
    return loadFromLocalStorageSheet('H_ERP_COLS_PAGE_PUR_DELV', [])
  })
  ...
  const onCellEdited = useCallback(
    async (cell, newValue) => {
      ...
      setGridData((prevData) => {
        ...
        if (
          key === 'Qty' &&
          updatedData[row]['Qty'] > updatedData[row]['POQty']
        ) {
          message.warning('Số lượng không được lớn hơn số lượng đặt hàng')
          updatedData[row]['Qty'] = updatedData[row]['POQty']
        }
  ...
  const updateHiddenColumns = (newHiddenColumns) => {
    setHiddenColumns((prevHidden) => {
      ...
      saveToLocalStorageSheet('H_ERP_COLS_PAGE_PUR_DELV', newHidden)
      return newHidden
    })
  }
  ...
  const updateVisibleColumns = (newVisibleColumns) => {
    setCols((prevCols) => {
      ...
      saveToLocalStorageSheet('S_ERP_COLS_PAGE_PUR_DELV', uniqueCols)
      return uniqueCols
    })
  }
  // Hàm ẩn cột
  ...
  const handleReset = () => {
    ...
    setHiddenColumns([])
    localStorage.removeItem('S_ERP_COLS_PAGE_PUR_DELV')
    localStorage.removeItem('H_ERP_COLS_PAGE_PUR_DELV')
  }
  ...
  const onColumnMoved = useCallback((startIndex, endIndex) => {
    setCols((prevCols) => {
      ...
      saveToLocalStorageSheet('S_ERP_COLS_PAGE_PUR_DELV', updatedCols)
      return updatedCols
    })
  }, [])
  ...
  const handleCheckboxChange = (columnId, isChecked) => {
    if (isChecked) {
      ...
      setCols((prevCols) => {
        ...
        saveToLocalStorageSheet('S_ERP_COLS_PAGE_PUR_DELV', newCols)
        return newCols
      })
      setHiddenColumns((prevHidden) => {
