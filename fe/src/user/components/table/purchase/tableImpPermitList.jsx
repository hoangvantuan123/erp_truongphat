...
function TableImpPermitList({
  ...
  const [hiddenColumns, setHiddenColumns] = useState(
    loadFromLocalStorageSheet('H_ERP_COLS_PAGE_IMP_PERMIT_LIST', []),
  )
  ...
  const getData = useCallback(
    ([col, row]) => {
      ...
      if (columnKey === 'IsExpended' || columnKey === 'IsStop') {
        const booleanValue =
          value === 1 || value === '1'
            ? true
            : value === 0 || value === '0'
      ...
      if (columnKey === 'PermitDate' || columnKey === 'AcceptDate') {
    ...
    async (cell, newValue) => {
      ...
      if (key === 'IsStop') {
        if (gridData[row].IsStop === '0') {
          setPermitRefNoTitle(
            `Xác nhận đơn dừng tờ khai số ${gridData[row].PermitRefNo}`,
          )
          setPermitSeq(gridData[row].PermitSeq)
          setPermitRefNo(gridData[row].PermitRefNo)
          setRowIndex(row)
          setIsStop('1')
        } else if (gridData[row].IsStop === '1') {
          setPermitRefNoTitle(
            `Hủy xác nhận dừng đơn tờ khai số ${gridData[row].PermitRefNo}`,
          )
          setPermitSeq(gridData[row].PermitSeq)
          setPermitRefNo(gridData[row].PermitRefNo)
          setRowIndex(row)
  ...
  const updateHiddenColumns = (newHiddenColumns) => {
    setHiddenColumns((prevHidden) => {
      ...
      saveToLocalStorageSheet('H_ERP_COLS_PAGE_IMP_PERMIT_LIST', newHidden)
      return newHidden
    })
  }
  ...
  const updateVisibleColumns = (newVisibleColumns) => {
    setCols((prevCols) => {
      ...
      saveToLocalStorageSheet('S_ERP_COLS_PAGE_IMP_PERMIT_LIST', uniqueCols)
      return uniqueCols
    })
  }
  ...
  const handleReset = () => {
    ...
    setHiddenColumns([])
    localStorage.removeItem('S_ERP_COLS_PAGE_IMP_PERMIT_LIST')
    localStorage.removeItem('H_ERP_COLS_PAGE_IMP_PERMIT_LIST')
  }
  ...
  const onColumnMoved = useCallback((startIndex, endIndex) => {
    setCols((prevCols) => {
      ...
      saveToLocalStorageSheet('S_ERP_COLS_PAGE_IMP_PERMIT_LIST', updatedCols)
      return updatedCols
    })
  }, [])
  ...
  const handleCheckboxChange = (columnId, isChecked) => {
    if (isChecked) {
      ...
      setCols((prevCols) => {
        ...
        saveToLocalStorageSheet('S_ERP_COLS_PAGE_IMP_PERMIT_LIST', newCols)
        return newCols
      })
      setHiddenColumns((prevHidden) => {
  ...
  const formattedData = gridData.map((item) => {
    ...
    visibleHeaders.forEach((header) => {
      ...
      if (item[header.id] !== undefined) {
        ...
        if (header.id === 'PermitDate' || header.id === 'AcceptDate') {
...
export default TableImpPermitList
