...
function TablePurOrdApprovalReqList({
  setSelection,
  selection,
  setShowSearch,
  showSearch,
  ...
  const [hiddenColumns, setHiddenColumns] = useState(
    loadFromLocalStorageSheet('H_ERP_COLS_PAGE_ORD_APPROVAL_REQ_LIST', []),
  )
    ...
    async (cell, newValue) => {
      ...
      if (key === 'Confirm') {
        if (gridData[row].Confirm === 0) {
          setReqNoTitle(`Xác nhận đơn yêu cầu số ${gridData[row].ApproReqNo}`)
          setApproReqSeq(gridData[row].ApproReqSeq)
          setApproReqNo(gridData[row].ApproReqNo)
          setRowIndex(row)
          setIsConfirm('1')
        } else if (gridData[row].Confirm === 1) {
          setReqNoTitle(
            `Hủy xác nhận đơn yêu cầu số ${gridData[row].ApproReqNo}`,
          )
          setApproReqSeq(gridData[row].ApproReqSeq)
          setApproReqNo(gridData[row].ApproReqNo)
          setRowIndex(row)
      ...
      if (key === 'IsStop') {
        if (gridData[row].IsStop === '0') {
          setReqNoTitle(
            `Xác nhận đơn dừng yêu cầu số ${gridData[row].ApproReqNo}`,
          )
          setApproReqSeq(gridData[row].ApproReqSeq)
          setApproReqNo(gridData[row].ApproReqNo)
          setRowIndex(row)
          ...
          setReqNoTitle(
            `Hủy xác nhận dừng đơn yêu cầu số ${gridData[row].ApproReqNo}`,
          )
          setApproReqSeq(gridData[row].ApproReqSeq)
          setApproReqNo(gridData[row].ApproReqNo)
          setRowIndex(row)
  ...
  const updateHiddenColumns = (newHiddenColumns) => {
    setHiddenColumns((prevHidden) => {
      ...
      saveToLocalStorageSheet(
        'H_ERP_COLS_PAGE_ORD_APPROVAL_REQ_LIST',
        newHidden,
      )
      return newHidden
    })
  ...
  const handleReset = () => {
    ...
    setHiddenColumns([])
    localStorage.removeItem('S_ERP_COLS_PAGE_ORD_APPROVAL_REQ_LIST')
    localStorage.removeItem('H_ERP_COLS_PAGE_ORD_APPROVAL_REQ_LIST')
  }
...
export default TablePurOrdApprovalReqList
