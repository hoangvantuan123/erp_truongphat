...
function PjtProjectItemTable({
  ...
  const [hiddenColumns, setHiddenColumns] = useState(() => {
    return loadFromLocalStorageSheet('edu_rst_item_h', [])
  })
  ...
  const highlightRegions = ['SMDelvTypeName', 'ItemName']
    .map((columnName) => ({
      color: '#E6F4FF',
      range: {
  ...
  const getData = useCallback(
    ([col, row]) => {
      ...
      const cellConfig = {
        ...
        SMDelvTypeName: {
          kind: 'cells-sm-combo',
          allowedValues: dataSmDelvType,
          setCacheData: setDataSmDelvType,
        },
      }
      ...
      if (
        ...
        columnKey === 'DelvPrice' ||
        columnKey === 'DelvAmt' ||
        columnKey === 'DelvVatAmt' ||
        columnKey === 'SumDelvAmt' ||
        columnKey === 'DelvDomPrice' ||
        columnKey === 'DelvDomAmt' ||
        columnKey === 'SumDelvDomAmt'
      ) {
        const isEmptyValue =
      ...
      if (columnKey === 'DelvExpectDate' || columnKey === 'ChangeDeliveyDate') {
        let displayValue = ''
        let dataValue = value
    ...
    [gridData, cols, dataItem, dataSmDelvType],
  )
  ...
  const updateHiddenColumns = (newHiddenColumns) => {
    setHiddenColumns((prevHidden) => {
      ...
      saveToLocalStorageSheet('edu_rst_item_h', newHidden)
      return newHidden
    })
  }
  ...
  const updateVisibleColumns = (newVisibleColumns) => {
    setCols((prevCols) => {
      ...
      saveToLocalStorageSheet('edu_rst_item_a', uniqueCols)
      return uniqueCols
    })
  }
  ...
  const handleReset = () => {
    ...
    setHiddenColumns([])
    localStorage.removeItem('edu_rst_item_a')
    localStorage.removeItem('edu_rst_item_h')
  }
  ...
  const onColumnMoved = useCallback((startIndex, endIndex) => {
    setCols((prevCols) => {
      ...
      saveToLocalStorageSheet('edu_rst_item_a', updatedCols)
      return updatedCols
    })
  }, [])
  ...
  const handleCheckboxChange = (columnId, isChecked) => {
    if (isChecked) {
      ...
      setCols((prevCols) => {
        ...
        saveToLocalStorageSheet('edu_rst_item_a', newCols)
        return newCols
      })
      setHiddenColumns((prevHidden) => {
      ...
      if (key === 'SMDelvTypeName') {
        if (newValue.kind === GridCellKind.Custom) {
          setGridData((prev) => {
            ...
            if (!selectedName) {
              selectedName = dataSmDelvType.find(
                (item) => item.MinorName === checkCopyData,
              )
            }
            if (selectedName) {
              product[cols[col].id] = selectedName.MinorName
              product['SMDelvTypeName'] = selectedName.MinorName
              product['SMDelvType'] = selectedName.Value
            } else {
              product[cols[col].id] = ''
              product['SMDelvTypeName'] = ''
              product['SMDelvType'] = ''
            }
        ...
        if (newValue.kind === GridCellKind.Number) {
          setGridData((prev) => {
            ...
            if (selectedName) {
              ...
              const delvAmt = parseFloat(selectedName * product['DelvPrice'])
              const delvVatAmt = parseFloat(delvAmt * 0.1)
              const sumDelvAmt = parseFloat(delvAmt + delvVatAmt)

              product['DelvQty'] = selectedName
              product['DelvVatAmt'] = delvVatAmt // Tien VAT
              product['DelvAmt'] = delvAmt
              product['SumDelvAmt'] = sumDelvAmt
              product['SumDelvDomAmt'] = sumDelvAmt
              product['DelvDomAmt'] = delvAmt
            } else {
              product[cols[col].id] = ''
              product['DelvQty'] = ''
              product['DelvVatAmt'] = ''
              product['SumDelvAmt'] = ''
              product['SumDelvDomAmt'] = ''
              product['DelvDomAmt'] = ''
            }
      ...
      if (key === 'DelvPrice') {
        if (newValue.kind === GridCellKind.Number) {
          setGridData((prev) => {
            ...
            if (selectedName) {
              product[cols[col].id] = selectedName
              const delvAmt = parseFloat(selectedName * product['DelvQty'])
              const delvVatAmt = parseFloat(delvAmt * 0.1)
              const sumDelvAmt = parseFloat(delvAmt + delvVatAmt)

              product['DelvPrice'] = selectedName
              product['DelvDomPrice'] = selectedName
              product['DelvVatAmt'] = delvVatAmt // Tien VAT
              product['DelvAmt'] = delvAmt
              product['SumDelvAmt'] = sumDelvAmt
              product['SumDelvDomAmt'] = sumDelvAmt
...
export default PjtProjectItemTable
