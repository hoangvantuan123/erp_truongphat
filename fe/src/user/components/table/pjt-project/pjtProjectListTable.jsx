...
import { CellsCostItem } from '../../sheet/cells/cellsCostItemName'
import moment from 'moment'

function PjtProjectListTable({
  dataCostItem,
  setDataCostItem,
  setSelection,
  selection,
  ...
  const [hiddenColumns, setHiddenColumns] = useState(() => {
    return loadFromLocalStorageSheet('pjt_project_list_h', [])
  })
  ...
  const columnNames = ['UMCostItemName']
  ...
  const getData = useCallback(
    ([col, row]) => {
      ...
      const cellConfig = {
        UMCostItemName: {
          kind: 'cells-cost-item',
          allowedValues: dataCostItem,
          setCacheData: setDataCostItem,
        },
      }

      if (
        columnKey === 'IsReturnItem' ||
        columnKey === 'IsInsur' ||
        columnKey === 'IsVAT'
      ) {
        const booleanValue =
      ...
      if (
        columnKey === 'PlanFrDate' ||
        columnKey === 'PlanToDate' ||
        columnKey === 'ContractFrDate' ||
        columnKey === 'ContractToDate' ||
        columnKey === 'QryFr' ||
        columnKey === 'QryTo' ||
        columnKey === 'PJTCfmDate' ||
        columnKey === 'LastDate'
      ) {
        let displayValue = ''
        let dataValue = value
  ...
  const updateHiddenColumns = (newHiddenColumns) => {
    setHiddenColumns((prevHidden) => {
      ...
      saveToLocalStorageSheet('pjt_project_list_h', newHidden)
      return newHidden
    })
  }
  ...
  const updateVisibleColumns = (newVisibleColumns) => {
    setCols((prevCols) => {
      ...
      saveToLocalStorageSheet('pjt_project_list_a', uniqueCols)
      return uniqueCols
    })
  }
  ...
  const handleReset = () => {
    ...
    setHiddenColumns([])
    localStorage.removeItem('pjt_project_list_a')
    localStorage.removeItem('pjt_project_list_h')
  }
  ...
  const onColumnMoved = useCallback((startIndex, endIndex) => {
    setCols((prevCols) => {
      ...
      saveToLocalStorageSheet('pjt_project_list_a', updatedCols)
      return updatedCols
    })
  }, [])
  ...
  const handleCheckboxChange = (columnId, isChecked) => {
    if (isChecked) {
      ...
      setCols((prevCols) => {
        ...
        saveToLocalStorageSheet('pjt_project_list_a', newCols)
        return newCols
      })
      setHiddenColumns((prevHidden) => {
      ...
      if (key === 'UMCostItemName') {
        if (newValue.kind === GridCellKind.Custom) {
          setGridData((prev) => {
            ...
            if (!selectedName) {
              selectedName = dataCostItem.find(
                (item) => item.UMCostItemName === checkCopyData,
              )
            }
            if (selectedName) {
              product[cols[col].id] = selectedName.UMCostItemName
              product['UMCostItemName'] = selectedName.UMCostItemName
              product['UMCostItem'] = selectedName.UMCostItem
              product['IsReturnItem'] = selectedName.IsReturnItem
            } else {
              product[cols[col].id] = ''
              product['UMCostItemName'] = ''
              product['UMCostItem'] = ''
              product['IsReturnItem'] = ''
            }
...
export default PjtProjectListTable
