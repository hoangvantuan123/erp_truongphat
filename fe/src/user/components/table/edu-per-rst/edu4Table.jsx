...
function Edu4Table({
  ...
  const [hiddenColumns, setHiddenColumns] = useState(() => {
    return loadFromLocalStorageSheet('edu_rst_emp_h', [])
  })
  ...
  const highlightRegions = ['EmpName']
    .map((columnName) => ({
      color: '#E6F4FF',
      range: {
      ...
      if (columnKey === 'EmpName') {
        return {
          ...
          data: {
            ...
            allowedValues: helpData,
            setCacheData: setHelpData,
            value: value,
            ...
            onFinishedEditing: (newCell) => {
              setGridData((prev) => {
                ...
                product['EmpName'] = newCell?.data[0]?.EmpName
                product['EmpSeq'] = newCell?.data[0]?.EmpSeq
                return newData
              })
            },
          },
  ...
  const updateHiddenColumns = (newHiddenColumns) => {
    setHiddenColumns((prevHidden) => {
      ...
      saveToLocalStorageSheet('edu_rst_emp_h', newHidden)
      return newHidden
    })
  }
  ...
  const updateVisibleColumns = (newVisibleColumns) => {
    setCols((prevCols) => {
      ...
      saveToLocalStorageSheet('edu_rst_emp_a', uniqueCols)
      return uniqueCols
    })
  }
  ...
  const handleReset = () => {
    ...
    setHiddenColumns([])
    localStorage.removeItem('edu_rst_emp_a')
    localStorage.removeItem('edu_rst_emp_h')
  }
  ...
  const onColumnMoved = useCallback((startIndex, endIndex) => {
    setCols((prevCols) => {
      ...
      saveToLocalStorageSheet('edu_rst_emp_a', updatedCols)
      return updatedCols
    })
  }, [])
  ...
  const showDrawer = () => {
    const invisibleCols = defaultCols
      ...
      .map((col) => col.id)
    const currentVisibleCols = loadFromLocalStorageSheet('edu_rst_emp_a', []).map(
      (col) => col.id,
    )
  ...
  const handleCheckboxChange = (columnId, isChecked) => {
    if (isChecked) {
      ...
      setCols((prevCols) => {
        ...
        saveToLocalStorageSheet('edu_rst_emp_a', newCols)
        return newCols
      })
      setHiddenColumns((prevHidden) => {
    ...
    <div className="w-full gap-1 h-full flex items-center justify-center">
      <div className="w-full h-full flex flex-col  bg-white  overflow-hidden ">
        ...
        <DataEditor
          ...
          customRenderers={[CellsSMAddressType, CellsEmpNameV2]}
