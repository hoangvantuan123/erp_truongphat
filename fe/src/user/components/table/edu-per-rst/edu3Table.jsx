...
function Edu3Table({
  ...
  const highlightRegions = ['SMBirthTypeName']
    .map((columnName) => ({
      color: '#E6F4FF',
      range: {
  ...
  const showDrawer = () => {
    const invisibleCols = defaultCols
      ...
      .map((col) => col.id)
    const currentVisibleCols = loadFromLocalStorageSheet('edu_rst_item_a', []).map(
      (col) => col.id,
    )
    ...
    <div className="w-full gap-1 h-full flex items-center justify-center">
      <div className="w-full h-full flex flex-col  bg-white  overflow-hidden ">
        ...
        <DataEditor
          ...
          customRenderers={[CellsSMAddressType]}
