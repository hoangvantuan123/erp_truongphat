...
import { CellsContractKind } from '../../sheet/cells/cellsContractKind'
function TableLaborContract({
  dataUser,
  setDataUser,
  ContractKindData,
  setContractKindData,
  ...
  const [hiddenColumns, setHiddenColumns] = useState(() => {
    return loadFromLocalStorageSheet('HR_LABOR_CONTRACT_H', [])
  })
  ...
  const columnNames = ['EmpName', 'ContractKindName', 'UMWsName']
  const grayColumns = []
  ...
  const getData = useCallback(
    ([col, row]) => {
      ...
      const cellConfig = {
        ...
        ContractKindName: {
          kind: 'cells-contract-kind',
          allowedValues: ContractKindData,
      ...
      if (key === 'ContractKindName') {
        if (newValue.kind === GridCellKind.Custom) {
          setGridData((prev) => {
            ...
            const checkCopyData = newValue.copyData
            if (!selectedName) {
              selectedName = ContractKindData.find(
                (item) => item.MinorName === checkCopyData,
              )
            }
            if (selectedName) {
              product[cols[col].id] = selectedName.MinorName
              product['ContractKindName'] = selectedName.MinorName
              product['ContractKind'] = selectedName.Value
            } else {
              product[cols[col].id] = ''
              product['ContractKindName'] = ''
              product['ContractKind'] = ''
            }
  ...
  const updateHiddenColumns = (newHiddenColumns) => {
    setHiddenColumns((prevHidden) => {
      ...
      saveToLocalStorageSheet('HR_LABOR_CONTRACT_H', newHidden)
      return newHidden
    })
  }
  ...
  const updateVisibleColumns = (newVisibleColumns) => {
    setCols((prevCols) => {
      ...
      saveToLocalStorageSheet('HR_LABOR_CONTRACT', uniqueCols)
      return uniqueCols
    })
  }
  ...
  const handleReset = () => {
    ...
    setHiddenColumns([])
    localStorage.removeItem('HR_LABOR_CONTRACT')
    localStorage.removeItem('HR_LABOR_CONTRACT_H')
    setShowMenu(null)
  }
  ...
  const onColumnMoved = useCallback((startIndex, endIndex) => {
    setCols((prevCols) => {
      ...
      saveToLocalStorageSheet('HR_LABOR_CONTRACT', updatedCols)
      return updatedCols
    })
  }, [])
  ...
  const handleCheckboxChange = (columnId, isChecked) => {
    if (isChecked) {
      ...
      setCols((prevCols) => {
        ...
        saveToLocalStorageSheet('HR_LABOR_CONTRACT', newCols)
        return newCols
      })
      setHiddenColumns((prevHidden) => {
    ...
    <div className="w-full  h-full flex items-center justify-center">
      <div className="w-full h-full flex flex-col bg-white  overflow-x-hidden overflow-hidden ">
        <DataEditor
          ...
          customRenderers={[CellsEmpNameV2, CellsContractKind]}
        />
        {showMenu !== null &&
          renderLayer(
            <div
...
export default TableLaborContract
