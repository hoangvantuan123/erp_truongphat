...
function TableWHStockAgingList({
  data,
  setSelection,
  selection,
  setShowSearch,
  ...
  const [hiddenColumns, setHiddenColumns] = useState(
    loadFromLocalStorageSheet('H_ERP_COLS_PAGE_STOCK_AGING_LIST', []),
  )
  ...
  const totalRowIndex = data?.findIndex((row) => row?.WHName === 'TOTAL')
  ...
  const highlightRegions = [
    {
      color: '#CBF39A',
      range: {
        x: reorderColumns(cols).indexOf('Qty'),
        y: 0,
        width: 1,
        height: numRows,
      },
    },
    {
      color: '#CBF39A',
      range: {
        x: reorderColumns(cols).indexOf('CQty'),
        y: 0,
        width: 1,
        height: numRows,
      },
    },
    {
      color: '#D8F5B4',
      range: {
        x: reorderColumns(cols).indexOf('Qty1'),
        y: 0,
        width: 1,
        height: numRows,
      },
    },
    {
      color: '#F7F4B5',
      range: {
        x: reorderColumns(cols).indexOf('Qty2'),
        y: 0,
        width: 1,
        height: numRows,
      },
    },
    {
      color: '#FFE6E6',
      range: {
        x: reorderColumns(cols).indexOf('Qty3'),
        y: 0,
        width: 1,
        height: numRows,
      },
    },
    {
      color: '#FFCACA',
      range: {
        x: reorderColumns(cols).indexOf('Qty6'),
        y: 0,
        width: 1,
        height: numRows,
      },
    },
    {
      color: '#FFCACA',
      range: {
        x: reorderColumns(cols).indexOf('Qty9'),
        y: 0,
        width: 1,
        height: numRows,
      },
    },
    {
      color: '#FFB0B0',
      range: {
        x: reorderColumns(cols).indexOf('Qty12'),
        y: 0,
        width: 1,
        height: numRows,
      },
    },
    {
      color: '#FFA6A6',
      range: {
        x: reorderColumns(cols).indexOf('Qty18'),
        y: 0,
        width: 1,
        height: numRows,
      },
...

    {
      color: '#FF8C8C',
      range: {
        x: reorderColumns(cols).indexOf('Qty24'),
        y: 0,
        width: 1,
        height: numRows,
      },
    },
    {
      color: '#FF6A6A',
      range: {
        x: reorderColumns(cols).indexOf('Qty36'),
        y: 0,
        width: 1,
        height: numRows,
      },
    },
    {
      color: '#FF6A6A',
      range: {
        x: reorderColumns(cols).indexOf('Qty99'),
        y: 0,
        width: 1,
        height: numRows,
      },
  ...
  const handleSort = (columnId, direction) => {
    setGridData((prevData) => {
      // Tách dòng "TOTAL" ra khỏi dữ liệu
      const totalRow = prevData.find((row) => row.WHName === 'TOTAL')
      const otherRows = prevData.filter((row) => row.WHName !== 'TOTAL')

      // Sắp xếp các dòng khác
  ...
  const updateHiddenColumns = (newHiddenColumns) => {
    setHiddenColumns((prevHidden) => {
      ...
      saveToLocalStorageSheet('H_ERP_COLS_PAGE_STOCK_AGING_LIST', newHidden)
      return newHidden
    })
  }
  ...
  const updateVisibleColumns = (newVisibleColumns) => {
    setCols((prevCols) => {
      ...
      saveToLocalStorageSheet('S_ERP_COLS_PAGE_STOCK_AGING_LIST', uniqueCols)
      return uniqueCols
    })
  }
  ...
  const handleReset = () => {
    ...
    setHiddenColumns([])
    localStorage.removeItem('S_ERP_COLS_PAGE_STOCK_AGING_LIST')
    localStorage.removeItem('H_ERP_COLS_PAGE_STOCK_AGING_LIST')
  }
  ...
  const onColumnMoved = useCallback((startIndex, endIndex) => {
    setCols((prevCols) => {
      ...
      saveToLocalStorageSheet('S_ERP_COLS_PAGE_STOCK_AGING_LIST', updatedCols)
      return updatedCols
    })
  }, [])
  ...
  const handleCheckboxChange = (columnId, isChecked) => {
    if (isChecked) {
      ...
      setCols((prevCols) => {
        ...
        saveToLocalStorageSheet('S_ERP_COLS_PAGE_STOCK_AGING_LIST', newCols)
        return newCols
      })
      setHiddenColumns((prevHidden) => {
...
export default TableWHStockAgingList
