...
function TableLGStockYearTrans({
  data,
  setSelection,
  selection,
  setShowSearch,
  ...
  const [hiddenColumns, setHiddenColumns] = useState(
    loadFromLocalStorageSheet('H_ERP_COLS_PAGE_STOCK_YEAR_TRANS', []),
  )
  ...
  const getData = useCallback(
    ([col, row]) => {
      ...
      if (columnKey === 'IsStock' || columnKey === 'IsAchievement') {
        const booleanValue =
          value === 1 || value === '1'
            ? true
            : value === 0 || value === '0'
      ...
      if (
        columnKey === 'StockDateTime' ||
        columnKey === 'AchievementDateTime'
      ) {
        if (!value) {
          return {
            kind: GridCellKind.Text,
  ...
  const updateHiddenColumns = (newHiddenColumns) => {
    setHiddenColumns((prevHidden) => {
      ...
      saveToLocalStorageSheet('H_ERP_COLS_PAGE_STOCK_YEAR_TRANS', newHidden)
      return newHidden
    })
  }
  ...
  const updateVisibleColumns = (newVisibleColumns) => {
    setCols((prevCols) => {
      ...
      saveToLocalStorageSheet('S_ERP_COLS_PAGE_STOCK_YEAR_TRANS', uniqueCols)
      return uniqueCols
    })
  }
  ...
  const handleReset = () => {
    ...
    setHiddenColumns([])
    localStorage.removeItem('S_ERP_COLS_PAGE_STOCK_YEAR_TRANS')
    localStorage.removeItem('H_ERP_COLS_PAGE_STOCK_YEAR_TRANS')
  }
  ...
  const onColumnMoved = useCallback((startIndex, endIndex) => {
    setCols((prevCols) => {
      ...
      saveToLocalStorageSheet('S_ERP_COLS_PAGE_STOCK_YEAR_TRANS', updatedCols)
      return updatedCols
    })
  }, [])
  ...
  const handleCheckboxChange = (columnId, isChecked) => {
    if (isChecked) {
      ...
      setCols((prevCols) => {
        ...
        saveToLocalStorageSheet('S_ERP_COLS_PAGE_STOCK_YEAR_TRANS', newCols)
        return newCols
      })
      setHiddenColumns((prevHidden) => {
  ...
  const formattedData = gridData.map((item) => {
    ...
    visibleHeaders.forEach((header) => {
      ...
      if (item[header.id] !== undefined) {
        ...
        if (header.id === 'StkDate' || header.id === 'CfmDate') {
...
export default TableLGStockYearTrans
