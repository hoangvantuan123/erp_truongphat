...
function TableWHBizStockList({
  ...
  const [hiddenColumns, setHiddenColumns] = useState(
    loadFromLocalStorageSheet('H_ERP_COLS_PAGE_BIZ_STOCK_LIST', []),
  )
  ...
  const totalRowIndex = data?.findIndex((row) => row?.ItemName === 'TOTAL')
  ...
  //   gridData.forEach((person, rowIndex) => {
  //     const stockQty = parseFloat(person['StockQty'] || 0)
  //     const safetyQty = parseFloat(person['SafetyQty'] || 0)
  //     if (stockQty < safetyQty) {
  //       regions.push({
  //         color: 'red', // Màu đỏ
  //         range: {
  //           x: cols.findIndex((col) => col.id === 'LackQty'),
  //           y: rowIndex,
  //           width: 1,
  //           height: 1,
  //         },
  ...
  const getData = useCallback(
    ([col, row]) => {
      ...
      if (columnKey === 'DateFr' || columnKey === 'DateTo') {
  ...
  const updateHiddenColumns = (newHiddenColumns) => {
    setHiddenColumns((prevHidden) => {
      ...
      saveToLocalStorageSheet('H_ERP_COLS_PAGE_BIZ_STOCK_LIST', newHidden)
      return newHidden
    })
  }
  ...
  const updateVisibleColumns = (newVisibleColumns) => {
    setCols((prevCols) => {
      ...
      saveToLocalStorageSheet('S_ERP_COLS_PAGE_BIZ_STOCK_LIST', uniqueCols)
      return uniqueCols
    })
  }
  ...
  const handleReset = () => {
    ...
    setHiddenColumns([])
    localStorage.removeItem('S_ERP_COLS_PAGE_BIZ_STOCK_LIST')
    localStorage.removeItem('H_ERP_COLS_PAGE_BIZ_STOCK_LIST')
  }
  ...
  const onColumnMoved = useCallback((startIndex, endIndex) => {
    setCols((prevCols) => {
      ...
      saveToLocalStorageSheet('S_ERP_COLS_PAGE_BIZ_STOCK_LIST', updatedCols)
      return updatedCols
    })
  }, [])
  ...
  const showDrawer = () => {
    ...
    const currentVisibleCols = loadFromLocalStorageSheet(
      'S_ERP_COLS_PAGE_BIZ_STOCK_LIST',
      [],
    ).map((col) => col.id)
  ...
  const handleCheckboxChange = (columnId, isChecked) => {
    if (isChecked) {
      ...
      setCols((prevCols) => {
        ...
        saveToLocalStorageSheet('S_ERP_COLS_PAGE_BIZ_STOCK_LIST', newCols)
        return newCols
      })
      setHiddenColumns((prevHidden) => {
...
export default TableWHBizStockList
