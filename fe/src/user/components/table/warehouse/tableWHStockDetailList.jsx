...
function TableWHStockDetailList({
  data,
  setSelection,
  selection,
  setShowSearch,
  ...
  const [hiddenColumns, setHiddenColumns] = useState(
    loadFromLocalStorageSheet('H_ERP_COLS_PAGE_STOCK_DETAIL_LIST', []),
  )
  ...
  const totalRowIndex = data?.findIndex(
    (row) => row?.InOutName === 'Tồn kho chuyển tiếp',
  )
  ...
  const highlightRegions = useMemo(() => {
    ...
    const hasTonKhoTaiSan = gridData.some(
      (row) => row.InOutName === 'Tồn kho tài sản',
    )
  ...
  const handleSort = (columnId, direction) => {
    setGridData((prevData) => {
      ...
      const totalRow1 = prevData.find(
        (row) => row.InOutName === 'Tồn kho tài sản',
      )
      const totalRow2 = prevData.find(
        (row) => row.InOutName === 'Tính toán thời gian',
      )
      const totalRow3 = prevData.find(
        (row) => row.InOutName === 'Tồn kho chuyển tiếp',
      )
      const otherRows = prevData.filter(
        (row) =>
          row.InOutName !== 'Tồn kho tài sản' &&
          row.InOutName !== 'Tính toán thời gian' &&
          row.InOutName !== 'Tồn kho chuyển tiếp',
      )
      ...
      if (totalRow1) {
        sortedData.push(totalRow1)
        sortedData.push(totalRow2)
        sortedData.push(totalRow3)
      }
  ...
  const updateHiddenColumns = (newHiddenColumns) => {
    setHiddenColumns((prevHidden) => {
      ...
      saveToLocalStorageSheet('H_ERP_COLS_PAGE_STOCK_DETAIL_LIST', newHidden)
      return newHidden
    })
  }
  ...
  const updateVisibleColumns = (newVisibleColumns) => {
    setCols((prevCols) => {
      ...
      saveToLocalStorageSheet('S_ERP_COLS_PAGE_STOCK_DETAIL_LIST', uniqueCols)
      return uniqueCols
    })
  }
  ...
  const handleReset = () => {
    ...
    setHiddenColumns([])
    localStorage.removeItem('S_ERP_COLS_PAGE_STOCK_DETAIL_LIST')
    localStorage.removeItem('H_ERP_COLS_PAGE_STOCK_DETAIL_LIST')
  }
  ...
  const onColumnMoved = useCallback((startIndex, endIndex) => {
    setCols((prevCols) => {
      ...
      saveToLocalStorageSheet('S_ERP_COLS_PAGE_STOCK_DETAIL_LIST', updatedCols)
      return updatedCols
    })
  }, [])
  ...
  const handleCheckboxChange = (columnId, isChecked) => {
    if (isChecked) {
      ...
      setCols((prevCols) => {
        ...
        saveToLocalStorageSheet('S_ERP_COLS_PAGE_STOCK_DETAIL_LIST', newCols)
        return newCols
      })
      setHiddenColumns((prevHidden) => {
  ...
  const freezeTrailingRowsValue = gridData.some(
    (row) => row.InOutName === 'Tồn kho tài sản',
  )
    ? 3
    : 0
    ...
    <div className="w-full h-full flex flex-col bg-white overflow-hidden">
      <div className="w-full h-full flex flex-col border bg-white rounded-lg overflow-hidden ">
        ...
        <DataEditor
          ...
          freezeColumns={5}
          freezeTrailingRows={freezeTrailingRowsValue}
          headerHeight={30}
          rowHeight={28}
...
export default TableWHStockDetailList
