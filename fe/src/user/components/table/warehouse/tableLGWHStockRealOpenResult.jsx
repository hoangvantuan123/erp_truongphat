...
function TableLGWHStockRealOpenResult({
  setSelection,
  selection,
  setShowSearch,
  showSearch,
  ...
  const [hiddenColumns, setHiddenColumns] = useState(
    loadFromLocalStorageSheet('H_ERP_COLS_PAGE_STOCK_REAL_OPEN_RESULT', []),
  )
  ...
  const updateHiddenColumns = (newHiddenColumns) => {
    setHiddenColumns((prevHidden) => {
      ...
      saveToLocalStorageSheet(
        'H_ERP_COLS_PAGE_STOCK_REAL_OPEN_RESULT',
        newHidden,
      )
      return newHidden
    })
  ...
  const handleReset = () => {
    ...
    setHiddenColumns([])
    localStorage.removeItem('S_ERP_COLS_PAGE_STOCK_REAL_OPEN_RESULT')
    localStorage.removeItem('H_ERP_COLS_PAGE_STOCK_REAL_OPEN_RESULT')
  }
  ...
  const findRowByItemNo = useCallback(
    (itemNo, stkMngSerl) =>
      gridData.findIndex(
        (row) => row.ItemNo === itemNo && row.StkMngSerl === stkMngSerl,
      ),
    [gridData],
  )
  const highlightRowByItemNo = useCallback(
    (itemNo) => {
      const rowIndex = findRowByItemNo(itemNo, stkMngSerl)
      if (rowIndex !== -1) {
        // Use -1 for not found
        setHighlightRegions([
          {
  ...
  useEffect(() => {
    highlightRowByItemNo(inputItemNo, stkMngSerl)
  }, [inputItemNo, stkMngSerl, highlightRowByItemNo])
...
export default TableLGWHStockRealOpenResult
