...
function TableLGEtcTransReqList({
  data,
  setSelection,
  selection,
  setShowSearch,
  ...
  const [hiddenColumns, setHiddenColumns] = useState(
    loadFromLocalStorageSheet('H_ERP_COLS_PAGE_ETC_TRANS_REQ_LIST', []),
  )
    ...
    async (cell, newValue) => {
      ...
      if (key === 'Confirm') {
        if (gridData[row].Confirm === 0) {
          setReqNoTitle(`Xác nhận đơn yêu cầu số ${gridData[row].ReqNo}`)
          setReqSeq(gridData[row].ReqSeq)
          setReqNo(gridData[row].ReqNo)
          setRowIndex(row)
          setIsConfirm('1')
        } else if (gridData[row].Confirm === 1) {
          setReqNoTitle(`Hủy xác nhận đơn yêu cầu số ${gridData[row].ReqNo}`)
          setReqSeq(gridData[row].ReqSeq)
          setReqNo(gridData[row].ReqNo)
          setRowIndex(row)
          setIsConfirm('0')
      ...
      if (key === 'IsStop') {
        if (gridData[row].IsStop === '0') {
          setReqNoTitle(`Xác nhận đơn dừng yêu cầu số ${gridData[row].ReqNo}`)
          setReqSeq(gridData[row].ReqSeq)
          setReqNo(gridData[row].ReqNo)
          setRowIndex(row)
          setIsStop('1')
          ...
          setReqNoTitle(
            `Hủy xác nhận dừng đơn yêu cầu số ${gridData[row].ReqNo}`,
          )
          setReqSeq(gridData[row].ReqSeq)
          setReqNo(gridData[row].ReqNo)
          setRowIndex(row)
  ...
  const updateHiddenColumns = (newHiddenColumns) => {
    setHiddenColumns((prevHidden) => {
      ...
      saveToLocalStorageSheet('H_ERP_COLS_PAGE_ETC_TRANS_REQ_LIST', newHidden)
      return newHidden
    })
  }
  ...
  const updateVisibleColumns = (newVisibleColumns) => {
    setCols((prevCols) => {
      ...
      saveToLocalStorageSheet('S_ERP_COLS_PAGE_ETC_TRANS_REQ_LIST', uniqueCols)
      return uniqueCols
    })
  }
    ...
    if (cols.length > 1) {
      ...
      setCols((prevCols) => {
        ...
        saveToLocalStorageSheet(
          'S_ERP_COLS_PAGE_ETC_TRANS_REQ_LIST',
          uniqueCols,
        )
        return uniqueCols
      })
  ...
  const handleReset = () => {
    ...
    setHiddenColumns([])
    localStorage.removeItem('S_ERP_COLS_PAGE_ETC_TRANS_REQ_LIST')
    localStorage.removeItem('H_ERP_COLS_PAGE_ETC_TRANS_REQ_LIST')
  }
  ...
  const onColumnMoved = useCallback((startIndex, endIndex) => {
    setCols((prevCols) => {
      ...
      saveToLocalStorageSheet('S_ERP_COLS_PAGE_ETC_TRANS_REQ_LIST', updatedCols)
      return updatedCols
    })
  }, [])
  ...
  const handleCheckboxChange = (columnId, isChecked) => {
    if (isChecked) {
      ...
      setCols((prevCols) => {
        ...
        saveToLocalStorageSheet('S_ERP_COLS_PAGE_ETC_TRANS_REQ_LIST', newCols)
        return newCols
      })
      setHiddenColumns((prevHidden) => {
...
export default TableLGEtcTransReqList
