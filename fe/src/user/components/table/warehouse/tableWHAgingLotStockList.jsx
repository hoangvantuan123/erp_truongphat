...
function TableWHAgingLotStockList({
  data,
  setSelection,
  selection,
  setShowSearch,
  ...
  const [hiddenColumns, setHiddenColumns] = useState(
    loadFromLocalStorageSheet('H_ERP_COLS_PAGE_AGING_LOT_STOCK_LIST', []),
  )
  ...
  const getData = useCallback(
    ([col, row]) => {
      ...
      if (
        ...
        columnKey === 'PrevQty' ||
        columnKey === 'ExpireQty' ||
        columnKey === 'Less3MoQty' ||
        columnKey === 'Less6MoQty' ||
        columnKey === 'Less9MoQty' ||
        columnKey === 'Less12MoQty' ||
        columnKey === 'More12MoQty' ||
        columnKey === 'StockQty'
      ) {
        return {
          kind: GridCellKind.Number,
  ...
  const updateHiddenColumns = (newHiddenColumns) => {
    setHiddenColumns((prevHidden) => {
      ...
      saveToLocalStorageSheet('H_ERP_COLS_PAGE_AGING_LOT_STOCK_LIST', newHidden)
      return newHidden
    })
  }
  ...
  const handleReset = () => {
    ...
    setHiddenColumns([])
    localStorage.removeItem('S_ERP_COLS_PAGE_AGING_LOT_STOCK_LIST')
    localStorage.removeItem('H_ERP_COLS_PAGE_AGING_LOT_STOCK_LIST')
  }
  ...
  const handleCheckboxChange = (columnId, isChecked) => {
    if (isChecked) {
      ...
      setCols((prevCols) => {
        ...
        saveToLocalStorageSheet('S_ERP_COLS_PAGE_AGING_LOT_STOCK_LIST', newCols)
        return newCols
      })
      setHiddenColumns((prevHidden) => {
        const newHidden = prevHidden.filter((id) => id !== columnId)
        saveToLocalStorageSheet(
          'H_ERP_COLS_PAGE_AGING_LOT_STOCK_LIST',
          newHidden,
        )
        return newHidden
      })
...
export default TableWHAgingLotStockList
