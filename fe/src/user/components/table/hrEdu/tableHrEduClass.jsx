...
function TableHrEduClass({
  ...
  const [hiddenColumns, setHiddenColumns] = useState(() => {
    return loadFromLocalStorageSheet('EDU_CLASS_LIST', [])
  })
  ...
  const columnNames = ['EduCompeName', 'UMEduMidClassName']
    ...
    ([col, row]) => {
      ...
      if (columnKey === 'EduCompeName') {
        return {
          ...
          data: {
            kind: 'cells-sm-combo',
            allowedValues: EduCompeData,
            setCacheData: setEduCompeData,
            value: value,
            ...
            onFinishedEditing: (newCell) => {
              setGridData((prev) => {
                ...
                product['EduCompeName'] = newCell?.data[0]?.MinorName
                product['EduCompeSeq'] = newCell?.data[0]?.Value
                return newData
              })
            },
          },
      ...
      if (columnKey === 'UMEduMidClassName') {
        return {
          kind: GridCellKind.Custom,
          allowOverlay: true,
          copyData: String(value),
          data: {
            kind: 'cells-sm-combo',
            allowedValues: MidClassData,
            setCacheData: setMidClassData,
            value: value,
            ...
            onFinishedEditing: (newCell) => {
              setGridData((prev) => {
                ...
                product['UMEduMidClassName'] =
                  newCell?.data[0]?.UMEduMidClassName
                product['UMEduMidClassSeq'] = newCell?.data[0]?.UMEduMidClassSeq
                return newData
              })
            },
          },
  ...
  const updateHiddenColumns = (newHiddenColumns) => {
    setHiddenColumns((prevHidden) => {
      ...
      saveToLocalStorageSheet('EDU_CLASS_LIST', newHidden)
      return newHidden
    })
  }
  ...
  const updateVisibleColumns = (newVisibleColumns) => {
    setCols((prevCols) => {
      ...
      saveToLocalStorageSheet('EDU_CLASS_LIST_h', uniqueCols)
      return uniqueCols
    })
  }
  ...
  const handleReset = () => {
    ...
    setHiddenColumns([])
    localStorage.removeItem('EDU_CLASS_LIST_h')
    localStorage.removeItem('EDU_CLASS_LIST')
  }
  ...
  const onColumnMoved = useCallback((startIndex, endIndex) => {
    setCols((prevCols) => {
      ...
      saveToLocalStorageSheet('EDU_CLASS_LIST_h', updatedCols)
      return updatedCols
    })
  }, [])
  ...
  const handleCheckboxChange = (columnId, isChecked) => {
    if (isChecked) {
      ...
      setCols((prevCols) => {
        ...
        saveToLocalStorageSheet('EDU_CLASS_LIST_h', newCols)
        return newCols
      })
      setHiddenColumns((prevHidden) => {
      ...
      if (key === 'UMEduMidClassName') {
        if (newValue.kind === GridCellKind.Custom) {
          setGridData((prev) => {
            ...
            if (!selectedName && Array.isArray(MidClassData)) {
              selectedName = MidClassData.find(
            ...
            if (selectedName || selectedName !== undefined) {
              if (Array.isArray(ClassData)) {
                highClass = ClassData.find(
                  (item) => item?.UMEduMidClass === selectedName?.Value,
                )
              }
              product[cols[col].id] = selectedName.MinorName
              product['UMEduMidClassName'] = selectedName.MinorName
              product['UMEduMidClass'] = selectedName.Value

              product['UMEduHighClass'] = highClass?.UMEduHighClass
              product['UMEduHighClassName'] = highClass?.UMEduHighClassName
            } else {
              product[cols[col].id] = ''
        ...
        if (newValue.kind === GridCellKind.Custom) {
          setGridData((prev) => {
            ...
            if (!selectedName && Array.isArray(ClassData)) {
              selectedName = EduCompeData.find(
                (item) => item?.MinorName === checkCopyData,
              )
            }
            ...
            if (selectedName || selectedName !== undefined) {
              product[cols[col].id] = selectedName.MinorName
              product['EduCompeName'] = selectedName.MinorName
              product['EduCompeSeq'] = selectedName.Value
            } else {
              product[cols[col].id] = ''
              product['EduCompeName'] = ''
              product['EduCompeSeq'] = ''
            }
...
export default TableHrEduClass
