...
import HrEduLecturerInsiteQuery from '../../query/hrEdu/hrEduLecturerInsiteQuery'
import { CellsEmpNameV2 } from '../../sheet/cells/cellsEmpNamev2'

function TableHrEduLecturerInsite({
  DeptData,
  setDeptData,
  EmpData,
  setEmpData,
  ...
  const [hiddenColumns, setHiddenColumns] = useState(() => {
    return loadFromLocalStorageSheet('HR_EDU_INSITE', [])
  })
  ...
  const columnNames = ['EmpName', 'SMPayTypeName']
    ...
    ([col, row]) => {
      ...
      if (columnKey === 'EmpName') {
        return {
          ...
          data: {
            ...
            allowedValues: EmpData,
            setCacheData: setEmpData,
            value: value,
            ...
            onFinishedEditing: (newCell) => {
              setGridData((prev) => {
                ...
                product['EmpName'] = newCell?.data[0]?.MinorName
                product['EmpSeq'] = newCell?.data[0]?.Value
                return newData
              })
            },
          },
      ...
      if (columnKey === 'SMPayTypeName') {
        return {
          ...
          data: {
            kind: 'cells-sm-combo',
            allowedValues: payTypeData,
            setCacheData: setPayTypeData,
            value: value,
            ...
            onFinishedEditing: (newCell) => {
              setGridData((prev) => {
                const newData = [...prev]
                const product = newData[row]
                product['SMPayTypeName'] = newCell?.data[0]?.MinorName
                product['SMPayType'] = newCell?.data[0]?.Value
                return newData
              })
            },
          },
  ...
  const updateHiddenColumns = (newHiddenColumns) => {
    setHiddenColumns((prevHidden) => {
      ...
      saveToLocalStorageSheet('HR_EDU_INSITE', newHidden)
      return newHidden
    })
  }
  ...
  const updateVisibleColumns = (newVisibleColumns) => {
    setCols((prevCols) => {
      ...
      saveToLocalStorageSheet('HR_EDU_INSITE_h', uniqueCols)
      return uniqueCols
    })
  }
  ...
  const handleReset = () => {
    ...
    setHiddenColumns([])
    localStorage.removeItem('HR_EDU_INSITE_h')
    localStorage.removeItem('HR_EDU_INSITE')
  }
  ...
  const onColumnMoved = useCallback((startIndex, endIndex) => {
    setCols((prevCols) => {
      ...
      saveToLocalStorageSheet('HR_EDU_INSITE_h', updatedCols)
      return updatedCols
    })
  }, [])
  ...
  const handleCheckboxChange = (columnId, isChecked) => {
    if (isChecked) {
      ...
      setCols((prevCols) => {
        ...
        saveToLocalStorageSheet('HR_EDU_INSITE_h', newCols)
        return newCols
      })
      setHiddenColumns((prevHidden) => {
      ...
      if (key === 'EmpName') {
        if (newValue.kind === GridCellKind.Custom) {
          setGridData((prev) => {
            ...
            if (!selectedName && Array.isArray(EmpData)) {
              selectedName = EmpData.find(
            ...
            if (selectedName || selectedName !== undefined) {
              ...
              product['PosSeq'] = selectedName.PosSeq
              product['UMJpSeq'] = selectedName.UMJpSeq
      ...
      if (key === 'SMPayTypeName') {
        if (newValue.kind === GridCellKind.Custom) {
          setGridData((prev) => {
            ...
            let selectedName = newValue.data[0]
            const checkCopyData = newValue.copyData
            if (!selectedName && Array.isArray(payTypeData)) {
              selectedName = payTypeData.find(
            ...
            if (selectedName || selectedName !== undefined) {
              ...
              product['SMPayTypeName'] = selectedName.MinorName
              product['SMPayType'] = selectedName.Value
            } else {
              product[cols[col].id] = ''
              product['SMPayTypeName'] = ''
              product['SMPayType'] = ''
            }
...
export default TableHrEduLecturerInsite
