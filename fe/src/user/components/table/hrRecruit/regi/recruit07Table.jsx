...
import { Cells30013 } from '../../../sheet/cells/cells30013'
import { Cells30012 } from '../../../sheet/cells/cells30012'
import moment from 'moment'

function Recruit07Table({
    setSelection,
    selection,
    setShowSearch,
    showSearch,
    ...
    const [hiddenColumns, setHiddenColumns] = useState(() => {
        return loadFromLocalStorageSheet('recruitment07_h', [])
    })
    ...
    const getData = useCallback(
        ([col, row]) => {
            ...
            if (columnKey === 'SkillLevelHigh' || columnKey === 'SkillLevelMedium' || columnKey === 'SkillLevelLow') {
                const booleanValue =
                    value === 1 || value === '1'
                        ? true
                        : value === 0 || value === '0'
        ...
        [gridData, cols, helpData01, helpData02, helpData03, helpData04]
    )
    ...
    const onCellEdited = useCallback(
        async (cell, newValue) => {
            ...
            const skillLevelKeys = ['SkillLevelHigh', 'SkillLevelMedium', 'SkillLevelLow']
            if (skillLevelKeys.includes(key)) {
                setGridData((prevData) => {
                    const updatedData = [...prevData]
                    if (!updatedData[row]) updatedData[row] = {}

                    // Đặt tất cả về false trước
                    skillLevelKeys.forEach((k) => {
                        updatedData[row][k] = false
                    })

                    // Đặt cột được chọn thành true
                ...
                if (newValue.kind === GridCellKind.Custom) {
                    setGridData((prev) => {
                        ...
                        if (selectedName) {
                            ...
                            product['SkillTypeSeq'] = selectedName.IdSeq
                        } else {
                            product[cols[col].id] = ''
                            product['SkillTypeSeq'] = ''
                        }
            ...
            if (key === 'SkillNameName') {
                if (newValue.kind === GridCellKind.Custom) {
                    setGridData((prev) => {
                        ...
                        if (selectedName) {
                            product[cols[col].id] = selectedName.DefineItemName
                            product['SkillNameSeq'] = selectedName.IdSeq
                        } else {
                            product[cols[col].id] = ''
                            product['SkillNameSeq'] = ''
                        }
        ...
        [cols, gridData, helpData01, helpData02, helpData03, canEdit]
    )
    ...
    const updateHiddenColumns = (newHiddenColumns) => {
        setHiddenColumns((prevHidden) => {
            ...
            saveToLocalStorageSheet('recruitment07_h', newHidden)
            return newHidden
        })
    }
    ...
    const updateVisibleColumns = (newVisibleColumns) => {
        setCols((prevCols) => {
            ...
            saveToLocalStorageSheet('recruitment07_a', uniqueCols)
            return uniqueCols
        })
    }
    ...
    const handleReset = () => {
        ...
        setHiddenColumns([])
        localStorage.removeItem('recruitment07_a')
        localStorage.removeItem('recruitment07_h')
    }
    ...
    const onColumnMoved = useCallback((startIndex, endIndex) => {
        setCols((prevCols) => {
            ...
            saveToLocalStorageSheet('recruitment07_a', updatedCols)
            return updatedCols
        })
    }, [])
    ...
    const showDrawer = () => {
        ...
        const currentVisibleCols = loadFromLocalStorageSheet('recruitment07_a', []).map(
            (col) => col.id
        )
    ...
    const handleCheckboxChange = (columnId, isChecked) => {
        if (isChecked) {
            ...
            setCols((prevCols) => {
                ...
                saveToLocalStorageSheet('recruitment07_a', newCols)
                return newCols
            })
            setHiddenColumns((prevHidden) => {
