...
function RptSalesCatTable({
    ...
    const [hiddenColumns, setHiddenColumns] = useState(() => {
        return loadFromLocalStorageSheet('rpt_sales_cat_h', [])
    })
    ...
    const getData = useCallback(
        ([col, row]) => {
            ...
            // Cột hiển thị kiểu số (format số)
            const numericColumns = [
                'YearSalesAmt', 'YearBenefitAmt', 'YearBenefitRate',
                'ThisSalesAmt', 'ThisBenefitAmt', 'ThisBenefitRate',
                'YearRtnAmt', 'ThisRtnAmt'
            ];

            // Cột được tính tổng
            const totalColumns = [
                'YearSalesAmt', 'YearBenefitAmt',
                'ThisSalesAmt', 'ThisBenefitAmt',
                'YearRtnAmt', 'ThisRtnAmt'
            ];
            ...
            // ===== Xử lý dòng cuối cùng (TÍNH TỔNG nếu nằm trong totalColumns) =====
            if (row === lastRowIndex) {
                ...
                // Dòng cuối các cột KHÔNG tính tổng
                return {
            ...
            // ===== Xử lý dòng dữ liệu thông thường =====
            if (numericColumns.includes(columnKey)) {
                const formattedValue = formatEtcOutAmt(value);
    ...
    const updateHiddenColumns = (newHiddenColumns) => {
        setHiddenColumns((prevHidden) => {
            ...
            saveToLocalStorageSheet('rpt_sales_cat_h', newHidden)
            return newHidden
        })
    }
    ...
    const updateVisibleColumns = (newVisibleColumns) => {
        setCols((prevCols) => {
            ...
            saveToLocalStorageSheet('rpt_sales_cat_a', uniqueCols)
            return uniqueCols
        })
    }
    ...
    const handleReset = () => {
        ...
        setHiddenColumns([])
        localStorage.removeItem('rpt_sales_cat_a')
        localStorage.removeItem('rpt_sales_cat_h')
    }
    ...
    const onColumnMoved = useCallback((startIndex, endIndex) => {
        setCols((prevCols) => {
            ...
            saveToLocalStorageSheet('rpt_sales_cat_a', updatedCols)
            return updatedCols
        })
    }, [])
    ...
    const showDrawer = () => {
        ...
        const currentVisibleCols = loadFromLocalStorageSheet('rpt_sales_cat_a', []).map(
            (col) => col.id
        )
    ...
    const handleCheckboxChange = (columnId, isChecked) => {
        if (isChecked) {
            ...
            setCols((prevCols) => {
                ...
                saveToLocalStorageSheet('rpt_sales_cat_a', newCols)
                return newCols
            })
            setHiddenColumns((prevHidden) => {
...
export default RptSalesCatTable
