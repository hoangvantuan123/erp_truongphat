...
function AcctDetailedBalanceListBTable({
    setSelection,
    selection,
    setShowSearch,
    showSearch,
    ...
    const [hiddenColumns, setHiddenColumns] = useState(() => {
        return loadFromLocalStorageSheet('acct_detailed_balance_list_b_h', [])
    })
    ...
    const getData = useCallback(
        ([col, row]) => {
            ...
            const numericColumns = ['DrAmt', 'CrAmt', 'ExRate', 'DrForAmt', 'CrForAmt'];
    
            // ===== DÒNG DỮ LIỆU BÌNH THƯỜNG =====
            if (numericColumns.includes(columnKey)) {
                let raw = parseFloat(value?.toString()?.replace(/,/g, '') || '0');
    
                // Nếu là ExRate thì luôn hiển thị 5 chữ số sau dấu chấm
                const formattedValue = columnKey === 'ExRate'
                    ? raw.toFixed(5)
                    : formatEtcOutAmt(raw);
    
                // Màu chữ theo giá trị
                let textColor = "#333";
                if (raw < 0) textColor = "#D32F2F";
                else if (raw > 0) textColor = "#388E3C";
    ...
    const updateHiddenColumns = (newHiddenColumns) => {
        setHiddenColumns((prevHidden) => {
            ...
            saveToLocalStorageSheet('acct_detailed_balance_list_b_h', newHidden)
            return newHidden
        })
    }
    ...
    const updateVisibleColumns = (newVisibleColumns) => {
        setCols((prevCols) => {
            ...
            saveToLocalStorageSheet('acct_detailed_balance_list_b_a', uniqueCols)
            return uniqueCols
        })
    }
    ...
    const handleReset = () => {
        ...
        setHiddenColumns([])
        localStorage.removeItem('acct_detailed_balance_list_b_a')
        localStorage.removeItem('acct_detailed_balance_list_b_h')
    }
    ...
    const onColumnMoved = useCallback((startIndex, endIndex) => {
        setCols((prevCols) => {
            ...
            saveToLocalStorageSheet('acct_detailed_balance_list_b_a', updatedCols)
            return updatedCols
        })
    }, [])
    ...
    const showDrawer = () => {
        ...
        const currentVisibleCols = loadFromLocalStorageSheet('acct_detailed_balance_list_b_a', []).map(
            (col) => col.id
        )
    ...
    const handleCheckboxChange = (columnId, isChecked) => {
        if (isChecked) {
            ...
            setCols((prevCols) => {
                ...
                saveToLocalStorageSheet('acct_detailed_balance_list_b_a', newCols)
                return newCols
            })
            setHiddenColumns((prevHidden) => {
...
export default AcctDetailedBalanceListBTable
