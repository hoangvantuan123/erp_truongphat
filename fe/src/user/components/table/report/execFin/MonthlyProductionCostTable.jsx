...
function MonthlyProductionCostTable({
    setSelection,
    selection,
    setShowSearch,
    showSearch,
    ...
    const [hiddenColumns, setHiddenColumns] = useState(() => {
        return loadFromLocalStorageSheet('monthly_production_cost_h', [])
    })
    ...
    const getData = useCallback(
        ([col, row]) => {
            ...
            const foreColorCode = person["SMItemForeColorCode"];
            const backColorCode = person["SMItemBackColorCode"];

            let themeOverride;

            if (foreColorCode && backColorCode) {
                themeOverride = {
                    textDark: `#0050b3`,
                    bgCell: `#f0f5ff`,
                    fontWeight: 'bold',
                };
            } else if (foreColorCode) {
                themeOverride = {
                    textDark: `#0050b3`,
                };
            } else {
    ...
    const updateHiddenColumns = (newHiddenColumns) => {
        setHiddenColumns((prevHidden) => {
            ...
            saveToLocalStorageSheet('monthly_production_cost_h', newHidden)
            return newHidden
        })
    }
    ...
    const updateVisibleColumns = (newVisibleColumns) => {
        setCols((prevCols) => {
            ...
            saveToLocalStorageSheet('monthly_production_cost_a', uniqueCols)
            return uniqueCols
        })
    }
    ...
    const handleReset = () => {
        ...
        setHiddenColumns([])
        localStorage.removeItem('monthly_production_cost_a')
        localStorage.removeItem('monthly_production_cost_h')
    }
    ...
    const onColumnMoved = useCallback((startIndex, endIndex) => {
        setCols((prevCols) => {
            ...
            saveToLocalStorageSheet('monthly_production_cost_a', updatedCols)
            return updatedCols
        })
    }, [])
    ...
    const showDrawer = () => {
        ...
        const currentVisibleCols = loadFromLocalStorageSheet('monthly_production_cost_a', []).map(
            (col) => col.id
        )
    ...
    const handleCheckboxChange = (columnId, isChecked) => {
        if (isChecked) {
            ...
            setCols((prevCols) => {
                ...
                saveToLocalStorageSheet('monthly_production_cost_a', newCols)
                return newCols
            })
            setHiddenColumns((prevHidden) => {
...
export default MonthlyProductionCostTable
