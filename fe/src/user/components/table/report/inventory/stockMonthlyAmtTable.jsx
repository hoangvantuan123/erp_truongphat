...
function StockMonthlyAmtTable({
    ...
    const [hiddenColumns, setHiddenColumns] = useState(() => {
        return loadFromLocalStorageSheet('stock_monthly_amt_h', [])
    })
    ...
    const getData = useCallback(
        ([col, row]) => {
            ...
            const amtColumns = [
                "PreAmt", "ProdAmt", "BuyAmt", "MvInAmt", "EtcInAmt", "ExchangeInAmt",
                "SalesAmt", "InputAmt", "MvOutAmt", "EtcOutAmt", "ExchangeOutAmt",
                "InAmt", "OutAmt", "StockAmt2", "StockAmt", "DiffAmt"
            ];

            const qtyColumns = [
                "PreQty", "ProdQty", "BuyQty", "MvInQty", "EtcInQty", "ExchangeInQty",
                "SalesQty", "InputQty", "MvOutQty", "EtcOutQty", "ExchangeOutQty",
                "InQty", "OutQty", "StockQty2", "StockQty", "DiffQty"
            ];

            const numericColumns = [...amtColumns, ...qtyColumns];
            ...
            if (row === lastRowIndex) {
                if (numericColumns.includes(columnKey)) {
                    ...
                    const decimalDigits = qtyColumns.includes(columnKey) ? 4 : 2;
            ...
            if (numericColumns.includes(columnKey)) {
                ...
                let textColor = "#333";
                if (rawText < 0) textColor = "#D32F2F";        // đỏ
                else if (rawText > 0) textColor = "#388E3C";   // xanh
    ...
    const updateHiddenColumns = (newHiddenColumns) => {
        setHiddenColumns((prevHidden) => {
            ...
            saveToLocalStorageSheet('stock_monthly_amt_h', newHidden)
            return newHidden
        })
    }
    ...
    const updateVisibleColumns = (newVisibleColumns) => {
        setCols((prevCols) => {
            ...
            saveToLocalStorageSheet('stock_monthly_amt_a', uniqueCols)
            return uniqueCols
        })
    }
    ...
    const handleReset = () => {
        ...
        setHiddenColumns([])
        localStorage.removeItem('stock_monthly_amt_a')
        localStorage.removeItem('stock_monthly_amt_h')
    }
    ...
    const onColumnMoved = useCallback((startIndex, endIndex) => {
        setCols((prevCols) => {
            ...
            saveToLocalStorageSheet('stock_monthly_amt_a', updatedCols)
            return updatedCols
        })
    }, [])
    ...
    const showDrawer = () => {
        ...
        const currentVisibleCols = loadFromLocalStorageSheet('stock_monthly_amt_a', []).map(
            (col) => col.id
        )
    ...
    const handleCheckboxChange = (columnId, isChecked) => {
        if (isChecked) {
            ...
            setCols((prevCols) => {
                ...
                saveToLocalStorageSheet('stock_monthly_amt_a', newCols)
                return newCols
            })
            setHiddenColumns((prevHidden) => {
...
export default StockMonthlyAmtTable
