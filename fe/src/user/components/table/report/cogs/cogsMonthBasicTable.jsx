...
function CogsMonthBasicTable({
    ...
    const [hiddenColumns, setHiddenColumns] = useState(() => {
        return loadFromLocalStorageSheet('cogs_month_basic_h', [])
    })
    ...
    const getData = useCallback(
        ([col, row]) => {
          ...
          const numericColumns = [
            'TotalAmt',
            'Mo1Qty', 'Mo1Price', 'Mo1Amt',
            'Mo2Qty', 'Mo2Price', 'Mo2Amt',
            'Mo3Qty', 'Mo3Price', 'Mo3Amt',
            'Mo4Qty', 'Mo4Price', 'Mo4Amt',
            'Mo5Qty', 'Mo5Price', 'Mo5Amt',
            'Mo6Qty', 'Mo6Price', 'Mo6Amt',
            'Mo7Qty', 'Mo7Price', 'Mo7Amt',
            'Mo8Qty', 'Mo8Price', 'Mo8Amt',
            'Mo9Qty', 'Mo9Price', 'Mo9Amt',
            'Mo10Qty', 'Mo10Price', 'Mo10Amt',
            'Mo11Qty', 'Mo11Price', 'Mo11Amt',
            'Mo12Qty', 'Mo12Price', 'Mo12Amt',
          ];
      
          const totalColumns = [...numericColumns];
      
          const getFractionDigits = (key) => {
          ...
          if (numericColumns.includes(columnKey)) {
            const raw = parseFloat(value?.toString()?.replace(/,/g, '') || '0');
            const formattedValue = formatEtcOutAmt(raw);
      
            // Màu chữ theo giá trị
            let textColor = "#333";
            if (raw < 0) textColor = "#D32F2F";        // đỏ
            else if (raw > 0) textColor = "#388E3C";   // xanh
            else textColor = "#999999";                // xám
      
            // Màu nền theo loại cột
            let bgColor = "#ffffff";
            if (columnKey.includes("Qty")) bgColor = "#F1F8E9";       // xanh lá nhạt
            else if (columnKey.includes("Price")) bgColor = "#E3F2FD"; // xanh dương nhạt
            else if (columnKey.includes("Amt")) bgColor = "#FFF8E1";   // vàng nhạt
    ...
    const updateHiddenColumns = (newHiddenColumns) => {
        setHiddenColumns((prevHidden) => {
            ...
            saveToLocalStorageSheet('cogs_month_basic_h', newHidden)
            return newHidden
        })
    }
    ...
    const updateVisibleColumns = (newVisibleColumns) => {
        setCols((prevCols) => {
            ...
            saveToLocalStorageSheet('cogs_month_basic_a', uniqueCols)
            return uniqueCols
        })
    }
    ...
    const handleReset = () => {
        ...
        setHiddenColumns([])
        localStorage.removeItem('cogs_month_basic_a')
        localStorage.removeItem('cogs_month_basic_h')
    }
    ...
    const onColumnMoved = useCallback((startIndex, endIndex) => {
        setCols((prevCols) => {
            ...
            saveToLocalStorageSheet('cogs_month_basic_a', updatedCols)
            return updatedCols
        })
    }, [])
    ...
    const showDrawer = () => {
        ...
        const currentVisibleCols = loadFromLocalStorageSheet('cogs_month_basic_a', []).map(
            (col) => col.id
        )
    ...
    const handleCheckboxChange = (columnId, isChecked) => {
        if (isChecked) {
            ...
            setCols((prevCols) => {
                ...
                saveToLocalStorageSheet('cogs_month_basic_a', newCols)
                return newCols
            })
            setHiddenColumns((prevHidden) => {
...
export default CogsMonthBasicTable
