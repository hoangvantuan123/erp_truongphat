...
function TableIqcCheckStatusList({
  dataUnit,
  setSelection,
  selection,
  setShowSearch,
  ...
  const [hiddenColumns, setHiddenColumns] = useState(() => {
    return loadFromLocalStorageSheet(
      'H_ERP_COLS_PAGE_IQC_CHECK_STATUS_LIST',
      [],
    )
  })
  ...
  const handleReset = () => {
    ...
    setHiddenColumns([])
    localStorage.removeItem('S_ERP_COLS_PAGE_IQC_CHECK_STATUS_LIST')
    localStorage.removeItem('H_ERP_COLS_PAGE_IQC_CHECK_STATUS_LIST')
    setShowMenu(null)
  }
  ...
  const formattedData = gridData.map((item) => {
    ...
    cols.forEach((header) => {
      if (item[header.id] !== undefined) {
        if (header.id === 'QCDate' || header.id === 'BLDate' || header.id === 'DelvDate' || header.id === 'TestEndDate') {
          ...
          renderLayer(
            ...
            >
              {showMenu.menuType === 'statusMenu' ? (
                ...
                fileName="PDQCImpBLResultListITMV"
                customHeaders={cols}
              />
              ) : (
                <LayoutMenuSheet
...
export default TableIqcCheckStatusList
