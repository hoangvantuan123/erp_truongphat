...
import { CellToolName } from '../../sheet/cells/cellsToolName'
import moment from 'moment'
function Tablepdsf51676({
  setSelection,
  selection,
  setShowSearch,
  showSearch,
  ...
  const [hiddenColumns, setHiddenColumns] = useState(() => {
    return loadFromLocalStorageSheet('pdsfc_work_report_51676_ah', [])
  })
  ...
  const columnNames = ['UMNonWorkTypeL', 'UMNonWorkTypeS', 'ToolName', 'ToolNo'];
  const grayColumns = [];
  ...
  const getData = useCallback(
    ([col, row]) => {
      ...
      const cellConfig = {
        ToolName: {
          kind: 'cells-tool-name',
          allowedValues: helpData14,
          setCacheData: setHelpData14,
        },
  ...
  const onCellEdited = useCallback(
      ...
      if (key === 'ToolName') {
        if (newValue.kind === GridCellKind.Custom) {
          setGridData((prev) => {
            ...
            if (!selectedName) {
              selectedName = helpData14.find(
                (item) => item.ToolName === checkCopyData,
              )
            }
            if (selectedName) {
              product[cols[col].id] = selectedName.ToolName
              product['ToolSeq'] = selectedName.ToolSeq
              product['ToolNo'] = selectedName.ToolNo
    ...
    [canEdit, cols, gridData, helpData14],
  )
  ...
  const updateHiddenColumns = (newHiddenColumns) => {
    setHiddenColumns((prevHidden) => {
      ...
      saveToLocalStorageSheet('pdsfc_work_report_51676_ah', newHidden)
      return newHidden
    })
  }
  ...
  const updateVisibleColumns = (newVisibleColumns) => {
    setCols((prevCols) => {
      ...
      saveToLocalStorageSheet('pdsfc_work_report_51676_a', uniqueCols)
      return uniqueCols
    })
  }
  ...
  const handleReset = () => {
    ...
    setHiddenColumns([])
    localStorage.removeItem('pdsfc_work_report_51676_a')
    localStorage.removeItem('pdsfc_work_report_51676_ah')
    setShowMenu(null)
  }
  ...
  const onColumnMoved = useCallback((startIndex, endIndex) => {
    setCols((prevCols) => {
      ...
      saveToLocalStorageSheet('pdsfc_work_report_51676_a', updatedCols)
      return updatedCols
    })
  }, [])
  ...
  const handleCheckboxChange = (columnId, isChecked) => {
    if (isChecked) {
      ...
      setCols((prevCols) => {
        ...
        saveToLocalStorageSheet('pdsfc_work_report_51676_a', newCols)
        return newCols
      })
      setHiddenColumns((prevHidden) => {
    ...
    <div className="w-full  h-full flex items-center justify-center">
      <div className="w-full h-full flex flex-col bg-white  overflow-x-hidden overflow-hidden ">
        <div className="flex items-center gap-3 p-2">
          <button onClick={handleSave6} className="flex items-center gap-1 px-2 py-1 text-emerald-600 text-xs hover:bg-emerald-50 rounded">
            <Save size={14} />
            {t('55168')}
          </button>
          <button onClick={handleDeleteDataSheet6} className="flex items-center gap-1 px-2 py-1 text-rose-600 text-xs hover:bg-rose-50 rounded">
            <Scissors size={14} />
            {t('14207')}
          </button>
