...
function TablePdsfcMatProgressList({
  setSelection,
  selection,
  setShowSearch,
  showSearch,
  ...
  const [hiddenColumns, setHiddenColumns] = useState(() => {
    return loadFromLocalStorageSheet('pdsfc_mat_progress_list_ah', [])
  })
  ...
  const getData = useCallback(
    ([col, row]) => {
      ...
      if (columnKey === "ProdQty" || columnKey === "OrderQty" || columnKey === "StdUnitQty"

        || columnKey === "ProgressQty" || columnKey === "OKQty" || columnKey === "BadQty"
        || columnKey === "NeedQty" || columnKey === "ReqQty"
        || columnKey === "OutQty" || columnKey === "InputQty" || columnKey === "ProdNeedQty"

        || columnKey === "STDStockQty"
      ) {
  ...
  const updateHiddenColumns = (newHiddenColumns) => {
    setHiddenColumns((prevHidden) => {
      ...
      saveToLocalStorageSheet('pdsfc_mat_progress_list_ah', newHidden)
      return newHidden
    })
  }
  ...
  const updateVisibleColumns = (newVisibleColumns) => {
    setCols((prevCols) => {
      ...
      saveToLocalStorageSheet('pdsfc_mat_progress_list_a', uniqueCols)
      return uniqueCols
    })
  }
  ...
  const handleReset = () => {
    ...
    setHiddenColumns([])
    localStorage.removeItem('pdsfc_mat_progress_list_a')
    localStorage.removeItem('pdsfc_mat_progress_list_ah')
    setShowMenu(null)
  }
  ...
  const onColumnMoved = useCallback((startIndex, endIndex) => {
    setCols((prevCols) => {
      ...
      saveToLocalStorageSheet('pdsfc_mat_progress_list_a', updatedCols)
      return updatedCols
    })
  }, [])
  ...
  const handleCheckboxChange = (columnId, isChecked) => {
    if (isChecked) {
      ...
      setCols((prevCols) => {
        ...
        saveToLocalStorageSheet('pdsfc_mat_progress_list_a', newCols)
        return newCols
      })
      setHiddenColumns((prevHidden) => {
  ...
  return (
    <div className="w-full  h-full flex items-center justify-center">
      <div className="w-full h-full flex flex-col bg-white overflow-x-hidden overflow-hidden ">
        <h2 className="text-[10px]  font-medium flex items-center gap-2 p-2 text-blue-600 uppercase">
          <TableOutlined />
          SHEET DATA
        </h2>
        <DataEditor
...
export default TablePdsfcMatProgressList
