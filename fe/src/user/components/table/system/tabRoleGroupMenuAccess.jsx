...
import { CellsHelpRootMenu } from '../../sheet/cells/help/cellHelpRootMenu'


function TabRoleGroupMenuAccess({
  setSelection,
  selection,
  setShowSearch,
  showSearch,
  ...
  const [hiddenColumns, setHiddenColumns] = useState(() => {
    return loadFromLocalStorageSheet('role_group_access_ah', [])
  })
  ...
  const columnNames = ['RootMenuLabel']
  ...
  const getData = useCallback(
    ([col, row]) => {
      ...
      const cellConfig = {
        RootMenuLabel: {
          kind: "cell-help-menu",
          allowedValues: dataHelp01,
          setCacheData: setDataHelp01
        },
      };
  ...
  const onCellEdited = useCallback(
    async (cell, newValue) => {
      ...
      if (key === "RootMenuLabel" && newValue.kind === GridCellKind.Custom) {
        let selectedName;
        ...
        setGridData((prev) => {
          ...
          if (selectedName) {
            const isDuplicate = newData.some((item, index) => item.RootMenuId === selectedName.Id && index !== row);
            if (isDuplicate) return prev;

            product[cols[col].id] = selectedName.Label;
            product["RootMenuId"] = selectedName.Id;
            product["Type"] = "rootmenu";
      ...
      if (key === 'Id' || key === 'RootMenuId' || key === 'GroupId' || key === 'Type') {
        return;
      }
    ...
    [cols, gridData, dataType, dataHelp01]
  )
  ...
  const updateHiddenColumns = (newHiddenColumns) => {
    setHiddenColumns((prevHidden) => {
      ...
      saveToLocalStorageSheet('role_group_access_ah', newHidden)
      return newHidden
    })
  }
  ...
  const updateVisibleColumns = (newVisibleColumns) => {
    setCols((prevCols) => {
      ...
      saveToLocalStorageSheet('role_group_access_a', uniqueCols)
      return uniqueCols
    })
  }
  // Hàm ẩn cột
  ...
  const handleReset = () => {
    ...
    setHiddenColumns([])
    localStorage.removeItem('role_group_access_a')
    localStorage.removeItem('role_group_access_ah')
  }
  ...
  const onColumnMoved = useCallback((startIndex, endIndex) => {
    setCols((prevCols) => {
      ...
      saveToLocalStorageSheet('role_group_access_a', updatedCols)
      return updatedCols
    })
  }, [])
  ...
  const showDrawer = () => {
    ...
    const currentVisibleCols = loadFromLocalStorageSheet('role_group_access_a', []).map(
      (col) => col.id
    )
  ...
  const handleCheckboxChange = (columnId, isChecked) => {
    if (isChecked) {
      ...
      setCols((prevCols) => {
        ...
        saveToLocalStorageSheet('role_group_access_a', newCols)
        return newCols
      })
      setHiddenColumns((prevHidden) => {
    ...
    <div className="w-full gap-1 border-r h-full flex items-center justify-center">
      <div className="w-full h-full flex flex-col  bg-white  overflow-hidden ">
        ...
        <DataEditor
          ...
          customRenderers={[CellsHelpRootMenu]}
        />
        {showMenu !== null &&
          renderLayer(
            <div
...
export default TabRoleGroupMenuAccess
