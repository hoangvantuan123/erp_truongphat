...
function TableDictSys({
  ...
  const [hiddenColumns, setHiddenColumns] = useState(() => {
    return loadFromLocalStorageSheet('H_ERP_COLS_PAGE_DICT_LIST', [])
  })
  ...
  const onCellEdited = useCallback(
    async (cell, newValue) => {
      ...
      if (dataType.length === 0) {
        message.warning('Vui lòng chọn ngôn ngữ')
        return;
      }
      ...
      if (key === 'LanguageSeq' || key === 'LanguageName') {
        return
      }
  ...
  const updateHiddenColumns = (newHiddenColumns) => {
    setHiddenColumns((prevHidden) => {
      ...
      saveToLocalStorageSheet('H_ERP_COLS_PAGE_DICT_LIST', newHidden)
      return newHidden
    })
  }
  ...
  const updateVisibleColumns = (newVisibleColumns) => {
    setCols((prevCols) => {
      ...
      saveToLocalStorageSheet('S_ERP_COLS_PAGE_DICT_LIST', uniqueCols)
      return uniqueCols
    })
  }
  // Hàm ẩn cột
  ...
  const handleReset = () => {
    ...
    setHiddenColumns([])
    localStorage.removeItem('S_ERP_COLS_PAGE_DICT_LIST')
    localStorage.removeItem('H_ERP_COLS_PAGE_DICT_LIST')
  }
  ...
  const onColumnMoved = useCallback((startIndex, endIndex) => {
    setCols((prevCols) => {
      ...
      saveToLocalStorageSheet('S_ERP_COLS_PAGE_DICT_LIST', updatedCols)
      return updatedCols
    })
  }, [])
  ...
  const showDrawer = () => {
    ...
    const currentVisibleCols = loadFromLocalStorageSheet('S_ERP_COLS_PAGE_DICT_LIST', []).map(
      (col) => col.id
    )
  ...
  const handleCheckboxChange = (columnId, isChecked) => {
    if (isChecked) {
      ...
      setCols((prevCols) => {
        ...
        saveToLocalStorageSheet('S_ERP_COLS_PAGE_DICT_LIST', newCols)
        return newCols
      })
      setHiddenColumns((prevHidden) => {
  ...
  return (
    <div className="w-full gap-1 h-full flex items-center justify-center border-l">
      <div className="w-full h-full flex flex-col  bg-white overflow-hidden ">
        ...
        <DataEditor
          ...
          onVisibleRegionChanged={({ y, height }) => {
            if (checkSearch && !loading && completedRequests < totalPages && y + height >= numRows - 130) {
              fetchAllDataDictSys(dataType[0]?.IdSeq, completedRequests + 1);
            }
          }}
