...
import { CellsAbsWkSort } from '../../../sheet/cells/CellAbsWkSort'
import { CellsLimitGrp } from '../../../sheet/cells/CellLimitGrp'
import { CellsAppMth } from '../../../sheet/cells/CellAppMth'
import { CellsWkMthGrp } from '../../../sheet/cells/CellWkMthGrp'

function DailyAtt01Table({
    setSelection,
    selection,
    setShowSearch,
    showSearch,
    ...
    const [hiddenColumns, setHiddenColumns] = useState(() => {
        return loadFromLocalStorageSheet('daily_att_01_h', [])
    })
    ...
    const booleanColumns = [
        'IsAbsReason',   // Có lý do nghỉ ngày làm không
        'IsLongAbs',     // Có hoặc không nghỉ làm trong thời gian dài
        'IsPaid',        // Có lương không
        'IsHalf',        // Có/không áp dụng nửa ngày phép
        'IsDeduc',       // Khấu trừ số tiền, giá trị
        'IsSat',         // Có/không áp dụng thứ 7
        'IsSun',         // Có hoặc không áp dụng chủ nhật
        'IsHoli',        // Có/không áp dụng ngày nghỉ
        'IsLimit',       // Có/không hạn mức
        'IsUse',         // Không sử dụng
                ...
                if (newValue.kind === GridCellKind.Custom) {
                    setGridData((prev) => {
                        ...
                        if (selectedName) {
                            ...
                            product['SMAbsWkSort'] = selectedName.Value
                        } else {
                            product[cols[col].id] = ''
                            product['SMAbsWkSort'] = ''
                        }
            ...
            if (key === 'AppMth') {
                if (newValue.kind === GridCellKind.Custom) {
                    setGridData((prev) => {
                        ...
                        if (selectedName) {
                            product[cols[col].id] = selectedName.MinorName
                            product['SMAppMth'] = selectedName.Value
                        } else {
                            product[cols[col].id] = ''
                            product['SMAppMth'] = ''
                        }
    ...
    const updateHiddenColumns = (newHiddenColumns) => {
        setHiddenColumns((prevHidden) => {
            ...
            saveToLocalStorageSheet('daily_att_01_h', newHidden)
            return newHidden
        })
    }
    ...
    const updateVisibleColumns = (newVisibleColumns) => {
        setCols((prevCols) => {
            ...
            saveToLocalStorageSheet('daily_att_01_a', uniqueCols)
            return uniqueCols
        })
    }
    ...
    const handleReset = () => {
        ...
        setHiddenColumns([])
        localStorage.removeItem('daily_att_01_a')
        localStorage.removeItem('daily_att_01_h')
    }
    ...
    const onColumnMoved = useCallback((startIndex, endIndex) => {
        setCols((prevCols) => {
            ...
            saveToLocalStorageSheet('daily_att_01_a', updatedCols)
            return updatedCols
        })
    }, [])
    ...
    const showDrawer = () => {
        ...
        const currentVisibleCols = loadFromLocalStorageSheet('daily_att_01_a', []).map(
            (col) => col.id
        )
    ...
    const handleCheckboxChange = (columnId, isChecked) => {
        if (isChecked) {
            ...
            setCols((prevCols) => {
                ...
                saveToLocalStorageSheet('daily_att_01_a', newCols)
                return newCols
            })
            setHiddenColumns((prevHidden) => {
        ...
        <div className="w-full gap-1 h-full flex items-center justify-center">
            <div className="w-full h-full flex flex-col  bg-white  overflow-hidden ">
                <div className='flex items-center px-2 p-1 justify-between border-b text-[10px] text-black  font-medium uppercase'>
                    <span>Đăng ký hạng mục chấm công ngày</span>
                </div>
                <DataEditor
                    {...cellProps}
                    ref={gridRef}
                    ...
                    onKeyUp={onKeyUp}
                    customRenderers={[CellsAbsWkSort, CellsLimitGrp, CellsAppMth, CellsWkMthGrp]}
...
export default DailyAtt01Table
