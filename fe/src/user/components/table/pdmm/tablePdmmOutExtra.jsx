  ...
  const highlightRegions = columnNames.flatMap((columnName) => {
    ...
    if (columnName === 'Memo2') {
      return gridData.map((row, rowIndex) => {
        const memo = row.Memo2 || '';
        const stockMatch = memo.match(/Tồn kho hiện tại:\s*(-?[\d.]+)/);
        const confirmedMatch = memo.match(/số lượng yêu cầu đã xác nhận:\s*(-?[\d.]+)/);
        const exportableMatch = memo.match(/số lượng có thể xuất theo yêu cầu này:\s*(-?[\d.]+)/);

        if (!stockMatch || !confirmedMatch || !exportableMatch) return null;

        const stockQty = parseFloat(stockMatch[1]) || 0;
        const confirmedQty = parseFloat(confirmedMatch[1]) || 0;
        const exportableQty = parseFloat(exportableMatch[1]) || 0;

        let color = '#ff0000'; // Màu đỏ mặc định

        if (exportableQty > 0) {
          color = '#52c41a'; // Màu xanh nếu có thể xuất
        } else if (confirmedQty > 0 && stockQty === 0) {
          color = '#FEAE00'; // Màu vàng nếu có yêu cầu nhưng không có tồn kho
        }
      ...
      const defaultColor = columnName === 'PrevQty' ? '#f0f0f0' : '#e8f0ff';
      return [
        {
          color: defaultColor,
          range: {
        ...
        if (newValue.kind === GridCellKind.Custom) {
          setGridData((prev) => {
            ...
            if (selectedName) {
              ...
              product['Memo1'] = selectedName.ProdInWhName
              product['Memo4'] = selectedName.MatOutWhSeq
              product['Memo5'] = selectedName.ProdInWhSeq
...
export default TablePdmmOutExtra
