...
function TablespdmmOutProcItemList({
  data,
  setSelection,
  selection,
  setShowSearch,
  ...
  const [hiddenColumns, setHiddenColumns] = useState(
    loadFromLocalStorageSheet('spdmm_out_proc_item_list_a_h', []),
  )
  ...
  const getData = useCallback(
    ([col, row]) => {
      ...
      if (row === lastRowIndex) {
        ...
        if (["MatReqQty"].includes(columnKey)) {
        ...
        if (["StdUnitQty"].includes(columnKey)) {
          const total = gridData.reduce((sum, item) => sum + (Number(item[columnKey]) || 0), 0);
          const formattedTotal = new Intl.NumberFormat("en-US", {
            minimumFractionDigits: 4,
            maximumFractionDigits: 4,
      ...
      if (
        ...
        columnKey === 'MatReqQty' || columnKey === 'ItemQty' || columnKey === 'StdUnitQty'
      ) {
        return {
          kind: GridCellKind.Number,
          data: value,
  ...
  const handleReset = () => {
    ...
    setHiddenColumns([])
    localStorage.removeItem('spdmm_out_proc_item_list_a')
    localStorage.removeItem('spdmm_out_proc_item_list_a_h')
  }
...
export default TablespdmmOutProcItemList
