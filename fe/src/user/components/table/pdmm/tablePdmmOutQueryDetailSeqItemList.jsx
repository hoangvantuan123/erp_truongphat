...
function TablePdmmOutQueryDetailSeqItemList({
  setSelection,
  selection,
  setShowSearch,
  showSearch,
  ...
  const [hiddenColumns, setHiddenColumns] = useState(() => {
    return loadFromLocalStorageSheet('pdmm_out_query_detail_list_seq_item_h', [])
  })
  ...
  const updateHiddenColumns = (newHiddenColumns) => {
    setHiddenColumns((prevHidden) => {
      ...
      saveToLocalStorageSheet('pdmm_out_query_detail_list_seq_item_h', newHidden)
      return newHidden
    })
  }
  ...
  const updateVisibleColumns = (newVisibleColumns) => {
    setCols((prevCols) => {
      ...
      saveToLocalStorageSheet('pdmm_out_query_detail_list_seq_item_a', uniqueCols)
      return uniqueCols
    })
  }
  // Hàm ẩn cột
  ...
  const handleReset = () => {
    ...
    setHiddenColumns([])
    localStorage.removeItem('pdmm_out_query_detail_list_seq_item_a')
    localStorage.removeItem('pdmm_out_query_detail_list_seq_item_h')
  }
  ...
  const onColumnMoved = useCallback((startIndex, endIndex) => {
    setCols((prevCols) => {
      ...
      saveToLocalStorageSheet('pdmm_out_query_detail_list_seq_item_a', updatedCols)
      return updatedCols
    })
  }, [])
  ...
  const handleCheckboxChange = (columnId, isChecked) => {
    if (isChecked) {
      ...
      setCols((prevCols) => {
        ...
        saveToLocalStorageSheet('pdmm_out_query_detail_list_seq_item_a', newCols)
        return newCols
      })
      setHiddenColumns((prevHidden) => {
  ...
  return (
    <div className="w-full h-full flex flex-col bg-white pb-[59px]">
      <div className="w-full h-full flex  border-r  border-l border-t flex-col  bg-white  overflow-hidden ">
        <h2 className="text-xs border-b font-medium flex items-center gap-2 p-2 text-blue-600 uppercase">
          <TableOutlined />
          THÔNG TIN XUẤT KHO SẢN PHẨM KHÁC
        </h2>
...
export default TablePdmmOutQueryDetailSeqItemList
