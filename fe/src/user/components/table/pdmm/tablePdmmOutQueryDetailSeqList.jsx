...
function TablePdmmOutQueryDetailSeqList({
  setSelection,
  selection,
  setShowSearch,
  showSearch,
  ...
  const gridRef = useRef(null) // Ref to the DataEditor
  ...
  const [hiddenColumns, setHiddenColumns] = useState(
    loadFromLocalStorageSheet('pdmm_out_query_detail_list_seq_h', []),
  )
  ...
  const updateHiddenColumns = (newHiddenColumns) => {
    setHiddenColumns((prevHidden) => {
      ...
      saveToLocalStorageSheet('pdmm_out_query_detail_list_seq_h', newHidden)
      return newHidden
    })
  }
  ...
  const updateVisibleColumns = (newVisibleColumns) => {
    setCols((prevCols) => {
      ...
      saveToLocalStorageSheet('pdmm_out_query_detail_list_seq_a', uniqueCols)
      return uniqueCols
    })
  }
  ...
  const handleReset = () => {
    ...
    setHiddenColumns([])
    localStorage.removeItem('pdmm_out_query_detail_list_seq_a')
    localStorage.removeItem('pdmm_out_query_detail_list_seq_h')
  }
  ...
  const onColumnMoved = useCallback((startIndex, endIndex) => {
    setCols((prevCols) => {
      ...
      saveToLocalStorageSheet('pdmm_out_query_detail_list_seq_a', updatedCols)
      return updatedCols
    })
  }, [])
  ...
  const handleCheckboxChange = (columnId, isChecked) => {
    if (isChecked) {
      ...
      setCols((prevCols) => {
        ...
        saveToLocalStorageSheet('pdmm_out_query_detail_list_seq_a', newCols)
        return newCols
      })
      setHiddenColumns((prevHidden) => {
  ...
  return (
    <div className="w-full gap-1 h-full flex items-center justify-center border-r">
      <div className="w-full h-full flex flex-col  bg-white overflow-hidden ">
        <h2 className="text-xs border-b font-medium flex items-center gap-2 p-2 text-blue-600 uppercase">
          <TableOutlined />
          THÔNG TIN YÊU CẦU DI CHUYỂN KHO
        </h2>
        <DataEditor
          {...cellProps}
          ref={gridRef}
...
export default TablePdmmOutQueryDetailSeqList
