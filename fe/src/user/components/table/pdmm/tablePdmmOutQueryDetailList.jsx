...
function TablePdmmOutQueryDetailList({
  data,
  setSelection,
  selection,
  setShowSearch,
  ...
  const [hiddenColumns, setHiddenColumns] = useState(
    loadFromLocalStorageSheet('pdmm_out_query_detail_list_h', []),
  )
  ...
  const getData = useCallback(
    ([col, row]) => {
      ...
      if (row === lastRowIndex) {
        ...
        if (["StockQty"].includes(columnKey)) {
          const total = gridData.reduce((sum, item) => sum + (Number(item[columnKey]) || 0), 0);
          const formattedTotal = new Intl.NumberFormat("en-US", {
            minimumFractionDigits: 4,
            maximumFractionDigits: 4,
      ...
      if (
        ...
        columnKey === 'STDQty' || columnKey === 'StockQty' || columnKey === 'LeftQty'
      ) {
        return {
          kind: GridCellKind.Number,
          data: value,
  ...
  const handleReset = () => {
    ...
    setHiddenColumns([])
    localStorage.removeItem('pdmm_out_query_detail_list_a')
    localStorage.removeItem('pdmm_out_query_detail_list_h')
  }
...
export default TablePdmmOutQueryDetailList
