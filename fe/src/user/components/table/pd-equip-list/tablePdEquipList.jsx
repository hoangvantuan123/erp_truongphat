...
import { CellsSMAddressType } from '../../sheet/cells/cellSMAddressType'
import moment from 'moment'
import { useTranslation } from 'react-i18next'

function TablePdEquipList({
    setSelection,
    selection,
    setShowSearch,
    showSearch,
    ...
    const [hiddenColumns, setHiddenColumns] = useState(() => {
        return loadFromLocalStorageSheet('pd_equip_list', [])
    })
        ...
        ([col, row]) => {
            ...
            if (columnKey === 'BuyCost') {
              ...
              return {
                ...
                data: value,
                copyData: formattedValue === '0.000' ? '' : String(value),
                displayData: formattedValue === '0.000' ? '' : formattedValue,
                readonly: column?.readonly || false,
                allowOverlay: true,
                hasMenu: column?.hasMenu || false,
                contentAlign: 'right',
            ...
            if (columnKey === 'IsMold' || columnKey === 'IsRegMold') {
              const booleanValue =
                value === 1 || value === '1'
                  ? true
                  : value === 0 || value === '0'
    ...
    const updateHiddenColumns = (newHiddenColumns) => {
        setHiddenColumns((prevHidden) => {
            ...
            saveToLocalStorageSheet('pd_equip_list', newHidden)
            return newHidden
        })
    }
    ...
    const updateVisibleColumns = (newVisibleColumns) => {
        setCols((prevCols) => {
            ...
            saveToLocalStorageSheet('emp_04_a', uniqueCols)
            return uniqueCols
        })
    }
    ...
    const handleReset = () => {
        ...
        setHiddenColumns([])
        localStorage.removeItem('emp_04_a')
        localStorage.removeItem('pd_equip_list')
    }
    ...
    const onColumnMoved = useCallback((startIndex, endIndex) => {
        setCols((prevCols) => {
            ...
            saveToLocalStorageSheet('emp_04_a', updatedCols)
            return updatedCols
        })
    }, [])
    ...
    const showDrawer = () => {
        ...
        const currentVisibleCols = loadFromLocalStorageSheet('emp_04_a', []).map(
            (col) => col.id
        )
    ...
    const handleCheckboxChange = (columnId, isChecked) => {
        if (isChecked) {
            ...
            setCols((prevCols) => {
                ...
                saveToLocalStorageSheet('emp_04_a', newCols)
                return newCols
            })
            setHiddenColumns((prevHidden) => {
                ...
                if (newValue.kind === GridCellKind.Custom) {
                    setGridData((prev) => {
                        ...
                        if (selectedName) {
                            ...
                            product['SMAddressType'] = selectedName.Value

                        } else {
                            product[cols[col].id] = ''
                            product['SMAddressType'] = ''
...
export default TablePdEquipList
