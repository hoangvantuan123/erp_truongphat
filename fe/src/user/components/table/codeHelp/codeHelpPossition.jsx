...
function CodeHelpPossition({
  ...
  useEffect(() => {
    if (possitionName) {
      setSearch(possitionName)
      setSearchSh(possitionName)
    }
  }, [possitionName])
  ...
  const handleCellClick = ([col, row]) => {
    ...
    if (dataClick[row]) {
      ...
      setPossitionName(selectedUserName)
      setPossitionSeq(dataClick[row].Value)
  ...
  const fetchCodeHelpDataUserSearch = useCallback(async (searchSh) => {
    try {
      const [data] = await Promise.all([
        GetCodeHelpVer2(10011, searchSh, '', '', '', '', '1', '', 1,'', 0, 0, 0),
      ])
      setFilteredData(
        data.data || [],
      )
  ...
  const onKeyDown = (e) => {
    ...
    if (e.key === 'Enter' && filteredData.length > 0) {
      ...
      setPossitionName(selectedUser?.MinorName)
      setPossitionSeq(selectedUser?.Value)
...
export default CodeHelpPossition
