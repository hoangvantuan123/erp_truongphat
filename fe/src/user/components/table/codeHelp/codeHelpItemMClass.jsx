...
function TableCodeHelpItemMClass({
  data,
  onCellClicked,
  setSelection,
  selection,
  ...
  const columns = useMemo(
    () => [
      { title: t('UMItemClass') },
      { title: t('ItemClassMName') },
      { title: t('ItemClassLName') },
    ],
    [],
  )
  ...
  const getData = useCallback(
    ([col, row]) => {
      ...
      const columnMap = {
        0: { kind: GridCellKind.Text, data: safeString(UMItemClass) },
        1: { kind: GridCellKind.Text, data: safeString(ItemClassMName) },
        2: { kind: GridCellKind.Text, data: safeString(ItemClassLName) },
      }
...
export default TableCodeHelpItemMClass
