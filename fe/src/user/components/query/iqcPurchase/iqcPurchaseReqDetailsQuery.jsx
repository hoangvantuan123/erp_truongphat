...
import { Button, Form, Input, Row, Col, DatePicker, Select, message, InputNumber } from 'antd'
import { CompactSelection } from '@glideapps/glide-data-grid'
import CodeHelpPeople from '../../table/codeHelp/codeHelpPeople'
import { useTranslation } from 'react-i18next'
export default function IqcPurchaseReqDetailQuery({
  dataMaster,
  dataSMQcType,
  dataSMAQLStrict,
  dataSMAQLLevelName,
    ...
    (data) => {
      ...
      if (itemSelect) {
        ...
        if (itemSelect.Value === 6035003) {
          setPassedQty(dataMaster.Qty)
          setReqInQty(dataMaster.Qty)
          setAcBadRatio(0)
          setRejectQty(0)
  ...
  const onChangeRejectQty = useCallback(
    (data) => {
      if (SMQcType === 6035006) {
        if (data > dataMaster.Qty || data < 0) {
          message.error(
            'Số lượng lỗi không lớn hơn hoặc nhỏ hơn số lượng kiểm tra',
          )
          setRejectQty(0)
          setAcBadRatio(0)
        } else {
          const passedQty = dataMaster.Qty - data
          setPassedQty(passedQty)
          setReqInQty(passedQty)
          setRejectQty(data)
          setAcBadRatio((passedQty / dataMaster.Qty) * 100)
        }
      }
    },

    [PassedQty, RejectQty, SMQcType],
  )
    ...
    (data) => {
      if (SMQcType === 6035006) {
        if (data > dataMaster.Qty || data < 0) {
          ...
          const rejectQty = dataMaster.Qty - data
          setPassedQty(data)
          setRejectQty(rejectQty)
          setReqInQty(data)
          setAcBadRatio((rejectQty / dataMaster.Qty) * 100)
        }
      }
    },

    [PassedQty, RejectQty, SMQcType, ReqInQty],
  )

  const onchangeRealSampleQty = (data) => {
    if (data < 0) {
      message.error('Số lượng mẫu thực không thể nhỏ hơn 0')
      setRealSampleQty(0)
    } else {
      setRealSampleQty(data)
    }
  }
  ...
  const onChangeDisposeQty = useCallback(
    (data) => {
      if (RejectQty < data || data < 0) {
        message.error('Số lượng hủy không lớn hơn hoặc nhỏ hơn số lượng lỗi')
        setDisposeQty(0)
      } else {
        setDisposeQty(data)
      }
  ...
  const onChangeSMAQLStrict = (data) => {
      ...
      <div className="flex p-2 gap-4 group [&_summary::-webkit-details-marker]:hidden border rounded-lg bg-white">
        <Form layout="vertical">
          <Row className="gap-4 flex items-center">
            ...
            <Col>
              <Form.Item
                label={<span className="uppercase text-[9px]">{t('698')}</span>}
                style={{ marginBottom: 0 }}
                labelCol={{ style: { marginBottom: 2, padding: 0 } }}
                wrapperCol={{ style: { padding: 0 } }}
              >
            ...
            <Col>
              <Form.Item
                label={
                  <span className="uppercase text-[10px]">{t('141')}</span>
                }
                style={{ marginBottom: 0 }}
                labelCol={{ style: { marginBottom: 2, padding: 0 } }}
                wrapperCol={{ style: { padding: 0 } }}
            ...
            <Col>
              <Form.Item
                label={
                  <span className="uppercase text-[9px]">{t('2336')}</span>
                }
                style={{ marginBottom: 0 }}
                labelCol={{ style: { marginBottom: 2, padding: 0 } }}
                wrapperCol={{ style: { padding: 0 } }}
            ...
            <Col>
              <Form.Item
                label={
                  <span className="uppercase text-[9px]">{t('25431')}</span>
                }
                style={{ marginBottom: 0 }}
                labelCol={{ style: { marginBottom: 2, padding: 0 } }}
                wrapperCol={{ style: { padding: 0 } }}
        ...
        <Form layout="vertical">
          <Row className="gap-4 flex items-center">
            <Col>
              <Form.Item
                label={
                  <span className="uppercase text-[9px]">{t('2219')}</span>
                }
                style={{ marginBottom: 0 }}
                labelCol={{ style: { marginBottom: 2, padding: 0 } }}
                wrapperCol={{ style: { padding: 0 } }}
            ...
            <Col>
              <Form.Item
                label={
                  <span className="uppercase text-[9px]">{t('2200')}</span>
                }
                style={{ marginBottom: 0 }}
                labelCol={{ style: { marginBottom: 2, padding: 0 } }}
                wrapperCol={{ style: { padding: 0 } }}
            ...
            <Col>
              <Form.Item
                label={
                  <span className="uppercase text-[10px]">{t('2192')}</span>
                }
                style={{ marginBottom: 0 }}
                labelCol={{ style: { marginBottom: 2, padding: 0 } }}
                wrapperCol={{ style: { padding: 0 } }}
            ...
            <Col>
              <Form.Item
                label={
                  <span className="uppercase text-[9px]">{t('2926')}</span>
                }
                style={{ marginBottom: 0 }}
                labelCol={{ style: { marginBottom: 2, padding: 0 } }}
                wrapperCol={{ style: { padding: 0 } }}
            ...
            <Col>
              <Form.Item
                label={
                  <span className="uppercase text-[9px]">{t('13933')}</span>
                }
                style={{ marginBottom: 0 }}
                labelCol={{ style: { marginBottom: 2, padding: 0 } }}
                wrapperCol={{ style: { padding: 0 } }}
            ...
            <Col>
              <Form.Item
                label={
                  <span className="uppercase text-[10px]">{t('2199')}</span>
                }
                style={{ marginBottom: 0 }}
                labelCol={{ style: { marginBottom: 2, padding: 0 } }}
                wrapperCol={{ style: { padding: 0 } }}
            ...
            <Col>
              <Form.Item
                label={
                  <span className="uppercase text-[9px]">{t('6995')}</span>
                }
                style={{ marginBottom: 0 }}
                labelCol={{ style: { marginBottom: 2, padding: 0 } }}
                wrapperCol={{ style: { padding: 0 } }}
            ...
            <Col>
              <Form.Item
                label={
                  <span className="uppercase text-[9px]">{t('18638')}</span>
                }
                style={{ marginBottom: 0 }}
                labelCol={{ style: { marginBottom: 2, padding: 0 } }}
                wrapperCol={{ style: { padding: 0 } }}
            ...
            <Col>
              <Form.Item
                label={
                  <span className="uppercase text-[9px]">{t('29792')}</span>
                }
                style={{ marginBottom: 0 }}
                labelCol={{ style: { marginBottom: 2, padding: 0 } }}
                wrapperCol={{ style: { padding: 0 } }}
            ...
            <Col>
              <Form.Item
                label={
                  <span className="uppercase text-[9px]">{t('15562')}</span>
                }
                style={{ marginBottom: 0 }}
                labelCol={{ style: { marginBottom: 2, padding: 0 } }}
                wrapperCol={{ style: { padding: 0 } }}
        ...
        <Form layout="vertical">
          <Row className="gap-4 flex items-center">
            ...
            <Col>
              <Form.Item
                label={
                  <span className="uppercase text-[9px]">{t('8017')}</span>
                }
                style={{ marginBottom: 0 }}
                labelCol={{ style: { marginBottom: 2, padding: 0 } }}
                wrapperCol={{ style: { padding: 0 } }}
            ...
            <Col>
              <Form.Item
                label={
                  <span className="uppercase text-[9px]">{t('2203')}</span>
                }
                style={{ marginBottom: 0 }}
                labelCol={{ style: { marginBottom: 2, padding: 0 } }}
                wrapperCol={{ style: { padding: 0 } }}
            ...
            <Col>
              <Form.Item
                label={
                  <span className="uppercase text-[9px]">{t('2201')}</span>
                }
                style={{ marginBottom: 0 }}
                labelCol={{ style: { marginBottom: 2, padding: 0 } }}
                wrapperCol={{ style: { padding: 0 } }}
            ...
            <Col>
              <Form.Item
                label={
                  <span className="uppercase text-[9px]">{t('6024')}</span>
                }
                style={{ marginBottom: 0 }}
                labelCol={{ style: { marginBottom: 2, padding: 0 } }}
                wrapperCol={{ style: { padding: 0 } }}
            ...
            <Col>
              <Form.Item
                label={
                  <span className="uppercase text-[9px]">{t('19799')}</span>
                }
                style={{ marginBottom: 0 }}
                labelCol={{ style: { marginBottom: 2, padding: 0 } }}
                wrapperCol={{ style: { padding: 0 } }}
            ...
            <Col>
              <Form.Item
                label={
                  <span className="uppercase text-[9px]">{t('19800')}</span>
                }
                style={{ marginBottom: 0 }}
                labelCol={{ style: { marginBottom: 2, padding: 0 } }}
                wrapperCol={{ style: { padding: 0 } }}
            ...
            <Col>
              <Form.Item
                label={
                  <span className="uppercase text-[9px]">{t('14831')}</span>
                }
                style={{ marginBottom: 0 }}
                labelCol={{ style: { marginBottom: 2, padding: 0 } }}
                wrapperCol={{ style: { padding: 0 } }}
