...
export default function ImpOrderQuery({
  fromDate,
  setFromDate,
  ...
  const [currNameSearch, setCurrNameSearch] = useState('')
  const [currNameSearchSh, setCurrNameSearchSh] = useState('')
  ...
  const [filteredCurrNameData, setFilteredCurrNameData] = useState([])
  ...
  const [dropdownCurrNameVisible, setDropdownCurrNameVisible] = useState(false)
  ...
  const [selectionCurrName, setSelectionCurrName] = useState({
    columns: CompactSelection.empty(),
    rows: CompactSelection.empty(),
  })
  ...
  const defaultCurrNameCols = [
    ...
    {
      title: t('Tỷ giá tiền gửi ngân hàng'),
      id: 'SendBank',
      kind: 'Text',
      readonly: true,
      width: 150,
  ...
  const [colsCurrName, setColsCurrName] = useState(defaultCurrNameCols)
  const onCurrNameFill = useOnFill(filteredCurrNameData, colsCurrName)
  ...
  useEffect(() => {
    ...
    setFilteredCurrNameData(dataCurrName)
  }, [dataWarehouse, dataDeptName, dataUserName, dataCustName, dataCurrName])
  ...
  useEffect(() => {
    ...
    if (currName) {
      setCurrNameSearch(currName)
      setCurrNameSearchSh(currName)
    }
  }, [userName, deptName, whName, custName, currName])
  ...
  const handleCurrNameSearch = (e) => {
    const value = e.target.value
    setCurrNameSearchSh(value)
    if (value.trim() === '' || value === null) {
      setCurrNameSearch('')
      setCurrName('')
      setCurrSeq('0')
      ...
      const filtered = dataCurrName.filter((item) =>
        item.CurrName.toLowerCase().includes(value.toLowerCase()),
      )
      setFilteredCurrNameData(filtered)
    }
    setDropdownCurrNameVisible(true)
  }
  ...
  const handleCurrNameCellClick = ([col, row]) => {
    const data =
      currNameSearchSh.trim() === '' ? dataCurrName : filteredCurrNameData
    if (data[row]) {
      const selectedCurrName = data[row].CurrName
      setCurrName(data[row].CurrName)
      setCurrSeq(data[row].CurrSeq)
      setExRate(data[row].RateBuy)
      setBasicAmt(data[row].BasicAmt)

      setCurrNameSearch(selectedCurrName)
      setCurrNameSearchSh(selectedCurrName)
      setDropdownCurrNameVisible(false)
    }
  }
  ...
  const getCurrNameData = useCallback(
    ([col, row]) => {
      const person = filteredCurrNameData[row] || {}
      const column = colsCurrName[col]
      const columnKey = column?.id || ''
      const value = person[columnKey] || ''
    ...
    [filteredCurrNameData, colsCurrName],
  )
  ...
  const onColumnCurrNameResize = useCallback(
    (column, newSize) => {
      const index = colsCurrName.indexOf(column)
      if (index !== -1) {
        const newCol = {
          ...column,
          width: newSize,
        }
        const newCols = [...colsCurrName]
        newCols.splice(index, 1, newCol)
        setColsCurrName(newCols)
      }
    },
    [colsCurrName],
  )
  ...
  const handleChangePurTypeName = (value) => {
    setPurTypeName(value)
  }
  const handleChangeUMPriceTermsName = (value) => {
    setUMPriceTermsName(value)
  }
  const handleChangeUMPayment1Name = (value) => {
    setUMPayment1Name(value)
  }
  const handleChangeUMPayment2Name = (value) => {
    setUMPayment2Name(value)
  }
  const handleChangeUMShipViaName = (value) => {
    setUMShipViaName(value)
  }
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      >
        <Row className="gap-4 flex items-center mb-2 ">
          <Col>
            <Form.Item
          ...
          <Col>
            <Form.Item
              label={<span className="uppercase text-[10px]">{t('651')}</span>}
              className="mb-2"
              style={{ marginBottom: 0 }}
              labelCol={{ style: { marginBottom: 2, padding: 0 } }}
              wrapperCol={{ style: { padding: 0 } }}
          ...
          <Col>
            <Form.Item
              label={<span className="uppercase text-[10px]">{t('2443')}</span>}
              className="mb-2"
              style={{ marginBottom: 0 }}
              labelCol={{ style: { marginBottom: 2, padding: 0 } }}
              wrapperCol={{ style: { padding: 0 } }}
          ...
          <Col>
            <Form.Item
              label={<span className="uppercase text-[10px]">{t('629')}</span>}
              required
              className="mb-2"
              style={{ marginBottom: 0 }}
              labelCol={{ style: { marginBottom: 2, padding: 0 } }}
            ...
            >
              <Select
                ...
                value={purTypeName === '0' ? undefined : purTypeName}
                onChange={handleChangePurTypeName}
                options={dataPurTypeName?.map((item) => ({
          ...
          <Col>
            <Form.Item
              label={<span className="uppercase text-[10px]">{t('731')}</span>}
              className="mb-2"
              style={{ marginBottom: 0 }}
              labelCol={{ style: { marginBottom: 2, padding: 0 } }}
              wrapperCol={{ style: { padding: 0 } }}
        ...
        <Row className="gap-4 flex items-center mb-2 ">
            ...
            >
              <div className="relative">
                <Input
                  placeholder=""
                  size="middle"
                  value={currNameSearch}
                  //onChange={handleSearch}
                  onFocus={() => setDropdownCurrNameVisible(true)}
                  style={{ backgroundColor: '#e8f0ff', width: 85 }}
                />
                {dropdownCurrNameVisible && (
                  <div
                    ref={dropdownRef}
                    className="  fixed z-50 w-auto bg-white border border-gray-300 rounded-lg top-[25vh] left-[50%] transform -translate-x-1/2"
                  >
                    <div className="flex items-center justify-between p-1">
                      ...
                      <Button
                        type="text"
                        icon={<CloseOutlined />}
                        onClick={() => setDropdownCurrNameVisible(false)}
                      />
                    </div>
                    <div className="p-2 border-b border-t">
                      <div className="w-full flex gap-2">
                        <SearchOutlined className="opacity-80 size-5" />
                        <input
                          value={currNameSearchSh}
                          onChange={handleCurrNameSearch}
                          onFocus={() => setDropdownCurrNameVisible(true)}
                          onKeyDown={(e) => {
                            if (
                              e.key === 'Enter' &&
                              filteredCurrNameData.length > 0
                            ) {
                              setCurrName(filteredCurrNameData[0].CurrName)
                              setCurrSeq(filteredCurrNameData[0].CurrSeq)
                              setCurrNameSearch(
                                filteredCurrNameData[0].CurrName,
                              )
                              setCurrNameSearchSh(
                                filteredCurrNameData[0].CurrName,
                              )

                              setExRate(filteredCurrNameData[0].BuyRate)
                              setBasicAmt(filteredCurrNameData[0].BasicAmt)
                    ...
                    <DataEditor
                      ...
                      width={950}
                      height={500}
                      onFill={onCurrNameFill}
                      className="cursor-pointer rounded-md"
                      rows={filteredCurrNameData.length}
                      columns={colsCurrName}
                      gridSelection={selectionCurrName}
                      onGridSelectionChange={setSelectionCurrName}
                      getCellsForSelection={true}
                      getCellContent={getCurrNameData}
                      getRowThemeOverride={(i) =>
                        i === hoverRow
                          ? {
                              bgCell: '#e8f0ff',
                      ...
                      isDraggable={false}
                      onItemHovered={onItemHovered}
                      onCellClicked={handleCurrNameCellClick}
                      freezeColumns="0"
                      onColumnResize={onColumnCurrNameResize}
                      rowMarkers={('checkbox-visible', 'both')}
                      rowSelect="single"
                    />
                  </div>
          ...
          <Col>
            <Form.Item
              label={<span className="uppercase text-[9px]">{t('783')}</span>}
              className="mb-2"
              style={{ marginBottom: 0 }}
              labelCol={{ style: { marginBottom: 2, padding: 0 } }}
              wrapperCol={{ style: { padding: 0 } }}
          ...
          <Col>
            <Form.Item
              label={
                <span className="uppercase text-[10px]">{t('31720')}</span>
              }
              className="mb-2"
              style={{ marginBottom: 0 }}
              labelCol={{ style: { marginBottom: 2, padding: 0 } }}
            ...
            >
              <div className="relative">
                <Input
                  ...
                  style={{ backgroundColor: '#e8f0ff', width: 395 }}
                />
          ...
          <Col>
            <Form.Item
              label={<span className="uppercase text-[10px]">{t('93')}</span>}
              required
              className="mb-2"
              style={{ marginBottom: 0 }}
              labelCol={{ style: { marginBottom: 2, padding: 0 } }}
            ...
            >
              <Select
                ...
                value={uMPriceTermsName === '0' ? undefined : uMPriceTermsName}
                onChange={handleChangeUMPriceTermsName}
                options={dataUMPriceTermsName?.map((item) => ({
                  label: item?.MinorName,
                  value: item?.Value,
                }))}
              />
        ...
        <Row className="gap-4 flex items-center ">
          <Col>
            <Form.Item
              label={<span className="uppercase text-[10px]">{t('2764')}</span>}
              className="mb-2"
              style={{ marginBottom: 0 }}
              labelCol={{ style: { marginBottom: 2, padding: 0 } }}
              wrapperCol={{ style: { padding: 0 } }}
            >
              <Select
                ...
                value={uMPayment1Name === '0' ? undefined : uMPayment1Name}
                onChange={handleChangeUMPayment1Name}
                options={dataUMPayment1Name?.map((item) => ({
                  label: item?.MinorName,
                  value: item?.Value,
                }))}
              />
          ...
          <Col>
            <Form.Item
              label={<span className="uppercase text-[10px]">{t('2515')}</span>}
              className="mb-2"
              style={{ marginBottom: 0 }}
              labelCol={{ style: { marginBottom: 2, padding: 0 } }}
              wrapperCol={{ style: { padding: 0 } }}
            >
              <Select
                ...
                value={uMPayment2Name === '0' ? undefined : uMPayment2Name}
                onChange={handleChangeUMPayment2Name}
                options={dataUMPayment2Name?.map((item) => ({
                  label: item?.MinorName,
                  value: item?.Value,
                }))}
              />
          ...
          <Col>
            <Form.Item
              label={<span className="uppercase text-[10px]">{t('2763')}</span>}
              className="mb-2"
              style={{ marginBottom: 0 }}
              labelCol={{ style: { marginBottom: 2, padding: 0 } }}
              wrapperCol={{ style: { padding: 0 } }}
          ...
          <Col>
            <Form.Item
              label={
                <span className="uppercase text-[10px]">{t('30756')}</span>
              }
              className="mb-2"
              style={{ marginBottom: 0 }}
              labelCol={{ style: { marginBottom: 2, padding: 0 } }}
            ...
            >
              <Select
                ...
                value={uMShipViaName === '0' ? undefined : uMShipViaName}
                onChange={handleChangeUMShipViaName}
                options={dataUMShipViaName?.map((item) => ({
                  label: item?.MinorName,
                  value: item?.Value,
                }))}
              />
