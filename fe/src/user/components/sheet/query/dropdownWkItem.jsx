...
const DropdownWkItem = ({ helpData, setSearchText, searchText, setItemText, setDataSearch, setDropdownVisible, dropdownVisible }) => {
    ...
    const defaultCols = [
        ...
        {
            title: t('Có áp dụng phép nửa ngày không'),
            id: 'IsHalf',
            kind: 'Text',
            readonly: true,
            width: 200,
    ...
    const handleSearch = (e) => {
        ...
        if (value.trim() === '' || value === null) {
            ...
            const filtered = helpData.filter(
                (item) =>
                    item.WkItemName.toLowerCase().includes(value.toLowerCase()) ||
                    item.WkItemSeq.toLowerCase().includes(value.toLowerCase()) ||
                    item.SMDTCType.toLowerCase().includes(value.toLowerCase()) ||
                    item.IsHalf.toLowerCase().includes(value.toLowerCase())
            );
    ...
    const handleCellClick = ([col, row]) => {
        ...
        if (data[row]) {
            const ItemName = data[row].WkItemName;
