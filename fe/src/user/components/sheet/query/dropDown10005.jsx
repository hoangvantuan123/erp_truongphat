...
const Dropdown10005 = ({
    ...
    const columnDefs = useMemo(() => [
        { title: t('363'), id: 'CurrName', kind: 'Text', readonly: true, width: 300 },
    ], [t]);
    ...
    const handleSearch = (e) => {
        ...
        if (!value.trim()) {
            ...
            setFilteredData(
                helpData.filter(
                    (item) =>
                        item.CurrName?.toLowerCase().includes(lower)
                )
            );
        }
    ...
    const handleCellClick = ([, row]) => {
        ...
        setSearchText(data.CurrName || '');
        setItemText(data.CurrName || '');
        setDataSearch(data);
        setDropdownVisible(false);
    };
