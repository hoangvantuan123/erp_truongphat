...
const Dropdown40003 = ({
    ...
    const columnDefs = useMemo(() => [
        { title: t('291'), id: 'RemName', kind: 'Text', readonly: true, width: 500 },
        { title: t('RemSeq'), id: 'RemSeq', kind: 'Text', readonly: true, width: 200 },
    ], [t]);
    ...
    const handleSearch = (e) => {
        ...
        if (!value.trim()) {
            ...
            setFilteredData(
                helpData.filter(
                    (item) =>
                        item.RemName?.toLowerCase().includes(lower)
                        || item.RemSeq?.toLowerCase().includes(lower)
    ...
    const handleCellClick = ([, row]) => {
        ...
        setSearchText(data.RemName || '');
        setItemText(data.RemName || '');
        setDataSearch(data);
        setDropdownVisible(false);
    };
