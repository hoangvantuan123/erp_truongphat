...
const DropdownUser = ({ helpData, setHelpData05, setSearchText, searchText, setItemText, setSearchText1, setDataSearch, setDataSearchDept, setDataSheetSearch, setDropdownVisible, dropdownVisible }) => {
    ...
    const debounceCallGetCodeHelp = debounce(async (key) => {
        ...
        try {
            const result = await GetCodeHelpVer2(10009, key, '', '', '', '', '', 1, 0, '', 0, 0, 0, signal);
            if (result && result.data) {
                setHelpData05((prev) => {
                    const safePrev = prev ?? [];
                    const existingItemSeqs = new Set(safePrev.map((item) => item.EmpSeq));
                    const newData = result.data.filter((item) => !existingItemSeqs.has(item.EmpSeq));
    ...
    const handleSearch = useCallback((e) => {
        ...
        const propertiesToSearch = ["EmpName", "EmpSeq", "EmpID"];
    ...
    const handleCellClick = ([col, row]) => {
        ...
        if (data[row]) {
            ...
            setSearchText1(data[row].DeptName);
            setItemText(ItemName);
            setDataSearch(data[row]);
            setDataSearchDept(data[row]);
