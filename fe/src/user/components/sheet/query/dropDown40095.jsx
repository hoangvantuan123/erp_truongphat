...
const Dropdown40095 = ({
    ...
    const columnDefs = useMemo(() => [
        { title: t('30857'), id: 'FSItemName', kind: 'Text', readonly: true, width: 300 },

        { title: t('30863'), id: 'AccNameTo', kind: 'Text', readonly: true, width: 200 },
        { title: t('2174'), id: 'AccNoTo', kind: 'Text', readonly: true, width: 100 },
        { title: t('30859'), id: 'AccNameFr', kind: 'Text', readonly: true, width: 200 },
        { title: t('30860'), id: 'AccNoFr', kind: 'Text', readonly: true, width: 100 },
    ...
    const handleSearch = (e) => {
        ...
        if (!value.trim()) {
            ...
            setFilteredData(
                helpData.filter(
                    (item) =>
                        item.FSItemName?.toLowerCase().includes(lower)
                        || item.AccNoTo?.toLowerCase().includes(lower)
                        || item.AccNameFr?.toLowerCase().includes(lower)
                        || item.AccNoFr?.toLowerCase().includes(lower)
                        || item.AccNameTo?.toLowerCase().includes(lower)
                )
            );
        }
    ...
    const handleCellClick = ([, row]) => {
        ...
        setSearchText(data.FSItemName || '');
        setItemText(data.FSItemName || '');
        setDataSearch(data);
        setDropdownVisible(false);
    };
