...
import { isPageLoading } from '../../../../utils/togglePageInteraction';
import { useTranslation } from 'react-i18next'
import { GetCodeHelpVer2 } from '../../../../features/codeHelp/getCodeHelpVer2';

const DropdownWarehouse = ({ helpData, setHelpData01, setSearchText, searchText, setItemText, setDataSearch, setDataSheetSearch, setDropdownVisible, dropdownVisible }) => {
    ...
    const debounceCallGetCodeHelp = debounce(async (key) => {
        ...
        try {
            const result = await GetCodeHelpVer2(10006,
                '',
                '',
                '',
                '',
            ...
            if (result && result.data) {
                setHelpData01((prev) => {
                    if (!result?.data?.length) return prev;

                    const existingItemSeqs = new Set(prev.map((item) => item.WHSeq));

                    const newData = [];
                    for (const item of result.data) {
                        if (item.WHSeq && !existingItemSeqs.has(item.WHSeq)) {
                            existingItemSeqs.add(item.WHSeq);
                            newData.push(item);
                        }
                    }
                    return [...prev, ...newData];
    ...
    const handleSearch = useCallback((e) => {
        ...
        const propertiesToSearch = ["WHName", "WHSeq", "BizUnitName"];
    ...
    const handleCellClick = ([col, row]) => {
        ...
        if (data[row]) {
            const ItemName = data[row].WHName;
...
export default DropdownWarehouse;