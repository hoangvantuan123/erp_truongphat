...
const DropdownAsset = ({
  ...
  const debounceCallGetCodeHelp = debounce(async (key) => {
    ...
    try {
      const result = await GetCodeHelpVer230427(
        40007,
        key,
        '',
        '',
      ...
      if (result && result.data) {
        ...
        // setHelpData05((prev) => {
        //   const safePrev = prev ?? []
        //   const existingItemSeqs = new Set(safePrev.map((item) => item.AsstSeq))
  ...
  const handleCellClick = ([col, row]) => {
    const data = AsstName?.trim() === '' ? helpData : filteredData
    if (data[row]) {
      const ItemName = data[row].AsstName
      setAsstName(ItemName)
      setAsstSeq(data[row].AsstSeq)
      setAsstNo(data[row].AsstNo)
      ...
      <div className="p-2 border-b border-t">
        <div className="w-full flex gap-2">
          <button
            onClick={() => {
              if (!isLoading) {
                debounceCallGetCodeHelp(AsstName)
              }
            }}
