...
const Dropdown40002 = ({
    ...
    const columnDefs = useMemo(() => [
        { title: t('8'), id: 'AccName', kind: 'Text', readonly: true, width: 300 },
        { title: t('2174'), id: 'AccNo', kind: 'Text', readonly: true, width: 100 },
        { title: t('586'), id: 'SMDrOrCrName', kind: 'Text', readonly: true, width: 200 },
        { title: t('508'), id: 'SMAccKindName', kind: 'Text', readonly: true, width: 100 },
    ], [t]);
    ...
    const handleSearch = (e) => {
        ...
        if (!value.trim()) {
            ...
            setFilteredData(
                helpData.filter(
                    (item) =>
                        item.AccNo?.toLowerCase().includes(lower)
                        || item.AccName?.toLowerCase().includes(lower)
                        || item.SMDrOrCrName?.toLowerCase().includes(lower)
                        || item.SMAccKindName?.toLowerCase().includes(lower)
                )
            );
        }
    ...
    const handleCellClick = ([, row]) => {
        ...
        setSearchText(data.AccName || '');
        setItemText(data.AccName || '');
        setDataSearch(data);
        setDropdownVisible(false);
    };
