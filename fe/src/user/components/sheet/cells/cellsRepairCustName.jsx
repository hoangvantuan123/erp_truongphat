...
const Editor = (p) => {
  ...
  const filteredData = (allowedValues || []).filter((item) => {
    ...
    const propertiesToSearch = ['CustName', 'CustNo', 'BizNo'];
...
export const CellsRepairCustName = {
  kind: GridCellKind.Custom,
  isMatch: (c) => c.data.kind === 'cell-RepairCustName',
  draw: (args, cell) => {
  ...
  onPaste: (v, d) => {
    ...
    const matchedValues = d.allowedValues.filter((item) =>
      ['CustName', 'CustNo', 'BizNo'].some((attr) => {
        const value = item[attr]
