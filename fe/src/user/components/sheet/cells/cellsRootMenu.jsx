...
const Editor = (p) => {
  ...
  const filteredData = Array.isArray(allowedValues)
    ? allowedValues.filter((item) => {
      ...
      const propertiesToSearch = ['Label', 'Key'];
  ...
  const [cols, setCols] = useState([
    { title: t('Label'), width: 250 },
    { title: t('Key'), width: 250 },
    ...
    <div ref={dropdownRef} className="">
      ...
      <DataEditor
        ...
        getCellContent={([col, row]) => {
          ...
          switch (col) {
            case 0:
              cellData = filteredData[row].Label || ''
              break
            default:
              cellData = filteredData[row].Key || ''
              break
          }
...
export const CellsRootName = {
  kind: GridCellKind.Custom,
  isMatch: (c) => c.data.kind === 'root-menu-help-cell',
  draw: (args, cell) => {
