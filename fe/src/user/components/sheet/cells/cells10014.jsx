...
const Editor = (p) => {
  ...
  const filteredData = Array.isArray(allowedValues)
    ? allowedValues.filter((item) => {
      ...
      const propertiesToSearch = ['ItemClassName', 'UMItemClass' , 'Value', 'ItemClassLName' , 'ItemClassMName', 'ItemClassLSeq' , 'ItemClassMSeq'];
  ...
  const [cols, setCols] = useState([
    { title: t('ItemClassName'), width: 250 },
    { title: t('UMItemClass'), width: 250 },
    { title: t('Value'), width: 250 },
    { title: t('ItemClassLName'), width: 250 },
    { title: t('ItemClassMName'), width: 250 },
    { title: t('ItemClassLSeq'), width: 250 },
    { title: t('ItemClassMSeq'), width: 250 },
  ...
  return (
    <div ref={dropdownRef} className="">
      <h2 className="text-xs font-medium flex items-center gap-2 p-2 text-blue-600 uppercase">
        <TableOutlined />
    PHÂN LOẠI NHỎ DANH MỤC SẢN PHẨM
      </h2>
        ...
        getCellContent={([col, row]) => {
          ...
          switch (col) {
            case 0:
              ...
              break
            case 1:
              cellData = filteredData[row].UMItemClass 
              break
            case 2:
              cellData = filteredData[row].Value 
              break
            case 3:
            ...
            case 4:
              ...
              break
            default:
              cellData = filteredData[row].ItemClassMSeq 
              break
          }
...
export const Cells10014 = {
  kind: GridCellKind.Custom,
  isMatch: (c) => c.data.kind === 'cell-10014',
  draw: (args, cell) => {
  ...
  onPaste: (v, d) => {
    ...
    const matchedValues = d.allowedValues.filter((item) =>
      ['ItemClassName', 'UMItemClass' , 'Value', 'ItemClassLName' , 'ItemClassMName', 'ItemClassLSeq' , 'ItemClassMSeq'].some((attr) => {
        const value = item[attr];
