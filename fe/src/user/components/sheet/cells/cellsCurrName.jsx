...
const Editor = (p) => {
  ...
  const filteredData = allowedValues.filter((item) => {
    ...
    const propertiesToSearch = ['CurrName']
  ...
  const [cols, setCols] = useState([
    { title: t('Mã tiền tệ'), width: 100 },
    { title: t('Tên tiền tệ'), width: 150 },
    { title: t('Ngày quy đổi tỷ giá'), width: 150 },
    { title: t('Tỷ giá tiền gửi ngân hàng'), width: 200 },
    { title: t('Tỷ giá mua hàng'), width: 200 },
    { title: t('Tỷ giá bán hàng'), width: 200 },
    { title: t('Tỷ giá bán tiền mặt'), width: 200 },
    { title: t('BasicAmt'), width: 200 },
  ])
    ...
    <div ref={dropdownRef} className="">
      ...
      <DataEditor
        ...
        getCellContent={([col, row]) => {
          ...
          switch (col) {
            case 0:
              cellData = filteredData[row].CurrSeq || ''
              break
            case 1:
              cellData = filteredData[row].CurrName || ''
              break
            case 2:
              cellData = filteredData[row].ExRateDate || ''
              break
            case 3:
              cellData = filteredData[row].SendBank || ''
              break
            case 4:
              cellData = filteredData[row].RateBuy || ''
              break
            case 5:
              cellData = filteredData[row].RateSell || ''
              break
            case 6:
              cellData = filteredData[row].WithdrawBank || ''
              break
            default:
              cellData = filteredData[row].BasicAmt || ''
              break
          }
...
export const CellsCurrName = {
  kind: GridCellKind.Custom,
  isMatch: (c) => c.data.kind === 'curr-name-cell',
  draw: (args, cell) => {
