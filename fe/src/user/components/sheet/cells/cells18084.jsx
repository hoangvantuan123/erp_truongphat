...
const Editor = (p) => {
    ...
    const filteredData = Array.isArray(allowedValues)
        ? allowedValues.filter((item) => {
            ...
            const propertiesToSearch = ['ItemName', 'ItemSeq', 'ItemNo', 'Spec', 'ItemClassLName', 'ItemClassMName', 'ItemClassName', 'UnitName'];
    ...
    const [cols, setCols] = useState([
        { title: 'Tên sản phẩm', width: 80 },
        { title: 'Mã tên sản phẩm', width: 240 },
        { title: 'Số tên sản phẩm', width: 120 },
        { title: 'Quy cách', width: 120 },
        { title: 'Tên tài sản', width: 120 },
        { title: 'Phân loại lớn', width: 240 },
        { title: 'Phân loại TB', width: 240 },
        { title: 'Phân loại nhỏ', width: 240 },
        { title: 'Đơn vị', width: 240 },
        { title: 'Đơn vị tiêu chuẩn', width: 240 },
        ...
        <div ref={dropdownRef} className="">
            ...
            <DataEditor
                ...
                getCellContent={([col, row]) => {
                    ...
                    switch (col) {
                        case 0:
                            cellData = filteredData[row].ItemName
                            break
                        case 1:
                            cellData = filteredData[row].ItemSeq
                            break
                        case 2:
                            cellData = filteredData[row].ItemNo
                            break
                        case 3:
                            cellData = filteredData[row].Spec
                            break
                        case 4:
                            cellData = filteredData[row].AssetName
                            break
                        case 5:
                            cellData = filteredData[row].ItemClassLName
                            break
                        case 6:
                            cellData = filteredData[row].ItemClassMName
                            break
                        case 7:
                            cellData = filteredData[row].ItemClassName
                            break
                        case 8:
                            cellData = filteredData[row].UnitName
                            break
                        default:
                            cellData = filteredData[row].STDUnitName
                            break
                    }
...
export const Cells18084 = {
    kind: GridCellKind.Custom,
    isMatch: (c) => c.data.kind === 'cell-18084',
    draw: (args, cell) => {
    ...
    onPaste: (v, d) => {
        ...
        const matchedValues = d.allowedValues.filter((item) =>
            ['ItemName', 'ItemSeq', 'ItemNo', 'Spec', 'ItemClassLName', 'ItemClassMName', 'ItemClassName', 'UnitName'].some((attr) => {
                const value = item[attr];
