...
const Editor = (p) => {
  ...
  const filteredData = allowedValues.filter((item) => {
    ...
    const propertiesToSearch = ['UMQCTitleName', 'UMQCTitleSeq', 'MinorName']
  ...
  return (
    <div className="w-full ">
      ...
      <DataEditor
        ...
        columns={[
          { title: 'Mã kiểm tra', width: 100 },
          { title: 'Hạng mục kiểm tra', width: 240 },
          { title: 'Phân loại nhập kho', width: 240 },
        ]}
        getCellContent={([col, row]) => {
          let cellData
          if (col === 0) {
            cellData = filteredData[row].UMQCTitleSeq ||filteredData[row].Value
          } else if (col === 1) {
            cellData = filteredData[row].UMQCTitleName || filteredData[row].MinorName
          } else {
            cellData = filteredData[row].SMInputTypeName || ''
          }
...
export const CellsUMQcTitleName = {
  kind: GridCellKind.Custom,
  isMatch: (c) => c.data.kind === 'mng-umqc-title-name-cell',
  draw: (args, cell) => {
  ...
  onPaste: (v, d) => {
    ...
    const matchedValues = d.allowedValues.filter((item) =>
      ['UMQCTitleName', 'UMQCTitleSeq'].some(
