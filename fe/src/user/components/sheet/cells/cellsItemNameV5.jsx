...
export const Editor = (p) => {
  ...
  const filteredData = useMemo(() => {
    ...
    const propertiesToSearch = ["ItemName", "ItemNo", "Spec", "UnitName"];
...
export const CellsItemNameV5 = {
  kind: GridCellKind.Custom,
  isMatch: (c) => c.data.kind === 'item-name-cell-v5',
  draw: (args, cell) => {
  ...
  onPaste: (v, d) => {
    ...
    const matchedValues = d.allowedValues.filter((item) =>
      ["ItemName", "ItemNo", "Spec", "UnitName"].some((attr) => {
        const value = item[attr]
