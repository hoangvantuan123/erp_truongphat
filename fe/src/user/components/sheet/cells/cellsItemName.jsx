...
const Editor = (p) => {
  ...
  const [cols, setCols] = useState([
    { title: t('Mã sản phẩm (Seq)'), width: 150 },
    { title: t('Tên sản phẩm'), width: 250 },
    { title: t('Mã sản phẩm'), width: 100 },
    { title: t('Quy cách'), width: 200 },
    { title: t('Đơn vị'), width: 200 },
  ])
    ...
    <div ref={dropdownRef} className="">
      ...
      <DataEditor
        ...
        getCellContent={([col, row]) => {
          ...
          switch (col) {
            case 0:
              cellData = filteredData[row]?.ItemSeq || 0
              break
            case 1:
...
export const CellsItemName = {
  kind: GridCellKind.Custom,
  isMatch: (c) => c.data.kind === 'item-name-cell',
  draw: (args, cell) => {
  ...
  onPaste: (v, d) => {
    ...
    const matchedValues = d.allowedValues.filter((item) =>
      ['ItemName', 'ItemSeq', 'ItemNo', 'Spec'].some((attr) => {
        const value = item[attr]
