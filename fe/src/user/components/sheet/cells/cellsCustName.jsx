...
const Editor = (p) => {
  ...
  const filteredData = allowedValues.filter((item) => {
    ...
    const propertiesToSearch = ['CustName', 'CustSeq', 'CustNo']
  ...
  const [cols, setCols] = useState([
    { title: t('Mã Khách hàng (Seq)'), width: 150 },
    { title: t('Tên khách hàng'), width: 250 },
    { title: t('Mã khách hàng'), width: 100 },
    { title: t('Mã số kinh doanh'), width: 200 },
    { title: t('Địa chỉ'), width: 200 },
  ])
...
export const CellsCustName = {
  kind: GridCellKind.Custom,
  isMatch: (c) => c.data.kind === 'cust-name-cell',
  draw: (args, cell) => {
  ...
  /*  onPaste: (v, d) => {
       const pastedValue = String(v).toString().trim().toLowerCase();
       const matchedValues = d.allowedValues.filter(item =>
           item.ItemName.toString().toLowerCase().trim() === pastedValue ||
           String(item.ItemSeq).toLowerCase().trim() === pastedValue ||
           String(item.ItemNo).toString().toLowerCase().trim() === pastedValue ||
           String(item.Spec).toString().toLowerCase().trim() === pastedValue
       );
       return matchedValues;
   } */
}
