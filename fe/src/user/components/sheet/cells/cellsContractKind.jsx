...
export const Editor = (p) => {
    ...
    const debounceCallGetCodeHelp = useMemo(() =>
        debounce(async (key) => {
            ...
            try {
                const result = await GetCodeHelpCombo('', 6, 19998, 1, '%', '1000447', '', '', '', signal)
    ...
    const finalFilteredData = useMemo(() => {
        ...
        const newData = extraData.filter((item) => !existingItemSeqs.has(item.EmpSeq));
...
export const CellsContractKind = {
    kind: GridCellKind.Custom,
    isMatch: (c) => c.data.kind === 'cells-contract-kind',
    draw: (args, cell) => {
    ...
    onPaste: async (v, d) => {
        ...
        if (typeof d.setCacheData === "function") {
            d.setCacheData((prev) => {
                ...
                const newData = matchedValuesFromApi.filter((item) => !existingItemSeqs.has(item.Value));
                return [...prev, ...newData];
            });
        }
...
export const callGetCodeHelp = async (key, d) => {
    ...
    try {
        const controller = new AbortController();
        const result = await GetCodeHelpVer2(10009, key, '', '', '', '', '', 1, 0, '', 0, 0, 0, controller.signal);
        ...
        if (typeof d.setCacheData === "function") {
            d.setCacheData((prev) => {
                const existingItemSeqs = new Set(prev.map((item) => item.EmpSeq));
                const newData = data.filter((item) => !existingItemSeqs.has(item.EmpSeq));
                return [...prev, ...newData];
            });
        }
