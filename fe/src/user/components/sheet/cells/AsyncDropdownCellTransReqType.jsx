    ...
    <div ref={dropdownRef} className="">
      ...
      <DataEditor
        ...
        columns={[
          ...
          { title: "MinorName", width: 240 }
        ]}
        getCellContent={([col, row]) => {
          const cellData = col === 0
            ? filteredData[row].Value
            : filteredData[row].MinorName || "";
...
export const AsyncDropdownCellTransReqType = {
  kind: GridCellKind.Custom,
  isMatch: (c) => c.data.kind === "async-dropdown-trans-req-type-cell",
  draw: (args, cell) => {
  ...
  onPaste: (v, d) => {
    ...
    const matchedValue = d.allowedValues.find(item =>
      item.MinorName.toLowerCase().trim() === pastedValue
    );
    return { 
      ...d, 
      value: matchedValue ? matchedValue.MinorName : '',
      InOutReqDetailKind: matchedValue ? matchedValue.InOutReqDetailKind : '',
    };
  }
};