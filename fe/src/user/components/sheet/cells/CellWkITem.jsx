...
const Editor = (p) => {
  ...
  const defaultCols = useMemo(
    () => [
      { title: 'Giá trị', id: 'WkItemName', kind: 'Text', readonly: true, width: 200 },
      { title: 'Mã giá trị', id: 'WkItemSeq', kind: 'Text', readonly: true, width: 200 },
      { title: 'Type', id: 'SMDTCType', kind: 'Text', readonly: true, width: 200 },
  ...
  const filteredData = (allowedValues || []).filter((item) => {
    ...
    const propertiesToSearch = ['WkItemName', 'WkItemSeq', 'SMDTCType'];
...
export const CellsWkItem = {
  kind: GridCellKind.Custom,
  isMatch: (c) => c.data.kind === 'cell-wk-item',
  draw: (args, cell) => {
  ...
  onPaste: (v, d) => {
    ...
    const matchedValues = d.allowedValues.filter((item) =>
      ['WkItemName', 'WkItemSeq', 'SMDTCType'].some((attr) => {
        const value = item[attr]
