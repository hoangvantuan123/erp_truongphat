...
const Editor = (p) => {
  ...
  const filteredData = allowedValues.filter((item) => {
    ...
    const propertiesToSearch = ['FactUnitName', 'FactUnit']
  ...
  return (
    <div ref={dropdownRef} className="">
      ...
      <DataEditor
        ...
        columns={[
          { title: "FactUnit", width: 100 },
          { title: "FactUnitName", width: 240 }
        ]}
        getCellContent={([col, row]) => {
          const cellData = col === 0
            ? filteredData[row].FactUnit
            : filteredData[row].FactUnitName || "";
...
export const CellsFactUnit = {
  kind: GridCellKind.Custom,
  isMatch: (c) => c.data.kind === "fact-unit-cell",
  draw: (args, cell) => {
