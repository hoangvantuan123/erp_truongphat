...
export const Editor = (p) => {
  ...
  const filteredData = useMemo(() => {
    ...
    const propertiesToSearch = ['UMCostItemName', 'MinorName', 'Value']
  ...
  const finalFilteredData = useMemo(() => {
    ...
    const newData = extraData.filter(
      (item) => !existingItemSeqs.has(item.Value),
    )
...
export const CellsCostItem = {
  kind: GridCellKind.Custom,
  isMatch: (c) => c.data.kind === 'cells-cost-item',
  draw: (args, cell) => {
  ...
  onPaste: (v, d) => {
    ...
    const matchedValues = d.allowedValues.filter((item) =>
      ['UMCostItemName', 'UMCostItem'].some((attr) => {
        const value = item[attr]
