...
const Editor = (p) => {
    ...
    const filteredData = allowedValues.filter((item) => {
        ...
        const propertiesToSearch = ['BeDeptName', 'BeDeptSeq' , 'DeptRemark']
    ...
    return (
        <div className="w-full "  >
            ...
            <DataEditor
                ...
                columns={[
                    { title: "BeDeptSeq", width: 100 },
                    { title: "BeDeptName", width: 240 },
                    { title: "DeptRemark", width: 240 }
                ]}
                getCellContent={([col, row]) => {
                    let cellData;
                    if (col === 0) {
                        cellData = filteredData[row].BeDeptSeq;
                    } else if (col === 1) {
                        cellData = filteredData[row].BeDeptName || "";
                    } else {
                        cellData = filteredData[row].DeptRemark || "";
                    }
...
export const CellsMngDeptName = {
    kind: GridCellKind.Custom,
    isMatch: (c) => c.data.kind === "mng-dept-name-cell",
    draw: (args, cell) => {
    ...
    onPaste: (v, d) => {
        ...
        const matchedValues = d.allowedValues.filter((item) =>
          ['BeDeptName', 'BeDeptSeq' , 'DeptRemark' ].some(
