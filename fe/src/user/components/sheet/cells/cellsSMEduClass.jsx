...
export const Editor = (p) => {
    ...
    const debounceCallGetCodeHelp = useMemo(() =>
        debounce(async (key) => {
            ...
            try {
                const result = await GetCodeHelpComboVer2('', 6, 19999, 1, key, '3908','','','', controller.signal);
    ...
    const filteredData = useMemo(() => {
        ...
        const propertiesToSearch = ["SMTermType", "SMTermTypeName",'MinorName', 'Value'];
    ...
    const finalFilteredData = useMemo(() => {
        ...
        if(filteredData.length === 0) return extraData
        const existingItemSeqs = new Set(filteredData.map((item) => item?.Value));
        const newData = extraData.filter((item) => !existingItemSeqs.has(item.Value));

        setCacheData((prev) => {
            const existingItemSeqs = new Set(prev.map((item) => item.Value));
            const newData = extraData?.filter((item) => !existingItemSeqs.has(item.Value));
            return [...prev, ...newData];
        });
    ...
    return (
        <div ref={dropdownRef} className="">
            ...
            <DataEditor
                ...
                rows={finalFilteredData?.length || 0}
                columns={cols}
...
export const CellsSmEduClass = {
    kind: GridCellKind.Custom,
    isMatch: (c) => c.data.kind === 'cells-sm-edu-class',
    draw: (args, cell) => {
...
export const callGetCodeHelp = async (key, d) => {
    ...
    try {
        const controller = new AbortController();
        const result = await GetCodeHelpComboVer2('',
          6,
          19998,
          1,
          key,
        ...
        if (typeof d.setCacheData === "function") {
            d.setCacheData((prev) => {
                const existingItemSeqs = new Set(prev.map((item) => item.Value));
                const newData = data.filter((item) => !existingItemSeqs.has(item.Value));
                return [...prev, ...newData];
            });
        }
