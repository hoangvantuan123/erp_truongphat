...
const Editor = (p) => {
  ...
  const filteredData = Array.isArray(allowedValues)
    ? allowedValues.filter((item) => {
      ...
      const propertiesToSearch = ['BeDeptSeq', 'BeBegDate', 'BeDeptName', 'BeEndDate', 'DeptRemark', 'IsUse', 'OnlySlipCCtrName', 'OnlySlipCCtrSeq'];

      const newFilteredData = propertiesToSearch.some((attr) => {
...
export const Cells10010 = {
  kind: GridCellKind.Custom,
  isMatch: (c) => c.data.kind === 'cell-10010',
  draw: (args, cell) => {
  ...
  onPaste: (v, d) => {
    ...
    const matchedValues = d.allowedValues.filter((item) =>
      ['BeDeptSeq', 'BeBegDate', 'BeDeptName', 'BeEndDate', 'DeptRemark', 'IsUse', 'OnlySlipCCtrName', 'OnlySlipCCtrSeq'].some((attr) => {
        const value = item[attr];
