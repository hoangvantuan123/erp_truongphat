...
const Editor = (p) => {
    ...
    const filteredData = allowedValues.filter((item) => {
        ...
        const propertiesToSearch = ['DefineItemName', 'Value']
...
export const Cells30005 = {
    kind: GridCellKind.Custom,
    isMatch: (c) => c.data.kind === 'cell-30005',
    draw: (args, cell) => {
    ...
    onPaste: (v, d) => {
        ...
        const matchedValues = d.allowedValues.filter((item) =>
            ['DefineItemName', 'Value',].some((attr) => {
                const value = item[attr]
