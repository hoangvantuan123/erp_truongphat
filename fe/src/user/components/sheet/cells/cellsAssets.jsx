...
const Editor = (p) => {
  ...
  const defaultCols = useMemo(
    () => [
      { title: 'Tên thiết bị', id: 'ToolName', kind: 'Text', readonly: true, width: 200 },
      { title: 'Số thiết bị', id: 'ToolNo', kind: 'Text', readonly: true, width: 200 },
      { title: 'Quy cách thiết bị', id: 'Spec', kind: 'Text', readonly: true, width: 200 },
      { title: 'Vị trí hiện tại', id: 'PrePlace', kind: 'Text', readonly: true, width: 200 },
      { title: 'Chủng loại thiết bị', id: 'UMToolKindName', kind: 'Text', readonly: true, width: 200 },
      { title: 'Tên nhân viên', id: 'EmpName', kind: 'Text', readonly: true, width: 200 },
      { title: 'Bộ phận', id: 'DeptName', kind: 'Text', readonly: true, width: 200 },
    ],
    []
  );
  ...
  const filteredData = (allowedValues || []).filter((item) => {
    ...
    const propertiesToSearch = ['ToolName', 'ToolNo', 'Spec', 'PrePlace', 'UMToolKindName', 'EmpName', 'DeptName'];
...
export const CellsAssets = {
  kind: GridCellKind.Custom,
  isMatch: (c) => c.data.kind === 'cell-Assets',
  draw: (args, cell) => {
  ...
  onPaste: (v, d) => {
    ...
    const matchedValues = d.allowedValues.filter((item) =>
      ['ToolName', 'ToolNo', 'Spec', 'PrePlace', 'UMToolKindName', 'EmpName', 'DeptName'].some((attr) => {
        const value = item[attr]
