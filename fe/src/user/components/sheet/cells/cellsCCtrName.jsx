...
export const Editor = (p) => {
    ...
    const defaultCols = useMemo(() => [
        ...
        {
            title: t('Phân loại chi phí'),
            id: 'UMSlipCostTypeName',
            kind: 'Text',
            readonly: true,
            width: 200,
            hasMenu: true,
        ...
        debounce(async (key) => {
            ...
            try {
                const result = await GetCodeHelpVer2(
                    50001, cleanedKey, "", "", "", "", "1", 1, 50, "IsNotUse <> ''1''", '', '', '', controller.signal
                );
    ...
    const filteredData = useMemo(() => {
        ...
        const propertiesToSearch = ["Value", "MinorName"];
...
export const CellsCCtrName = {
    kind: GridCellKind.Custom,
    isMatch: (c) => c.data.kind === 'cells-cctr-name',
    draw: (args, cell) => {
...
export const callGetCodeHelp = async (key, d) => {
    ...
    try {
        ...
        const controller = new AbortController();
        const result = await GetCodeHelpVer2(
            60011, cleanedKey, "", "", "", "", "1", 1, 50, "IsComplete = ''0''", '', '', '', controller.signal
        );
