...
import { CellsHelpMenu } from '../../sheet/cells/help/cellHelpMenu'
import { getHelpMenu } from '../../../../features/help/getHelpMenu'


function TabRoleMenuAccessPermiss({
  setSelection,
  selection,
  setShowSearch,
  showSearch,
  ...
  const [hiddenColumns, setHiddenColumns] = useState(() => {
    return loadFromLocalStorageSheet('role_group_menu_ah', [])
  })
  ...
  const getData = useCallback(
    ([col, row]) => {
      ...
      const cellConfig = {
        MenuLabel: {
          kind: "cell-menu",
          allowedValues: dataHelp02,
          setCacheData: setDataHelp02
        },
      };
      ...
      if (
        columnKey === 'View' || columnKey === 'Create' || columnKey === 'Edit' || columnKey === 'Delete'
  ...
  const callGetCodeHelpNow = async (key) => {
    ...
    try {
      const result = await getHelpMenu(1, 1000, key, controller.signal);
  ...
  const onCellEdited = useCallback(
    async (cell, newValue) => {
      ...
      if (key === "MenuLabel" && newValue.kind === GridCellKind.Custom) {
        let selectedName;
        ...
        setGridData((prev) => {
          ...
          if (selectedName) {
            const isDuplicate = newData.some((item, index) => item.MenuId === selectedName.Id && index !== row);
            if (isDuplicate) return prev;

            product[cols[col].id] = selectedName.Label;
            product["MenuId"] = selectedName.Id;
            product["Type"] = "menu";
            product["GroupId"] = dataType[0].Id;
            product["MenuType"] = selectedName.Type;
          } else {
            product[cols[col].id] = "";
            product["MenuId"] = "";
            product["Type"] = "";
      ...
      if (key === 'Id' || key === 'MenuId' || key === 'GroupId' || key === 'Type' || key === 'MenuType') {
        return;
      }
    ...
    [cols, gridData, dataType, dataHelp02]
  )
  ...
  const updateHiddenColumns = (newHiddenColumns) => {
    setHiddenColumns((prevHidden) => {
      ...
      saveToLocalStorageSheet('role_group_menu_ah', newHidden)
      return newHidden
    })
  }
  ...
  const updateVisibleColumns = (newVisibleColumns) => {
    setCols((prevCols) => {
      ...
      saveToLocalStorageSheet('role_group_menu_a', uniqueCols)
      return uniqueCols
    })
  }
  // Hàm ẩn cột
  ...
  const handleReset = () => {
    ...
    setHiddenColumns([])
    localStorage.removeItem('role_group_menu_a')
    localStorage.removeItem('role_group_menu_ah')
  }
  ...
  const onColumnMoved = useCallback((startIndex, endIndex) => {
    setCols((prevCols) => {
      ...
      saveToLocalStorageSheet('role_group_menu_a', updatedCols)
      return updatedCols
    })
  }, [])
  ...
  const showDrawer = () => {
    ...
    const currentVisibleCols = loadFromLocalStorageSheet('role_group_menu_a', []).map(
      (col) => col.id
    )
  ...
  const handleCheckboxChange = (columnId, isChecked) => {
    if (isChecked) {
      ...
      setCols((prevCols) => {
        ...
        saveToLocalStorageSheet('role_group_menu_a', newCols)
        return newCols
      })
      setHiddenColumns((prevHidden) => {
...
export default TabRoleMenuAccessPermiss
