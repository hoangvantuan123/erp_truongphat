...
const item = filteredItems.length > 0 
  ? filteredItems.reduce((minItem, currentItem) => (currentItem?.PermitSerl < minItem?.PermitSerl ? currentItem : minItem))
  : null;
...
if (parts.length === 5) { // Xử lý barcode có 5 phần
  ...
  return {
      ...
      data: {
          ...
          SMImpKind: item?.SMImpKind,
          SMImpKindName: item?.SMImpKindName,
          Price: item?.Price,
          CurAmt: qty * item?.Price,
          InOutType: 240,
          PermitQty: item?.Qty,
          PermitAmt: item?.CurAmt,
      },
  };
