...
export class HrContractPrintController {
    constructor(private readonly contractPrintService: HrContractPrintService, private readonly databaseService: DatabaseService) { }
    ...
    @GrpcMethod('HrContractPrintService', 'printContract')
    printContract(request: any): Observable<MetadataResponse> {
        return this.handleGrpcRequest(request, this.contractPrintService.printContract.bind(this.contractPrintService));
    }

    @GrpcMethod('HrContractPrintService', 'printBasCertificate')
    printBasCertificate(request: any): Observable<MetadataResponse> {
        return this.handleGrpcRequest(request, this.contractPrintService.printBasCertificate.bind(this.contractPrintService));
    }
