...
@Controller('secure-file-item')
export class SecureFileItemController {
    private readonly basePath: string;
    private readonly publicDir = join(__dirname, '..', '..', '..', 'public');

    constructor(private readonly configService: ConfigService) {
        this.basePath = this.configService.get<string>(
            'FILE_STORAGE_BASE_SYSTEM_ITEM_PDF_PATH',
        );
    }

    @Options('system/items/pdf/:date/:printId/:filename')
    @HttpCode(204)
    handlePreflight(
    ...
    @Get('system/items/pdf/:date/:printId/:filename')
    async getFile(
        @Param('date') date: string,
