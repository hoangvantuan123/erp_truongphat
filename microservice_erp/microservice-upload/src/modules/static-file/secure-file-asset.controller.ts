...
@Controller('secure-file-asset')
export class SecureFileAssetController {
    private readonly basePath: string;
    private readonly publicDir = join(__dirname, '..', '..', '..', 'public');

    constructor(private readonly configService: ConfigService) {
        this.basePath = this.configService.get<string>(
            'FILE_STORAGE_BASE_SYSTEM_ASSETS_FILE_PATH',
        );
    }

    @Get('system/assets/files/:assetSeq/:date/:filename')
    async getFile(
        @Param('date') date: string,
        @Param('assetSeq') assetSeq: string,
        @Param('filename') filename: string,
        @Res() res: Response,
        @Req() req: Request,
    ) {
        try {
            const rawPath = join(this.basePath, assetSeq, date, filename);
