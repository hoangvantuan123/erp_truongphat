...
export class DatabaseService {
  ...
  executeQueryVer03(query: string, useTransaction = false, priority = 200): Observable<any> {
    return new Observable(subscriber => {
      ...
      const config = {
        ...
        options: {
          encrypt: false, // tuỳ server
          enableArithAbort: true,
          trustServerCertificate: true // cần nếu là dev
        }
      };
      ...
      pool.connect()
        ...
        .then(result => {
          const endTime = Date.now();
          this.logger.log(`✅ [DONE] Query ${queryId} - Time: ${endTime - startTime}ms - Active Queries: ${this.activeQueries - 1}`);
          subscriber.next({ success: true, recordsets: result.recordsets });
          subscriber.complete();
        })
        .catch(error => {
          this.logger.error(`❌ [FAILED] Query ${queryId} - Error: ${error.message}`);
