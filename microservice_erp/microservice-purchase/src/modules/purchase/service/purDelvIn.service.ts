...
export class PurDelvInService {
  ...
  async SPUDelvInMasterQueryWEB(
    ...
    const xmlDocument =
      await this.generateXmlService.generateXMLSPUDelvInMasterQueryWEB(result);
    const query = `
      EXEC _SPUDelvInQuery_WEB
  ...
  async SPUDelvInSheetQueryWEB(
    ...
    const xmlDocument =
      await this.generateXmlService.generateXMLSPUDelvInSheetQueryWEB(result);
    const query = `
  EXEC _SPUDelvInItemQuery_WEB
    @xmlDocument = N'${xmlDocument}',
    @xmlFlags = 2,
    @ServiceSeq = 2548,
    @WorkingTag = N'',
...
`;
  ...
  async SPUDelvInMasterLinkQueryWEB(
    ...
    const xmlDocument =
      await this.generateXmlService.generateXMLSPUDelvInMasterLinkQueryWEB(
        result,
      );
    const query = `
  EXEC _SPUDelvInLinkQuery_WEB
    @xmlDocument = N'${xmlDocument}',
    @xmlFlags = 2,
    @ServiceSeq = 2608,
    @WorkingTag = N'',
...
`;
  ...
  async SPUDelvInSheetLinkQueryWEB(
    ...
    const xmlDocument =
      await this.generateXmlService.generateXMLSPUDelvInSheetLinkQueryWEB(
        result,
      );
    const query = `
EXEC _SPUDelvInItemLinkQuery_WEB
@xmlDocument = N'${xmlDocument}',
@xmlFlags = 2,
@ServiceSeq = 2608,
@WorkingTag = N'',
    ...
    const checkResult = async () => {
      try {
        const xmlDocumentCheck =
          await this.generateXmlService.generateXMLSPurDelvInQRCheckWEB(result);
        const queryCheck = generateQuery(
          xmlDocumentCheck,
          '_SPurDelvInQRCheck_WEB',
        );
    ...
    const saveResult = async () => {
      try {
        ...
        const xmlDocumentCloseCheck =
          await this.generateXmlService.generateXMLSCOMCloseItemCheckWEBDelvIn(
            dataMaster,
            workingTag,
          );
        ...
        const xmlDocumentMasterCheck =
          await this.generateXmlService.generateXMLSPUDelvInMasterCheckWEB(
            dataMaster,
            workingTag,
          );
        ...
        if (dataSheetAUD.length > 0) {
          const dataSheetAUDUpdate = dataSheetAUD.map((item) => {
            return {
              ...
              DelvInSeq:
                item.DelvInSeq === '0'
                  ? resultMasterCheck[0].DelvInSeq
                  : item.DelvInSeq,
              WHSeq: dataMaster[0].WHSeq,
              DelvInDate: resultMasterCheck[0].DelvInDate,
              BizUnit: resultMasterCheck[0].BizUnit,
              DelvInNo: resultMasterCheck[0].DelvInNo,
              CustSeq: resultMasterCheck[0].CustSeq,
            };
          });

          const xmlDocumentSheetCheck =
            await this.generateXmlService.generateXMLSPUDelvInSheetCheckWEB(
              dataSheetAUDUpdate,
            );
          const querySheetCheck = generateQuery(
            xmlDocumentSheetCheck,
            '_SPUDelvInItemCheck_WEB',
          );
          ...
          const xmlDocumentMasterSave =
            await this.generateXmlService.generateXMLSPUDelvInMasterSaveWEB(
              resultMasterCheck,
            );
          ...
          const xmlDocumentSheetSave =
            await this.generateXmlService.generateXMLSPUDelvInSheetSaveWEB(
              resultSheetCheck,
            );
          const querySheetSave = generateQuery(
            xmlDocumentSheetSave,
            '_SPUDelvInItemSave_WEB',
          );
          ...
          const dataSheetAUDUpdateSerl = dataSheetAUDUpdate.map((item) => {
            ...
            return {
              ...item,
              DelvInSerl: matchingResult
                ? matchingResult.DelvInSerl
                : item.DelvInSerl,
            };
          });
          ...
          const xmlDocumentDailyBatchSave =
            await this.generateXmlService.generateXMLSLGInOutDailyBatchWEBDelvIn(
              resultMasterSave,
            );
          ...
          const dataResultSheetSave = resultSheetSave.map((item) => {
            ...
            return {
              ...item,
              DelvInNo: resultMasterSave[0].DelvInNo,
              DelvSeq: matchingResult ? matchingResult.DelvSeq : item.DelvSeq,
              DelvSerl: matchingResult
                ? matchingResult.DelvSerl
                : item.DelvSerl,
              DelvQty: matchingResult ? matchingResult.DelvQty : item.DelvQty,
              DelvAmt: matchingResult ? matchingResult.DelvAmt : item.DelvAmt,
