...
@Injectable()
export class ImpPermitItemListService {
    constructor(private readonly databaseService: DatabaseService, private readonly generateXmlService: GenerateXmlService) { }

    async SSLImpPermitItemListQuery(result: any[], companySeq: number, userSeq: number, pgmSeq: number): Promise<SimpleQueryResult> {
        const xmlDocument = await this.generateXmlService.generateXMLSSLImpPermitItemListQueryWEB(result);
        const query = `
      EXEC _SSLImpPermitItemListQuery_WEB
