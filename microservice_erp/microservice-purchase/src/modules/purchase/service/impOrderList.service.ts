...
@Injectable()
export class ImpOrderListService {
  constructor(
  ...
  async SIMPORDERListQueryWEB(
    ...
    const xmlDocument =
      await this.generateXmlService.generateXMLSIMPORDERListQueryWEB(result);
    const query = `
  EXEC _SSLImpOrderListQuery_WEB
  ...
  async SPOStopWEB(
    ...
    const checkResult = async () => {
      try {
        ...
        const queryCheck = generateQuery(xmlDocumentCheck, '_SPUORDPOStop_WEB');
        const resultCheck = await this.databaseService.executeQuery(queryCheck);
        const invalidStatuses = resultCheck.some(
          (item: any) => item.Status !== 0,
        );
