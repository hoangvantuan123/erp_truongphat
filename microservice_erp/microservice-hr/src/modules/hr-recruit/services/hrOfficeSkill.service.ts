...
import { HrOfficeSkillsRecruit } from '../entities/hr.officeSkills.recruit.entity';
@Injectable()
export class ErpOfficeSkillsRecruitService {
    constructor(
    ...
    HrOfficeSkillsA(result: any[]): Observable<any> {
    ...
    HrOfficeSkillsU(result: any[]): Observable<any> {
        if (!result || result.length === 0) {
    ...
    HrOfficeSkillsD(records: any[]): Observable<any> {
        ...
        return from(this.dataSource.transaction(async (manager) => {
            try {
                ...
                const result = await manager.delete(HrOfficeSkillsRecruit, { IdSeq: In(ids) });
    ...
    HrOfficeSkillsQ(result: any): Observable<any> {
        if (!result) {
            return of({
                success: false,
        ...
        return from(this.dataSource.transaction(async (manager) => {
            try {
                const queryBuilder = manager.createQueryBuilder(HrOfficeSkillsRecruit, 'q')
                    .select([
                        'q.IdSeq as "IdSeq"',
                        'q.IdxNo as "IdxNo"',
                        'q.EmpSeq as "EmpSeq"',
                        ...
                        'q.SkillNameSeq as "SkillNameSeq"',
                        'q.SkillTypeSeq as "SkillTypeSeq"',
                        'q.SkillLevelHigh as "SkillLevelHigh"',
                        'q.SkillLevelMedium as "SkillLevelMedium"',
                        'q.SkillLevelLow as "SkillLevelLow"',
                        ...
                        's3.DefineItemName as "SkillNameName"',
                        's4.DefineItemName as "SkillTypeName"',
                    ])
                    ...
                    .leftJoin(
                        "_ERPDefineItem",
                        's3',
                        'q."SkillNameSeq" = s3."IdSeq"',
                    )
                    .leftJoin(
                        "_ERPDefineItem",
                        's4',
                        'q."SkillTypeSeq" = s4."IdSeq"',
                    )
