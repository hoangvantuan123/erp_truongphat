...
@Injectable()
export class ErpFamilyRecruitService {
    constructor(
    ...
    FamilyRecruitA(result: any[]): Observable<any> {
    ...
    FamilyRecruitU(result: any[]): Observable<any> {
        if (!result || result.length === 0) {
    ...
    FamilyRecruitD(records: any[]): Observable<any> {
        ...
        return from(this.dataSource.transaction(async (manager) => {
            try {
                ...
                const result = await manager.delete(ERPFamilyRecruit, { IdSeq: In(ids) });
    ...
    FamilyRecruitQ(result: any): Observable<any> {
        if (!result) {
            return of({
                success: false,
        ...
        return from(this.dataSource.transaction(async (manager) => {
            try {
                const queryBuilder = manager.createQueryBuilder(ERPFamilyRecruit, 'q')
                    .select([
                        'q.IdSeq as "IdSeq"',
                        'q.IdxNo as "IdxNo"',
                        'q.EmpSeq as "EmpSeq"',
