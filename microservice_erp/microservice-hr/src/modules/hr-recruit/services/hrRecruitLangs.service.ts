...
import { ERPLangsRecruit } from '../entities/hr.langs.recruit.entity';
@Injectable()
export class ErpLangsRecruitService {
    constructor(
    ...
    HrLangsRecruitA(result: any[]): Observable<any> {
    ...
    HrLangsRecruitU(result: any[]): Observable<any> {
        if (!result || result.length === 0) {
    ...
    HrLangsRecruitD(records: any[]): Observable<any> {
        ...
        return from(this.dataSource.transaction(async (manager) => {
            try {
                ...
                const result = await manager.delete(ERPLangsRecruit, { IdSeq: In(ids) });
    ...
    HrLangsRecruitQ(result: any): Observable<any> {
        if (!result) {
            return of({
                success: false,
        ...
        return from(this.dataSource.transaction(async (manager) => {
            try {
                const queryBuilder = manager.createQueryBuilder(ERPLangsRecruit, 'q')
                    .select([
                        'q.IdSeq as "IdSeq"',
                        'q.IdxNo as "IdxNo"',
                        'q.EmpSeq as "EmpSeq"',
                        ...
                        'q.LanguageSeq as "LanguageSeq"',
                        'q.CertifiTypeSeq as "CertifiTypeSeq"',
                        'q.Score as "Score"',
                        'q.ProfiLevelSeq as "ProfiLevelSeq"',
                        'q.Notes as "Notes"',
                        'q.IsLanguageBonus as "IsLanguageBonus"',
                        ...
                        's2.DefineItemName as "ProfiLevelName"',
                        's3.DefineItemName as "LanguageName"',
                        's4.DefineItemName as "CertifiTypeName"',
                    ])
                    ...
                    .leftJoin(
                        ...
                        "_ERPDefineItem",
                        's2',
                        'q."ProfiLevelSeq" = s2."IdSeq"',
                    ).leftJoin(
                        "_ERPDefineItem",
                        's3',
                        'q."LanguageSeq" = s3."IdSeq"',
                    ).leftJoin(
                        "_ERPDefineItem",
                        's4',
                        'q."CertifiTypeSeq" = s4."IdSeq"',
                    )
