...
import { ERPCareerItemRecruit } from '../entities/hr.careerItem.recruit.entity';

@Injectable()
export class ErpHrCareerItemRecruitService {
    constructor(
    ...
    HrCareerItemRecruitA(result: any[]): Observable<any> {
    ...
    HrCareerItemRecruitU(result: any[]): Observable<any> {
        if (!result || result.length === 0) {
    ...
    HrCareerItemRecruitD(records: any[]): Observable<any> {
        ...
        return from(this.dataSource.transaction(async (manager) => {
            try {
                ...
                const result = await manager.delete(ERPCareerItemRecruit, { IdSeq: In(ids) });
    ...
    HrCareerItemRecruitQ(result: any): Observable<any> {
        if (!result) {
            return of({
                success: false,
        ...
        return from(this.dataSource.transaction(async (manager) => {
            try {
                const queryBuilder = manager.createQueryBuilder(ERPCareerItemRecruit, 'q')
                    .select([
                        'q.IdSeq as "IdSeq"',
                        ...
                        'q.ProjectName as "ProjectName"',
                        'q.StartDate as "StartDate"',
                        'q.EndDate as "EndDate"',
                        'q.JobDescription as "JobDescription"',
                        'q.ProjectYears as "ProjectYears"',
                        'q.ProjectSummary as "ProjectSummary"',
                        'q.CareerRecruitSeq as "CareerRecruitSeq"',
                        'q.EmpSeq as "EmpSeq"',
                ...
                if (result.KeyItem6) {
                    queryBuilder.andWhere('q.CareerRecruitSeq = :KeyItem6', { KeyItem6: result.KeyItem6 });
                }
