...
@Injectable()
export class SPRWkMmEmpDaysService {
    constructor(
    ...
    SPRWkMmEmpDaysQ(
        ...
        const xmlDocument = this.generateXmlService.generateXMLSPRWkMmEmpDaysQ(result);

        const query = `
            EXEC _SPRWkMmEmpDaysQuery_WEB
    ...
    SPRWkMmEmpDaysObjQ(
        ...
        const query = `
            EXEC _SPRWkMmEmpDaysQueryObj_WEB
            @xmlDocument = N'${xmlDocument}',
            @xmlFlags = 2,
            @ServiceSeq = 2384,
            @WorkingTag = N'${result[0].WorkingTag || 'Q'}',
    ...
    private AutoCheckSPRWkMmEmpDays(result: any[], userSeq: number,
        ...
        const xmlDoc1 = this.generateXmlService.generateXMLSPRWkMmEmpDaysAUD(result, workingTag);
        const query1 = generateQuery(xmlDoc1, '_SPRWkMmEmpDaysCheck', 2384, 1020086);
        ...
        return forkJoin([
            ...
            switchMap(([data1]) => {
                ...
                const saveXmlDoc1 = this.generateXmlService.generateXMLSPRWkMmEmpDaysAUD(data1, workingTag);
                const saveQuery1 = generateQuery(saveXmlDoc1, '_SPRWkMmEmpDaysSave', 2384, 1020086);
    ...
    SPRWkMmEmpDaysAUD(result: any, companySeq: number, userSeq: number, workingTag: string): Observable<any> {
        return this.AutoCheckSPRWkMmEmpDays(result, companySeq, userSeq, result[0].WorkingTag);
    }
    ...
    SPRWkMmEmpDaysProc(
        ...
        const xmlDocument = this.generateXmlService.generateXMLSPRWkMmEmpDaysProc(result);

        const query = `
            EXEC VTN_SPRWkMmEmpDaysProc_WEB            
            @xmlDocument = N'${xmlDocument}',
            @xmlFlags = 2,
            @ServiceSeq = 10067,
            @WorkingTag = N'',
