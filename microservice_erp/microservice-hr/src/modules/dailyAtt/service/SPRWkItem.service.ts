...
export class SPRWkItemService {
    ...
    SPRWkItemQ(
        ...
        const xmlDocument = this.generateXmlService.generateXMLSPRWkItemQ(result);

        const query = `
            EXEC _SPRWkItemQuery_WEB
    ...
    private AutoCheckSPRWkItem(result: any[], userSeq: number,
        ...
        const xmlDoc1 = this.generateXmlService.generateXMLSPRWkItemAUD(result, workingTag);
        const query1 = generateQuery(xmlDoc1, '_SPRWkItemSaveCheck', 2290, 1628);
        ...
        return forkJoin([
            ...
            switchMap(([data1]) => {
                ...
                const saveXmlDoc1 = this.generateXmlService.generateXMLSPRWkItemAUD(data1, workingTag);
                const saveQuery1 = generateQuery(saveXmlDoc1, '_SPRWkItemSave', 2290, 1628);
    ...
    SPRWkItemAUD(result: any, companySeq: number, userSeq: number, workingTag: string): Observable<any> {
        return this.AutoCheckSPRWkItem(result, companySeq, userSeq, result[0].WorkingTag);
    }
