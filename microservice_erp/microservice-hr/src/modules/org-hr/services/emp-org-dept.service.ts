...
@Injectable()
export class EmpOrgDeptService {
  constructor(
  ...
  searchTreeEmpOrg(
    ...
    const xmlDocument = this.generateXmlService.searchEmpOrgTreeBy(result);
    const query = `
      EXEC _SHROrgDeptEmpCntQueryTree
  ...
  flattenTree(treeData: any) {
    ...
    function traverse(nodes: any, parent = null, level = 1) {
  ...
  getEmpByOrgDept(
    ...
    const xmlDocument = this.generateXmlService.getEmpByOrgDept(result);
    const query = `
      EXEC _SHROrgDeptByQuery_Web
        @xmlDocument = N'${xmlDocument}',
        @xmlFlags = 2,
        @ServiceSeq = 1879,
        @WorkingTag = N'',
