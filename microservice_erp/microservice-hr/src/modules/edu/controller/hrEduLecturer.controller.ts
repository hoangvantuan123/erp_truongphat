...
@Controller()
export class HrEduLecturerController {
  constructor(
    private readonly hrEduLecturerService: HrEduLecturerService,
  ...
  @GrpcMethod('HrEduLecturerService', 'searchEduLecturer')
  searchEduLecturer(request: any[]): Observable<MetadataResponse> {
    return this.handleGrpcRequest(
      request,
      this.hrEduLecturerService.searchEduLecturer.bind(this.hrEduLecturerService),
    );
  }

  @GrpcMethod('HrEduLecturerService', 'auEduLecturer')
  auEduLecturer(request: any): Observable<MetadataResponse> {
    return this.handleGrpcRequest(
      request,
      this.hrEduLecturerService.auEduLecturer.bind(this.hrEduLecturerService),
    );
  }

  @GrpcMethod('HrEduLecturerService', 'deleteEduLecturer')
  deleteEduLecturer(request: any): Observable<MetadataResponse> {
    return this.handleGrpcRequest(
      request,
      this.hrEduLecturerService.deleteEduLecturer.bind(this.hrEduLecturerService),
    );
  }
