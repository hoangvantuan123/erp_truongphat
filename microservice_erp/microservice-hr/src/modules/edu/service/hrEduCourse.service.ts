...
@Injectable()
export class HrEduCourseService {
  constructor(
  ...
  searchEduCourse(
    ...
    const xmlDocument =
      this.generateXmlService.generateXMLSearchEduCourse(result);
    const query = `
            EXEC _SHREduCourseQuery_WEB
  ...
  auEduCourse(result: any, companySeq: number, userSeq: number): Observable<any> {
    const xmlFlags = 2;
    const languageSeq = 6;
    const pgmSeq = 1858;
    const serviceSeq = 2856;
    const generateQuery = (
      xmlDocument: string,
      procedure: string,
      serviceSeq: number,
      pgmSeq: number,
            ...
            @PgmSeq = ${pgmSeq};
        `;
    const xmlDoc1 = this.generateXmlService.generateXMLEduCourse(result);
    ...
    return forkJoin([from(this.databaseService.executeQuery(query1))]).pipe(
      switchMap(([data1]) => {
        ...
        const saveXmlDoc1 = this.generateXmlService.generateXMLEduCourse(data1);
