...
export class GenerateXmlService {
      ...
      .map(
        (item, index) =>
          ...
          <DataBlock1>
            ...
            <ItemNameQry>${item?.ItemNameQry || ''}</ItemNameQry>
            <ItemNoQry>${item?.ItemNoQry || ''}</ItemNoQry>
            <SpecQry>${item?.SpecQry || ''}</SpecQry>
            <LotNo>${item?.LotNo || ''}</LotNo>
            <WorkOrderNo>${item?.WorkOrderNo || ''}</WorkOrderNo>
            <OSPItemNameQry>${item?.OSPItemNameQry || ''}</OSPItemNameQry>
            <OSPItemNoQry>${item?.OSPItemNoQry || ''}</OSPItemNoQry>
            <OSPSpecQry>${item?.OSPSpecQry || ''}</OSPSpecQry>
            <FactUnit>${item?.FactUnit || ''}</FactUnit>
            <FactUnitName>${item?.FactUnitName || ''}</FactUnitName>
            <DelvDateFrom>${item?.DelvDateFr || ''}</DelvDateFrom>
            <DelvDateTo>${item?.DelvDateTo || ''}</DelvDateTo>
            <QCDateFrom>${item?.QCDateFrom || ''}</QCDateFrom>
            <QCDateTo>${item?.QCDateTo || ''}</QCDateTo>
            <CustSeq>${item?.CustSeq || ''}</CustSeq>
            <SMQcType>${item?.SMQcType || ''}</SMQcType>
            <DelvNo>${item?.DelvNo || ''}</DelvNo>
      ...
      .map(
        (item, index) =>
          ...
          <DataBlock1>
            ...
            <OSPItemName>${this.escapeXml(item.OSPItemName || '')}</OSPItemName>
            <OSPItemNo>${this.escapeXml(item.OSPItemNo || '')}</OSPItemNo>
            <OSPSpec>${this.escapeXml(item.OSPSpec || '')}</OSPSpec>
            ...
            <SourceSerl>${this.escapeXml(item.SourceSerl || 0)}</SourceSerl>
            <SourceType>${this.escapeXml(item.SourceType || 0)}</SourceType>
      ...
      .map(
        (item, index) =>
          ...
          <DataBlock1>
            ...
            <SourceSeq>${item.SourceSeq || 0}</SourceSeq>
            <SourceType>${item.SourceType || 0}</SourceType>
          </DataBlock1>
  ...
  async saveSPDOSPDelvInAutoSave(result: any[]): Promise<string> {
    const xmlBlocks = result
      .map(
        (item, index) =>
          `
          <DataBlock1>
            ...
            <OSPDelvSeq>${item?.SourceSeq}</OSPDelvSeq>
            <OSPDelvSerl>${item?.SourceSerl}</OSPDelvSerl>
            <OSPAssySeq>${item?.ItemSeq}</OSPAssySeq>
            <OSPDelvInDate>${item?.TestEndDate}</OSPDelvInDate>
            <Qty>${item?.ReqQty}</Qty>
            <EmpSeq>${item?.EmpSeq}</EmpSeq>
  ...
  async saveQcTestReportItemSave(
    result: Array<{ [key: string]: any }>,
  ): Promise<string> {
    const xmlBlocks = result
      .map(
  ...
  async getQcListTestReportCheck(
    result: Array<{ [key: string]: any }>,
  ): Promise<string> {
    const xmlBlocks = result
      .map(
      ...
      .map(
        (item, index) =>
          ...
          <DataBlock1>
            ...
            <TABLE_NAME>DataBlock1</TABLE_NAME>
            <IsChangedMst>0</IsChangedMst>
            <ItemClassSSeq>${this.escapeXml(item?.ItemClassSSeq) || ''}</ItemClassSSeq>
  ...
  async delvInAutoSave(result: Array<{ [key: string]: any }>) {
    const xmlBlocks = result
      .map(
        (item, index) =>
          `
  ...
  async checkDeleteQcOutsourceTestReport(
    result: Array<{ [key: string]: any }>,
  ): Promise<string> {
    const xmlBlocks = result
      .map(
