...
export class GenerateXmlService {
      ...
      .map(
        (item, index) =>
          ...
          <DataBlock1>
            ...
            <DelvDateFrom>${this.escapeXml(item.DelvDateFr || '')}</DelvDateFrom>
      ...
      .map(
        (item, index) =>
          ...
          <DataBlock1>
            ...
            <PJTName>${this.escapeXml(item.PJTName || '')}</PJTName>
            <PJTNo>${this.escapeXml(item.PJTNo || '')}</PJTNo>
            <PJTSeq>${this.escapeXml(item.PJTSeq || 0)}</PJTSeq>
            <WBSSeq>${this.escapeXml(item.WBSSeq || 0)}</WBSSeq>
            <ItemName>${this.escapeXml(item.ItemName || '')}</ItemName>
            <ItemNo>${this.escapeXml(item.ItemNo || '')}</ItemNo>
            <Spec>${this.escapeXml(item.Spec || '')}</Spec>
  ...
  async saveQcTestReport(resultCheck: any): Promise<string> {
    const xmlBlocks = 
          ...
          <DataBlock1>
            ...
            <SourceType>${this.escapeXml(resultCheck.SourceType || 1)}</SourceType>
  ...
  async getQcPurchaseListTestReportCheck(result: Array<{ [key: string]: any }>): Promise<string> {
    const xmlBlocks = result
      .map(
        (item, index) =>
          `
      ...
      .map(
        (item, index) =>
          ...
          <DataBlock1>
            ...
            <Status>0</Status>
            <SourceType>${item?.SourceType}</SourceType>
            <DelvSeq>${item?.SourceSeq}</DelvSeq>
            <DelvSerl>${item?.SourceSerl}</DelvSerl>
            <DelvInDate>${item?.TestEndDate}</DelvInDate>
            <Qty>${item?.ReqQty}</Qty>
  ...
  async checkDeleteQcPurchaseTestReport(result: Array<{ [key: string]: any }>): Promise<string> {
    const xmlBlocks = result
      .map(
        (item, index) =>
          `
