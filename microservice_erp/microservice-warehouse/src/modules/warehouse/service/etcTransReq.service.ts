...
export class EtcTransReqService {
  constructor(
    ...
    private readonly generateXmlService: GenerateXmlEtcTransService,
  ) {}
...
`;
  ...
  async SLGInOutInventoryCheckWEB(
    ...
    const checkResult = async () => {
      try {
        const xmlDocumentCheck =
          await this.generateXmlService.generateXMLSLGInOutInventoryCheckWEB(
            result,
          );
        const queryCheck = generateQuery(
          xmlDocumentCheck,
          '_SLGInOutInventoryTransCheck_WEB',
        );
        const resultCheck = await this.databaseService.executeQuery(queryCheck);
        return { success: true, data: resultCheck };
      } catch (error) {
      ...
      try {
        ...
        if (dataSheetAUD.length > 0) {
          const dataSheetAUDUpdate = dataSheetAUD.map((item) => {
            return {
              ...item,
              InOutSeq: resultMasterCheck[0].InOutSeq,
