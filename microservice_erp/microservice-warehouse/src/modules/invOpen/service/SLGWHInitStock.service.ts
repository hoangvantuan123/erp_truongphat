...
@Injectable()
export class SLGWHInitStockService {
    constructor(
    ...
    SLGWHInitStockQ(
        ...
        const xmlDocument = this.generateXmlService.generateXMLSLGWHInitStockQ(result);

        const query = `
            EXEC _SLGWHInitStockQuery_WEB
    ...
    private AutoCheckSLGWHInitStock(result: any[], userSeq: number,
        ...
        const xmlDoc1 = this.generateXmlService.generateXMLSLGWHInitStockAUD(result, workingTag);
        const query1 = generateQuery(xmlDoc1, '_SLGWHInitStockCheck', 4817, 5519);
        ...
        return forkJoin([
            ...
            switchMap(([data1]) => {
                ...
                const saveXmlDoc1 = this.generateXmlService.generateXMLSLGWHInitStockAUD(data1, workingTag);
                const saveQuery1 = generateQuery(saveXmlDoc1, '_SLGWHInitStockSave', 4817, 5519);
    ...
    SLGWHInitStockAUD(result: any, companySeq: number, userSeq: number, workingTag: string): Observable<any> {
        return this.AutoCheckSLGWHInitStock(result, companySeq, userSeq, result[0].WorkingTag);
    }
