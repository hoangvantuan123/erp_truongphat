...
@Injectable()
export class DatabaseService230427 {
  private queryRunner: QueryRunner;
  private activeQueries = 0; // Số truy vấn đang chạy
  private logger = new Logger(DatabaseService230427.name);
  constructor(
    @InjectConnection('ITMV230427') private readonly ITMV230427: Connection,
  ) {
    this.queryRunner = this.ITMV230427.createQueryRunner();
  ...
  executeQuery(
    ...
    const queryRunner = this.ITMV230427.createQueryRunner(); // Tạo QueryRunner riêng biệt
    ...
    this.logger.log(
      `🔵 [START] Query ${queryId} - Priority: ${priority} - Active Queries: ${this.activeQueries}`,
    );
    ...
    return queryRunner
      ...
      .then((result) => {
        return (
          useTransaction ? queryRunner.commitTransaction() : Promise.resolve()
        ).then(() => {
          const endTime = Date.now();
          this.logger.log(
            `✅ [DONE] Query ${queryId} - Time: ${endTime - startTime}ms - Active Queries: ${this.activeQueries - 1}`,
          );
          return result;
        });
      })
      .catch((error) => {
        return (
          useTransaction ? queryRunner.rollbackTransaction() : Promise.resolve()
        ).then(() => {
          this.logger.error(
            `❌ [FAILED] Query ${queryId} - Error: ${error.message}`,
          );
  ...
  private async checkConnection() {
    try {
      if (this.ITMV230427.isConnected) {
    ...
    try {
      ...
      if (!result || result.length === 0) {
        throw new NotFoundException(
          `UserSeq ${UserSeq} not found in the system`,
        );
      }
