...
interface EmpOrgDeptService {
  searchTreeEmpOrg(data: {
    result: any;
...
export class GrpcEmpOrgDeptService {
  ...
  searchTreeEmpOrg(
    ...
    const metadataService = client.getService<EmpOrgDeptService>('EmpOrgDeptService');
    return metadataService.searchTreeEmpOrg({ result, metadata }).pipe(
      map((grpcResponse) => ({
  ...
  getEmpByDept(
    ...
    const client = this.createGrpcClient();
    const metadataService = client.getService<EmpOrgDeptService>('EmpOrgDeptService');
    return metadataService.getEmpByDept({ result, metadata }).pipe(
      map((grpcResponse) => ({
        success: grpcResponse?.status ?? false,
        message: grpcResponse?.message || 'No message received',
        data: grpcResponse?.data || null,
