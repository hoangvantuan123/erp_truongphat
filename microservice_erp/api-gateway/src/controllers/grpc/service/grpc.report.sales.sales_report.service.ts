...
interface SalesReportService {
    SSLEISItemAnalysisQ(data: { result: any[]; metadata: Record<string, string> }): Observable<any>;
}

@Injectable()
export class GrpcSalesReportService {
    ...
    private createGrpcClient(): ClientGrpc {
        return ClientProxyFactory.create({
            ...
            options: {
                ...
                package: ['report.sales.sales_report',],
                protoPath: [
    ...
    SSLEISItemAnalysisQ(result: any, metadata: Record<string, string> = {}): Observable<any> {
        const client = this.createGrpcClient();
        const metadataService = client.getService<SalesReportService>('SalesReportService');
        return metadataService.SSLEISItemAnalysisQ({ result, metadata }).pipe(
            map((grpcResponse) => ({
