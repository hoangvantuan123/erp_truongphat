...
interface HrFileService {
    HrFileQ(data: { result: any; metadata: Record<string, string> }): Observable<any>;
    HrFileD(data: { result: any; metadata: Record<string, string> }): Observable<any>;
}

@Injectable()
export class GrpcHrFileService {
    ...
    HrFileQ(result: any, metadata: Record<string, string> = {}): Observable<any> {
        const client = this.createGrpcClient();
        const metadataService = client.getService<HrFileService>('HrFileService');
        return metadataService.HrFileQ({ result, metadata }).pipe(
            map((grpcResponse) => ({
    ...
    HrFileD(result: any, metadata: Record<string, string> = {}): Observable<any> {
        const client = this.createGrpcClient();
        const metadataService = client.getService<HrFileService>('HrFileService');
        return metadataService.HrFileD({ result, metadata }).pipe(
            map((grpcResponse) => ({
                success: grpcResponse?.status ?? false,
                message: grpcResponse?.message || 'No message received',
                data: grpcResponse?.data || null,
