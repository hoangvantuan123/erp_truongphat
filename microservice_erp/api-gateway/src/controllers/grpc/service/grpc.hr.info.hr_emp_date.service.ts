...
interface HrEmpDateService {
    HrEmpDateQ(data: { result: any; metadata: Record<string, string> }): Observable<any>;
}

@Injectable()
export class GrpcHrEmpDateService {
    ...
    HrEmpDateQ(result: any, metadata: Record<string, string> = {}): Observable<any> {
        const client = this.createGrpcClient();
        const metadataService = client.getService<HrEmpDateService>('HrEmpDateService');
        return metadataService.HrEmpDateQ({ result, metadata }).pipe(
            map((grpcResponse) => ({
