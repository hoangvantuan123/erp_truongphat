...
interface UploadHrEmpInfoService {
    EmpInfoRptQ(data: { result: any; metadata: Record<string, string> }): Observable<any>;
}

@Injectable()
export class GrpcUploadHrEmpInfoService {
    ...
    private createGrpcClient(): ClientGrpc {
        return ClientProxyFactory.create({
            ...
            options: {
                ...
                package: ['upload.hr.hr_emp_info',],
                protoPath: [
                    join(__dirname, '..', '..', '..', '..', '..', 'proto', 'upload', 'hr', 'hr_emp_info.proto'),
                ],
                loader: {
                    keepCase: true,
                    longs: String,
    ...
    EmpInfoRptQ(result: any, metadata: Record<string, string> = {}): Observable<any> {
        const client = this.createGrpcClient();
        const metadataService = client.getService<UploadHrEmpInfoService>('UploadHrEmpInfoService');
        return metadataService.EmpInfoRptQ({ result, metadata }).pipe(
            map((grpcResponse) => ({
