...
export class LotMasterController {
    ...
    async SLGLotNoMasterQueryWEB(
        ...
        const resu = await this.sendRequest('lg-lotno-master-query', { result, authorization });
        return res.status(HttpStatus.OK).json(resu);
    }
    @Post('lg-lotno-master-create')
    async SLGLotNoMasterAUD(
        ...
        const authorization = this.getAuthorization(req);
        const resu = await this.sendRequest('lg-lotno-master-create', { result, authorization });
        return res.status(HttpStatus.OK).json(resu);
    }
