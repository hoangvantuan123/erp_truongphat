...
export class BizStockListController {
    ...
    @Post('slg-wh-biz-stock-list-query')
    async processSLGBizUnitStockListQuery(
        @Body('result') result: any[],
        @Req() req: Request,
        @Res() res: Response,
    ) {
        const authorization = this.getAuthorization(req);
        const resu = await this.sendRequest('slg-wh-biz-stock-list-query', { result, authorization });
