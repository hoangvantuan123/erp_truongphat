...
export class ImpPermitController {
    constructor(
    ...
    private async sendRequest(pattern: string, payload: any) {
        ...
        try {
            const result = await firstValueFrom(
                this.purchaseService.send(pattern, payload).pipe(
    ...
    async SSLImpPermitListQuery(
        ...
        const resu = await this.sendRequest('simp-permit-sheet-query', { result, authorization });
        return res.status(HttpStatus.OK).json(resu);
    }
    @Post('simp-permit-list-stop')
    async SSLImpPermitStop(
        ...
        const authorization = this.getAuthorization(req);
        const resu = await this.sendRequest('simp-permit-list-stop', { result, authorization });
        return res.status(HttpStatus.OK).json(resu);
    }
