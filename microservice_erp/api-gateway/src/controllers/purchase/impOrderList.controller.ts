...
export class ImpOrderListController {
  ...
  @Post('simp-order-list-sheet-query')
  async SIMPORDERListQueryWEB(
    ...
    const resu = await this.sendRequest('simp-order-list-sheet-query', {
      result,
      authorization,
    });
  ...
  async SCOMConfirmWEBPO(
    ...
    const authorization = this.getAuthorization(req);
    const resu = await this.sendRequest('simp-order-list-confirm', {
      result,
      authorization,
    });
    return res.status(HttpStatus.OK).json(resu);
  ...
  async SPOStopWEB(
    ...
    const authorization = this.getAuthorization(req);
    const resu = await this.sendRequest('simp-order-list-stop', {
      result,
      authorization,
    });
    return res.status(HttpStatus.OK).json(resu);
