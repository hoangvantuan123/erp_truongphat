...
export class ImpDeliveryItemListController {
  ...
  private async sendRequest(pattern: string, payload: any) {
    const requestTimeout = parseInt(process.env.REQUEST_TIMEOUT || '1360000');
  ...
  @Post('simp-delivery-item-sheet-query')
  async SSLImpDeliveryItemListQuery(
    ...
    const resu = await this.sendRequest('simp-delivery-item-sheet-query', {
      result,
      authorization,
    });
