...
export class PurOrdPOListController {
  constructor(
  ...
  async SPUORDPOListQueryWEB(
    ...
    const resu = await this.sendRequest('sord-po-list-sheet-query', {
      result,
      authorization,
    });
  ...
  async SCOMConfirmWEBPO(
    ...
    const authorization = this.getAuthorization(req);
    const resu = await this.sendRequest('sord-po-list-confirm', {
      result,
      authorization,
    });
    return res.status(HttpStatus.OK).json(resu);
  ...
  async SPOStopWEB(
    ...
    const authorization = this.getAuthorization(req);
    const resu = await this.sendRequest('sord-po-list-stop', {
      result,
      authorization,
    });
    return res.status(HttpStatus.OK).json(resu);
