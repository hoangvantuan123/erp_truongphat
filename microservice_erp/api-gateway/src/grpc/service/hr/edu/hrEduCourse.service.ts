...
interface HrEduCourseService {
  searchEduCourse(data: {
    result: any[];
...
export class GrpcEduCourseService {
  ...
  searchEduCourse(
    ...
    const metadataService =
      client.getService<HrEduCourseService>('HrEduCourseService');
    return metadataService.searchEduCourse({ result, metadata }).pipe(
      map((grpcResponse) => ({
  ...
  auEduCourse(
    ...
    const metadataService =
      client.getService<HrEduCourseService>('HrEduCourseService');
    return metadataService.auEduCourse({ result, metadata }).pipe(
      map((grpcResponse) => ({
        success: grpcResponse?.success ?? false,
        message: grpcResponse?.message || 'No message received',
        data: grpcResponse?.data || null,
  ...
  deleteEduCourse(
    ...
    const metadataService =
      client.getService<HrEduCourseService>('HrEduCourseService');
    return metadataService.deleteEduCourse({ result, metadata }).pipe(
      map((grpcResponse) => ({
        success: grpcResponse?.success ?? false,
        message: grpcResponse?.message || 'No message received',
        data: grpcResponse?.data || null,
