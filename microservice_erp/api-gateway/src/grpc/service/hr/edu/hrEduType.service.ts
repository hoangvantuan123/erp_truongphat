...
export class GrpcEduTypeService {
  ...
  searchEduType(
    ...
    const metadataService =
      client.getService<HrEduTypeService>('HrEduTypeService');
    return metadataService.searchEduType({ result, metadata }).pipe(
      map((grpcResponse) => ({
  ...
  auEduType(
    ...
    const metadataService =
      client.getService<HrEduTypeService>('HrEduTypeService');
    return metadataService.auEduType({ result, metadata }).pipe(
      map((grpcResponse) => ({
        success: grpcResponse?.success ?? false,
        message: grpcResponse?.message || 'No message received',
        data: grpcResponse?.data || null,
  ...
  deleteEduType(
    ...
    const metadataService =
      client.getService<HrEduTypeService>('HrEduTypeService');
    return metadataService.deleteEduType({ result, metadata }).pipe(
      map((grpcResponse) => ({
        success: grpcResponse?.success ?? false,
        message: grpcResponse?.message || 'No message received',
        data: grpcResponse?.data || null,
