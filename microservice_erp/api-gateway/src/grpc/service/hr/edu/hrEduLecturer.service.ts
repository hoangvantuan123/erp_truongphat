...
interface HrEduLecturerService {
  searchEduLecturer(data: {
    result: any[];
...
export class GrpcEduLecturerService {
  ...
  searchEduLecturer(
    ...
    const metadataService =
      client.getService<HrEduLecturerService>('HrEduLecturerService');
    return metadataService.searchEduLecturer({ result, metadata }).pipe(
      map((grpcResponse) => ({
  ...
  auEduLecturer(
    ...
    const metadataService =
      client.getService<HrEduLecturerService>('HrEduLecturerService');
    return metadataService.auEduLecturer({ result, metadata }).pipe(
      map((grpcResponse) => ({
        success: grpcResponse?.success ?? false,
        message: grpcResponse?.message || 'No message received',
        data: grpcResponse?.data || null,
  ...
  deleteEduLecturer(
    ...
    const metadataService =
      client.getService<HrEduLecturerService>('HrEduLecturerService');
    return metadataService.deleteEduLecturer({ result, metadata }).pipe(
      map((grpcResponse) => ({
        success: grpcResponse?.success ?? false,
        message: grpcResponse?.message || 'No message received',
        data: grpcResponse?.data || null,
