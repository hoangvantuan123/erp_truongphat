...
interface BOMService {
    SPDBOMReportAllQ(data: { result: any[]; metadata: Record<string, string> }): Observable<any>;

}

@Injectable()
export class GrpcBOMService {
    private createGrpcClient(): ClientGrpc {
        return ClientProxyFactory.create({
            transport: Transport.GRPC,
            options: {
                url: process.env.HOST_RGPC_PDMM,
                package: ['produce.bom.bom'],
                protoPath: [
    ...
    SPDBOMReportAllQ(result: any, metadata: Record<string, string> = {}): Observable<any> {
        const client = this.createGrpcClient();
        const metadataService = client.getService<BOMService>('BOMService');
        return metadataService.SPDBOMReportAllQ({ result, metadata }).pipe(
            map((grpcResponse) => ({
