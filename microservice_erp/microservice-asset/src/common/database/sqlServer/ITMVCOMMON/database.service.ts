...
@Injectable()
export class DatabaseServiceCommon {
  private queryRunner: QueryRunner;
  private activeQueries = 0; 
  private logger = new Logger(DatabaseServiceCommon.name);
  constructor(@InjectConnection('ITMVCOMMON') private readonly ITMVCOMMON: Connection) {
    this.queryRunner = this.ITMVCOMMON.createQueryRunner();
    this.checkConnection();
  }

  private async checkConnection() {
    try {
      if (this.ITMVCOMMON.isConnected) {
  ...
  async executeQuery(query: string): Promise<any> {
    const queryRunner = this.ITMVCOMMON.createQueryRunner();
  ...
  executeQueryVer02(query: string, useTransaction = false, priority = 200): Observable<any> {
    return new Observable(subscriber => {
      const queryRunner = this.ITMVCOMMON.createQueryRunner(); // Tạo QueryRunner riêng biệt
